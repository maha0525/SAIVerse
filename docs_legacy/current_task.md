...
- [x] **`saiverse_manager.py`の修正**:
  - [x] `execute_tool(tool_id, persona_id, arguments)`メソッドを実装する。このメソッドは、DBからモジュールパスと関数名を取得し、`importlib`を用いて動的にツールを実行する。
...
#### タスク 4.2.3: ワールドエディタのUI拡張 (完了)

- [x] **`main.py`の修正**:
  - [x] ワールドエディタに「ツール管理」セクションを新設し、ツールのCRUD UIを実装した。
  - [x] 「Building管理」セクションを拡張し、各Buildingに利用可能なツールを紐付けるUIを実装した。
