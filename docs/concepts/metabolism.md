...
3. **提示の節約**（2026-09-09、intent [`presented_context_reduction.md`](../intent/presented_context_reduction.md) 設計 1・2）: head を描き直した**後**に、用の済んだ操作通知（スペルの増減・コア記憶などの操作のお知らせ — head が今の状態を見せた時点で重複）を提示から下ろし、**現在地でない部屋の様子**を一行へ縮めて画像も外す。やるのは記録の追加だけで、台帳の行も確定文面も書き換えない — 縮んだ姿は提示を組むときに写しの上で作られる。**この縮みが起きるのは Metabolism の瞬間だけ**（移動や発言で提示の途中を書き換えるとプロンプトキャッシュの前方一致が割れる）。実装は `sai_memory/presented_reduction.py`、発火は `SessionLifecycle._reduce_presented_perceptions`。**入口の門で引き返す回にも縮みが走る**（2026-09-10）: 4 つの入口はどれも「会話の行が残す量以下」だと本体へ入らないので、会話がずっと小さく知覚だけが大きいペルソナは縮みに届かない。だから門が共有する一点（`_handle_perception_over_budget` = 超過の旗を立てるところ）で、`has_pending_reductions` が「縮めるものがある」と答えたときにだけ head の描き直し + 縮みを走らせる（LLM は呼ばない。無ければ描き直しごと見送るので、超過が続く間に前置きが毎ターン書き変わることはない）
4. **新しい Session を開始する**
