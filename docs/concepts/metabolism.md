...
**context 過剰の判定は文字数の二数** = 上限と残す量（2026-07-28、intent [`arasuji_levels.md`](../intent/arasuji_levels.md) §9）。旧「モデルごとのメッセージ数」は 2026-07-25 に単位ごと廃止（三水位へ）、三水位の低水位は 2026-07-28 に eviction から未使用化（残す量が保護を兼ねる）を経て、**2026-09-04 に廃止**（最後に残っていた役割 = anchor 未確立時の初期読み込み量は残す量を流用する。発話直前の最終防衛ライン §15-5 が窓を残す量まで埋め直すため、独立した値として効く場面が無かった — [issue](../issues/archive/watermarks_unsatisfiable_when_perception_is_large.md) 裁定 5）。モデル JSON や旧 DB に残る `metabolism_low_chars` キーは黙って無視される。

| 数 | 意味 | 組み込み既定 | モデル設定キー |
|---|---|---|---|
| 残す量 | 畳んだ後に残す直近の**会話の行**の文字数 = 保護範囲。**主語は会話の行だけ**（知覚ブロックは消費しない — 2026-09-03 裁定。スペル結果・通知などの機構名義の行も消費しない — 2026-09-04 裁定。機構行は畳みの対象には含まれたまま、数えないだけ）。上限との差がバッファで、発火は「たまに・まとめて」になる（キャッシュ保護）。anchor 未確立（新規ペルソナ / 修復直後）の初期読み込み量もこの値 | 20,000 | `metabolism_target_chars` |
| 上限 | これを超えたら発火（未設定 = null なら `token_triggered` のみで発火）。**主語は実際に送る合計**（会話の行 + 知覚ブロック） | 60,000 | `metabolism_high_chars` |

**しきい値はこの一組だけ**（2026-09-09、intent [`presented_context_reduction.md`](../intent/presented_context_reduction.md) 設計 3）。2026-09-05 に置いた知覚専用の二水位（`perception_high_chars` / `perception_target_chars`）は廃止した — 「会話を畳んでも減らせない大物」だった操作通知と現在地でない部屋の様子が、Metabolism の瞬間の縮み（下の「発火時にやること」3）で減るようになったため、知覚だけの上限を持つ理由が消えた。モデル JSON や旧 DB に残る `perception_*_chars` キーは黙って無視される（`metabolism_low_chars` と同じデータ互換の形）。規則で縮めても合計が上限を下回らない回は、黙って切らずに超過の旗が立つ（`perception_over_budget`）。

なお、廃止したのは「知覚の合計がしきい値を超えたら古い側を下ろす」という引き金だけで、**下ろした境界の器は残っている** — 廃止前に下ろされた区間はそのまま提示に戻らず、跡地の機構名義の一行（`[省略された記録]` + 件数）も出続ける。件数は下ろした**記録の数**であってバッチ（1 回の知覚）の数ではない。台帳は無傷なので、その期間の編纂が来れば材料として引き取られる。

**組み込み既定はプリセットの「デフォルト」そのもの**（2026-09-09 まはー裁定）。全体設定の画面は 3 段のプリセット（多い = 40,000 / 120,000、デフォルト = 20,000 / 60,000、少ない = 10,000 / 30,000）と「カスタム」を出す。どの段も 残す量 : 上限 = 1 : 3 で、幅（上限 − 残す量）が整理の頻度を決めるので、小さい段ほど整理の LLM 呼び出しとキャッシュの読み直しが増えうる（画面はその注意書きを添える）。定義は `saiverse/model_configs.py` の `METABOLISM_PRESETS` の一枚に置き、`GET /api/config/metabolism-defaults` の `presets` として画面へ渡す（画面に数字を書き写すと、既定を動かしたときにプリセットだけ古くなる）。
...
**保存時検査**（`api/routes/config.py` の `_watermark_constraints_error` 一本。モデルの作成・更新・複製・チャットからの保存・全体設定の PUT が全部ここを通る）は二つ:

1. **型**: 数値でも null でもない値を弾く（実行時は既定へ黙って落ちるが、保存の入口では教える）。
2. **順序**: 残す量 ≤ 上限。空欄のキーは**全体設定込みの実効既定**で埋めて比べる。

（2026-09-05 に置いた三つ目の「余裕」の検査 — 整理をはじめる量 − 残す量 > 知覚の上限 + 余裕 — は、知覚の二水位ごと 2026-09-09 に撤去した。二系統をまたぐ検査だったので、系統が一つになった時点で問い自体が消えている。この検査が保存を拒み続けた実害の記録は [issue](../issues/archive/watermarks_unsatisfiable_when_perception_is_large.md)。）

逆向きも守る: 全体設定の変更が、水位の一部だけを書いている既存モデルの実効の組を壊す（例: target=25万 だけ書いたモデルがあるのに全体の high を 6万 に戻す）ときは保存を拒み、エラーはそのモデル名を挙げる（先にモデル側を直すか空欄にする）。既に保存済みの壊れた組は起動時に直さない — 実行時の縮退（水位逆転のクランプ）が受け止める。
...
| **失効後の先回り畳み**（§14-4） | EventScheduler の定期見張り（10 分周期）→ `cold_precompaction_status` / `run_cold_precompaction` | 全 anchor 行が冷え切った + 提示ウィンドウが残す量と上限の**中間**を超過。編纂の総作業量は畳む時期によらず不変なので、前倒しで「冷えた再開時の定価読み」だけが消える。Chronicle 生成が有効（自律確認 ON）な persona のみ。**加えて、前回の試行から保存行の状態が変わっていない persona には畳みを試さない**（2026-09-10、intent §14-4 追補 — 記録はプロセスのメモリだけで、再起動と設定の再読込で消える） |
...
3. **提示の節約**（2026-09-09、intent [`presented_context_reduction.md`](../intent/presented_context_reduction.md) 設計 1・2）: head を描き直した**後**に、用の済んだ操作通知（スペルの増減・コア記憶などの操作のお知らせ — head が今の状態を見せた時点で重複）を提示から下ろし、**現在地でない部屋の様子**を一行へ縮めて画像も外す。やるのは記録の追加だけで、台帳の行も確定文面も書き換えない — 縮んだ姿は提示を組むときに写しの上で作られる。**この縮みが起きるのは Metabolism の瞬間だけ**（移動や発言で提示の途中を書き換えるとプロンプトキャッシュの前方一致が割れる）。実装は `sai_memory/presented_reduction.py`、発火は `SessionLifecycle.reduce_presentation`（head の描き直し → 縮み、をこの順で行う一枚）。**操作通知が下りるのはモデルごと**（2026-09-10）: head は (persona, model) ごとに描き直されるので、あるモデルの Metabolism で全モデルの提示から通知を消すと、head が凍結されたままのモデルは変更を知る手段を失う。だから通知の境界はモデルを主キーにした表（`perception_notice_presentation`）で持ち、そのモデルの head を**描き直せた回だけ**進める（`on_metabolism` は失敗を `False` で返すので戻り値を見る）。部屋の様子は head に載らないので、縮めた印はペルソナ共通のまま。**Metabolism 本体へ入れずに引き返す回にも縮みが走る**（同日）: 入口の門（会話の行が残す量以下）・退場の見送り（編纂やスルースの失敗）・429 の小休止はどれも本体に届かないので、そのままだと提示が育ちっぱなしになる。これらの回は `has_pending_reductions` が「縮めるものがある」と答えたときにだけ head の描き直し + 縮みを走らせる（LLM は呼ばない。無ければ描き直しごと見送るので、状態が続く間に前置きが毎ターン書き変わることはない）
4. **新しい Session を開始する**
