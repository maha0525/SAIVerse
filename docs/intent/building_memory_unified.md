...
- v0.3.0 より前の部屋は、ID に `/` を含むことがある (表示名「2/28」→ `2/28_city_a`)。その部屋の log.json は 2 段のフォルダの奥にあり、取り込みが毎起動空振りしながら警告だけが出続けた。**この検算より前 (部屋を読み込む前) に、区切り記号 (`/` `\`) を `_` に置き換えた ID へ付け替える** (`saiverse/building_id_repair.py`)。付け替えの後は、この検算が新しい 1 段のフォルダから会話を移す。古いファイルでの元のメッセージ ID (`legacy_message_id`) は、この検算の突き合わせに使うので変えない。
- **確認処理と取り込み処理は、同じ関数で部屋のファイルの場所を決める** (`legacy_log_path` → `_child_by_name`)。付け替えを見送って区切り記号入りの ID が残った部屋は、確認処理でも「フォルダ名として使えない」として `check_failed` に出す。「移せていない」と「対象 0 件」が同時に成立する状態を作らない。
