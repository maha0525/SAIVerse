...
2. **スキップ判定は「現物が読めるか」だけ**: マーカーファイルの有無では判定しない。「取り込み痕跡 (legacy_seq 付き行) が既にある」= 冪等 skip、「痕跡なしで通常経路の行がある」= 順序保護のため skip (自動マージしない)。cursor 取り込みは生きた cursor 行がある環境では何もしない (稼働中の値を古いファイルで上書きしない)。取り込みを実行した部屋では、既存の cursor 行を取り込み末尾まで前進させる — 過去ログを「未読の新着」に化けさせない (cursor=0 のまま数百件を取り込むと、次の Pulse が全件を一気に消化しに行く。ファイル側の ingested_by は疎らで印だけでは防げないことを 2026-08-16 に実測)。リリース版ユーザーの経路ではこの時点で cursor 行が無く、位置は後続の conscious_log 取り込みが決める。なお conscious_log の remap は「既読位置の尊重」なので、アップグレード前からの未読残があるペルソナはその分を新着として消化する — これは既存設計のままで、未読残が大きい環境での体験は未検証の開放点。
