...
7. **読んだ位置が分からないとき、たまっている履歴を一度に読ませない** (2026-08-16 追加): ペルソナの「この部屋をどこまで読んだか」の記録が無いとき、既定は「**起動時にひかえた末尾**」であって 0 (= 1 件目から全部未読) ではない。0 に落とすと部屋の全履歴が一度に個人記憶へ転記される。転記に件数の上限は無く、そのまま有料の推論に載る。失うのは「読まれなかったかもしれない過去」だけなので、分からない側のコストが小さい向きへ倒す。**この判断はもともと入室処理 (`_mark_entry` の初回訪問) だけに置かれていて、入室を通らない経路 (起動時に既にその部屋にいる / 旧バージョンから記録が移っていない / 記録の読み込みに失敗した) では逆の答えが出ていた。** 歯止めは経路ごとではなく、値が使われる場所 (`_ingest_round`) に置く。

   **境界を数える時点は「誰も書き込めない時点」でなければならない** (2026-08-16 Codex 指摘)。読みに来た時点の末尾を境界にすると、起動からそのペルソナが最初に喋るまでの間に**ユーザーが送ったメッセージ**まで既読になり、一度も転記されない。そこで manager 初期化時に部屋ごとの末尾をひかえ (`startup_seq_watermark`)、記録が無いときはその水位を使う。

   水位は**登録済みの全部屋を 0 で埋めてから** MAX(seq) で上書きする。行がある部屋だけで作ると空の部屋が漏れ、そこへ届いた最初のメッセージがそのまま境界になって誰にも読まれない (同 2 巡目の指摘。同じ穴が空室で再現していた)。水位に載っていない部屋 = 起動後に作られた部屋なので、そこは 0 (最初から読む) でよい — 作られた時点で空なので読み過ぎにならない。**フォールバックで「現在の末尾」を数えてはいけない**、それがこの穴の正体だった。水位そのものを測れなかったときは全部屋 0 のまま進み、起動アラートに出す (読み過ぎは高くつくが、ユーザーのメッセージを黙って捨てるよりはよい)。
...
**ステータス**: 完了 (2026-08-16 実機検証済み — テスタロッサの部屋の 360 件が UI に復帰、ナビの部屋の 94 件も既存 72 件の前へ自動で取り込み、一気読みなし)。経緯とレビュー 3 巡の記録は [ハンドオフ](../handoff/2026-08-16_legacy_log_import_prevention_handoff.md)。
...
2. **取り込んだ過去ログは 0 より小さい seq を持つ** (2026-08-16 確立): 通常の発言は 1 から採番されるので、過去ログは必ずその手前に並ぶ。この一本の規則で 3 つのことが同時に片付く。**並び順** — 既に会話が始まっている部屋でも古い会話が時系列どおり前に入り、既存の行は 1 つも動かない (行の seq / message_id は、ペルソナ個人の記憶に残る転記元の目印や AddonMessageMetadata から参照されている。番号を振り直すとそれらが全部ずれ、同じ会話を二重に記憶しかねない)。**この約束の例外は部屋 ID の付け替えだけ** (2026-09-11、[issue](../issues/building_id_contains_path_separator.md)): フォルダ名や URL を壊す文字を含む古い部屋 ID を付け替えるときは、message_id を新しい部屋 ID の形に動かし、参照している側 (AddonMessageMetadata、ペルソナの記憶の転記元の目印) も同じ付け替えで書き換える。取り込みについての約束は変わらない。**既読の扱い** — 「どこまで読んだか」は 0 以上なので、負の seq は常に既読側に入る。過去ログを「未読の新着」に化けさせないための cursor 操作が要らなくなり、前進先を間違えて実末尾を追い越す事故も起きない。**見分け** — その部屋の過去ログは `seq < 0` で正確に引ける。

   入れるのは **まだ DB に居ない発言だけ**。判定は検算と同じ規則 (ファイルの各発言の message_id が DB のどこにも無いものが欠け) を使う — 「取り込む対象」と「欠けとして警告する対象」がずれないため。message_id を持たない発言は、その部屋に行が 1 つも無いとき (= ファイルが唯一の写し) だけ入れる。行があるときは既に入っているのか未取り込みなのか見分けられないので触らない (入れると二重に並ぶ)。この規則があるので、取り込みは印の有無に頼らず冪等になる。

   スキップ判定にマーカーファイル (`log.json.corrupted_*`) の有無は使わない。cursor 取り込みは生きた cursor 行がある環境では何もしない (稼働中の値を古いファイルで上書きしない)。なお conscious_log の remap は「既読位置の尊重」なので、アップグレード前からの未読残があるペルソナはその分を新着として消化する — 既存設計のままで、未読残が大きい環境での体験は未検証の開放点。

3. **常設の検算は、見つけたらその場で直す**: 毎起動、`manager/initialization.py` が log.json ↔ DB を突き合わせ (`scan_legacy_log_deficits`)、欠けている部屋を見つけたらその場で取り込み、直せなかったものだけ startup_alerts (UI バナー) に載せる。**見つけられるのに放置してバナーだけ出す形は却下された** (2026-08-16 まはー裁定) — ユーザーは「取り込まれていません」と言われるだけで何もできず、バナーは開発者が気づくためだけの装置になる。取り込みは冪等なので毎起動走らせてよい。直した証拠は自分の帳簿でなく検算をやり直して取る。

   突き合わせは件数でなく**メッセージ ID 単位** — dual-write 期の環境ではファイル末尾の数件が「通常経路の行」として DB に居るため、件数比較は欠けていないのに欠けて見える (本番データで 6 部屋の偽陽性を実測して精密化した)。**取り込みが将来別の理由で漏れても、解消されるまで毎起動アラートが出続ける**。

   関所である以上、**検算自身の沈黙を許さない**: 部屋ひとつの例外で全体を黙らせないよう判定は部屋ごとに隔離し、倒れた部屋は `check_failed` としてバナーに出す。検算全体が倒れた場合もバナーに出す。黙って 0 件を返すと「漏れ無し」と見分けがつかず、この節が防ごうとしている沈黙そのものになるため。log.json の読み取りは読めない理由を戻り値で返し、ファイルが原因で例外を投げない (0 バイト / 読み取り失敗 / 文字コード異常 / JSON 破損 / 構造異常)。

4. **部屋ひとつが「全部入るか、1 つも入らないか」の単位** (2026-08-16 改訂): 1 行でも入らなければその部屋は丸ごと巻き戻す。当初は「1 行の異常で部屋全体を落とさない」設計だったが、部分的に入れると 2 つの壊れ方が出ることが分かった (Codex 指摘)。(a) message_id を持たない行が残ると、次回からその部屋は「行がある」側に回って対象外になり、検算も欠けを 0 と数える — **欠けたまま誰にも気づかれない**。(b) 残りを次回入れると、採番が既存の負 seq より手前に付いてファイル順が崩れる (A,B,C の B だけ失敗 → 再実行で B,A,C)。全件やり直しなら、失敗した部屋は行が 1 つも無い状態で残り、検算が毎起動アラートにする。うるさいが、黙って欠けるよりよい。

   1 部屋の失敗は他の部屋を巻き込まない (部屋単位 SAVEPOINT)。手動 CLI は部屋ごと / ペルソナごとに commit して確定させる — 個別復旧の入口なので、後半で失敗しても進んだ分は残す方が復旧しやすい。⚠️ SQLite では**最外周の SAVEPOINT を RELEASE した時点で確定する**ため、SAVEPOINT だけでは「commit していないから残らない」とは言えない (2026-08-16 実測)。境界は明示的な commit で作る。**この性質のため、取り込みをバージョンアップグレードのハンドラに置くのをやめた** — ハンドラの SAVEPOINT が最外周になると、枠組みの commit を待たずに確定し、後続ハンドラの失敗で巻き戻せなくなる。dev5 の City エッジは no-op として残し (連なりを切らないため)、取り込みは毎起動の検算だけが持つ。

転移テスト (この防止策が他の同型事故も捕まえるか): (a) log.json が本当に壊れている部屋の取り込み不能も同じ検算がアラートにする (旧実装はログ 1 行で沈黙)。(b) 移行前に世界が動いて新規行が付いた部屋 (saiverse_navi 型) も毎起動の取り込みで直る。(c) 検算自体が倒れたケースも `check_failed` として可視化される。いずれも「一括移行が黙って一部を落とし、落ちた結果が『元から無い』と見分けがつかない」という族で、検算はこの族全体を覆う。

#### 残っている入口 (2026-08-16 時点)

- **message_id を持たない行の照合**: 取り込みも検算もこの行を個別に見分けられず、両方が「部屋に行があるときは触らない / 欠けに数えない」で逃げている。過去に一部だけ取り込まれた部屋があると永久に沈黙する。新規発生は「全件やり直し」で塞いだが、既存状態の検出は未対応 ([issue](../issues/legacy_log_no_id_rows_cannot_be_reconciled.md)、既知の実例は無い)。
- **壊れて読めないファイル**: 毎起動バナーが出るだけで、ユーザーに閉じる手段が無い。直せないものを出し続けると全バナーが読み飛ばされ、検算という仕組み自体が死ぬ。設計は「バナーの『移動する』ボタンで壊れたファイルを脇へ退避し、次の起動から出さない」— 確認済みフラグを別に持たず、**ファイルが移っていること自体を『ユーザーが認識した』の記録にする** (派生状態を増やさない)。未実装。
- **ログの文面**: 「取り込み痕跡なしで通常経路の行が N 件」のような、実装を知らないと読めない文が残っている。ログもユーザーが読むもの。

#### 区切り記号を含む部屋 ID (2026-09-11)

- v0.3.0 より前の部屋は、ID にフォルダ名や URL を壊す文字 (`/` `\` `:` `*` `?` `"` `<` `>` `|` `#` `%`、制御文字、末尾のドットと空白) を含むことがある (表示名「2/28」→ `2/28_city_a`)。その部屋の log.json は 2 段のフォルダの奥にあり、取り込みが毎起動空振りしながら警告だけが出続けた。**この検算より前 (部屋を読み込む前) に、それらの文字を `_` に置き換えた ID へ付け替える** (`saiverse/building_id_repair.py`)。付け替えの後は、この検算が新しい 1 段のフォルダから会話を移す。古いファイルでの元のメッセージ ID (`legacy_message_id`) のうち値がある行は、この検算の突き合わせに使うので変えない。付け替えの前後で、この検算と同じ規則で数えた欠けが増える部屋は、付け替えを見送る。
- **欠けの規則は 1 つ** (`count_missing_legacy_messages`)。確認処理 (警告に出す数)・取り込み処理 (入れる行の選び方)・部屋 ID の付け替え (前後の比較) が同じ関数を通る。
- **確認処理と取り込み処理は、同じ関数で部屋のファイルの場所を決める** (`legacy_log_path` → `_child_by_name`、断る基準は `manager.ids.is_safe_path_component`)。付け替えを見送ってフォルダ名として使えない ID が残った部屋は、確認処理でも `check_failed` に出す。「移せていない」と「対象 0 件」が同時に成立する状態を作らない。
- **部屋 ID は常に DB の値を使い、フォルダ名から取らない**。部屋を指定しない手動の取り込みも、DB に登録された部屋 ID から場所を決め、登録されていない部屋のフォルダの log.json は取り込まない (buildings の直下のフォルダにあるものはログへ出す)。
- 詳細: [building_id_contains_path_separator.md](../issues/building_id_contains_path_separator.md)
