...
4. **失敗の閉じ込め単位は部屋ひとつ / ペルソナひとり**: 1 行の異常データで部屋全体を落とさず (行単位 SAVEPOINT)、1 部屋の失敗で他の部屋を巻き込まない (部屋単位 SAVEPOINT)。失敗した部屋は行が 1 つも残らないので、検算が「未取込」として拾う。手動 CLI は部屋ごと / ペルソナごとに commit して確定させる — 個別復旧の入口なので、後半で失敗しても進んだ分は残す方が復旧しやすい。アップグレード経路は commit せず、枠組みのエンティティ単位 commit に相乗りする。⚠️ SQLite では**最外周の SAVEPOINT を RELEASE した時点で確定する**ため、SAVEPOINT だけでは「commit していないから残らない」とは言えない (2026-08-16 実測)。境界は明示的な commit で作る。
