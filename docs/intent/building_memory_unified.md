...
2. **スキップ判定は「現物が読めるか」だけ**: マーカーファイルの有無では判定しない。「取り込み痕跡が既にある」= 冪等 skip、「痕跡なしで通常経路の行がある」= 順序保護のため skip (自動マージしない)。cursor 取り込みは生きた cursor 行がある環境では何もしない (稼働中の値を古いファイルで上書きしない)。取り込みを実行した部屋では、既存の cursor 行を前進させる — 過去ログを「未読の新着」に化けさせない (cursor=0 のまま数百件を取り込むと、次の Pulse が全件を一気に消化しに行く。ファイル側の ingested_by は疎らで印だけでは防げないことを 2026-08-16 に実測)。**前進先はファイル件数ではなく、実際に INSERT できた最大 seq**。件数まで進めると、末尾の行が失敗したときに cursor が DB の実末尾を追い越し、以後の本物の新着 (この部屋の seq は max(seq)+1 で採番される) が既読として食われて永久に見えなくなる。リリース版ユーザーの経路ではこの時点で cursor 行が無く、位置は後続の conscious_log 取り込みが決める。なお conscious_log の remap は「既読位置の尊重」なので、アップグレード前からの未読残があるペルソナはその分を新着として消化する — これは既存設計のままで、未読残が大きい環境での体験は未検証の開放点。

   **「取り込み痕跡」の定義は 1 箇所** (`legacy_log_import.imported_row_filter`): `legacy_seq` か `legacy_message_id` のどちらかが埋まっている行。通常の書き込み経路はこの 2 列を NULL のまま入れるので区別がつく。冪等判定と検算が同じ定義を使う (2 箇所に別々の条件を書くと必ずずれる)。精度の限界: 元ファイルの行が seq も message_id も持たない場合は取り込んでも痕跡が残らず判定から漏れる (全行がその形の部屋が本番データに 3 つある)。漏れたときの影響は「取り込み済みの部屋を『通常経路の行あり』と誤ラベルする」だけで、行が 1 つでもある部屋は必ず skip されるため二重取り込みにはならない。正確にするには取り込み印の列が要るが、実害がラベルだけなので入れていない。

3. **常設の検算**: 毎起動、`manager/initialization.py` が log.json ↔ DB を突き合わせ (`scan_legacy_log_deficits`)、ファイルにあるのに DB に無い履歴を startup_alerts (UI バナー) に載せる。ループ内の帳簿でなく DB を SELECT し直して確かめる。突き合わせは件数でなく**メッセージ ID 単位** — dual-write 期の環境ではファイル末尾の数件が「通常経路の行」として DB に居るため、件数比較は欠けていないのに欠けて見える (本番データで 6 部屋の偽陽性を実測して精密化した)。**取り込みが将来別の理由で漏れても、解消されるまで毎起動アラートが出続ける**——ここが唯一の関所で、アップグレード側は黙って続行してよい。
...
転移テスト (この防止策が他の同型事故も捕まえるか): (a) log.json が本当に壊れている部屋の取り込み不能も同じ検算がアラートにする (旧実装はログ 1 行で沈黙)。(b) 移行前に世界が動いて新規行が付いた部屋 (saiverse_navi 型) も「live_rows_only」として毎起動可視化される。(c) 検算自体が倒れたケースも `check_failed` として可視化される。いずれも「一括移行が黙って一部を落とし、落ちた結果が『元から無い』と見分けがつかない」という族で、検算はこの族全体を覆う。
