...
1. **同梱した定義だけで完全に動く。** カタログは足すか上書きするだけで、無くても機能は落ちない。カタログが消えて止まるのは「新しいモデルが自動で増えること」だけ
   - 2026-09-12 追記 (メティス、まはー未確認): [persona_model_selection.md](persona_model_selection.md) の決まったこと 7 (選んだモデルが SAIVerse に無いとき、代わりのモデルで動かさない) の下では、取得済みの定義が消えると、それを選んだペルソナが止まる。この不変条件と 4 を守るため、段階 3 以降では、カタログが取れなくなっても取得済みの定義を消さない。
