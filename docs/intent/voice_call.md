...
> **ステータス: 検証待ち (2026-09-16 起草・同日実装) — 実機体感の第一巡を反映済み**。まはーの GO は「まず作って体感し、フィードバックで磨く」。次はバックエンドを再起動しての実機体感の続き (音声保存の実機確認を含む)。
...
- 2026-09-16: 起草と同日に backend・frontend を実装し、敵対レビューの指摘の修正まで収束。初回の実機通話で「ペルソナが一言喋った時点で切断」「身体表現 JSON の読み上げ」が見つかり同日修正。「声の指定が効いていない」疑いは A/B 検証 (同じ台詞を二つの声で実 API に喋らせる) で「指定は届いている。カタログ (TTS モデルのサンプル) と Live モデルでは同名の声でも鳴りが違う」と切り分けた。まはーの指示 (記憶用 + 聞き返し) で発話単位の音声保存を追加。
