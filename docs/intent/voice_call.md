...
| 音声の保存 (2026-09-16 まはー指示「記憶用、聞き返しにも使う」) | 通話ごとに `~/.saiverse/personas/<id>/voice_calls/<開始時刻>/` を切り、発話単位で `NNN_user.wav` (16kHz) / `NNN_persona.wav` (24kHz) を保存。連番 NNN は発話の区切り (turn_complete / interrupted) ごとに 1 から振り、同じ区切りのユーザーとペルソナは同じ番号を共有する。文字起こしが取れなかった区切りも番号を消費するので、wav の並びと書き戻した発話の並びはずれない。記憶の行と建物履歴の行の metadata に `voice_audio` = ペルソナフォルダからの相対パスを書く — ただし紐を書くのは wav が実際に書けた発話だけ (存在しないファイルを指す紐を残さない)。文字起こしが取れず記憶の行にならなかった区切りの音声も、ファイルとしては保存する (紐だけ無し)。文字起こしゼロの通話に立てる痕跡の一行にも紐は付けない (特定の発話に対応しないため) | 声は記憶の素材なので、文字にならなかった分も捨てない。発話の区切りは文字起こしと同じ (turn_complete / interrupted)。ユーザー側の wav はその区切りのマイク音声全部で、無音やペルソナの発話中の音も含む (発話だけを切り出す音声区間検出は第一巡では持たない)。音声は通話中メモリに蓄積し終了時に一括保存 (10 分で ~50MB、プロトタイプでは許容)。容量の上限・掃除は未設計 |
