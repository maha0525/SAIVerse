...
SAIVerse の `database/migrate.py` は `Base.metadata` を比較する自動マイグレーション方式（個別マイグレーションファイルは持たない）。`XReplyLog` を `database/models.py` から削除した時点で、次回 `python database/migrate.py` 実行時に `needs_migration()` が True を返し、`x_reply_log` テーブルが自動で DROP される（旧 DB はタイムスタンプ付きバックアップとして残る）。新規マイグレーションスクリプトは不要。

- persona テーブルへの X 関連カラムは元から存在しないため対応不要
- `x_reply_log` の既存レコードは破棄（既存トークンを無効化して再認可させる以上、同一ツイートへの再リプライ発生確率は極めて低く、移行コストに見合わない）
- 旧 `~/.saiverse/personas/<id>/x_credentials.json` と `x_mention_state.json` は触らない（アドオン側からも参照しないため、放置で害なし。クリーンアップしたいユーザーは手動削除）
- 既存ユーザーへの案内: README/CHANGELOG に「X 連携の再設定が必要」を明記（Phase 3）
...
14. DBマイグレーション: `XReplyLog` を models.py から削除済み。ユーザー側で次回 `python database/migrate.py` 実行時に `x_reply_log` テーブルが自動 DROP される（個別マイグレーションスクリプト不要）
