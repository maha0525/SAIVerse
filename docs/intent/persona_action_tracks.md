...
> ⚠ **alert 状態・メタレイヤー判断に関する記述は廃止 (2026-08-14)**: 本ドキュメントの `alert` 遷移 (`set_alert`)・alert observer・MetaLayer の状況分類メタ判断 (v1) は Track 撤廃の順序① ([track_retirement.md](track_retirement.md) §7.4) で退役した。別行動中のユーザー発話の仲裁は on_event 判断点への直結 (`autonomy_wiring.handle_user_utterance_conflict`) が後継。STATUS_ALERT は既存 DB 行の互換のためだけに残る (書き手なし)。
>
