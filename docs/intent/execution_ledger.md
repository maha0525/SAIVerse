...
**ステータス**: 設計中 (v0.2, 2026-07-16) — まはー承認済み: 基盤化 + world DB 配置 + **記憶の順序一貫性 (Pulse 前 flush の必須関所化、v0.2)**。残レビューは §11
...
なお、main lane と META lane の並列（会話とメタ判断の並走）は「未配達分の追い越し」ではなく「現在時刻での同時書き込み」であり、書き込み順で時系列が決まるため本関所の対象外（並列設計自体の是非は Session/Stelis 境界の議論で扱う）。

### 2.3 回復処理 (recovery) — 台帳と現実を照合する掃除役
...
解決済み（v0.2）: ~~回復 tick の住処~~ → 独立の世界レベル定期ジョブ（§2.3。watchdog 相乗りは自律 OFF ペルソナへの配送が死ぬため不採用）。~~配送遅延の時系列~~ → Pulse 前 flush の必須関所化で構造的に消滅（§2.2 / 不変条件 8）。
