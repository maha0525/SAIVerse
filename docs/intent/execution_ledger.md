...
- 配送内容は実行時点で凍結し、配送が遅れても変形しない。**例外は部屋 ID の付け替えだけ** (2026-09-11、[issue](../issues/building_id_contains_path_separator.md)): 未配達の行と、まだ終わっていない実行の入力 (`execution_ledger.PAYLOAD_JSON`) の中にある、付け替えた部屋を指す識別子だけを新しい ID に置き換える。同じ部屋を指す名前の置き換えなので、表す事実は変わらない。
- **配達中に生まれた配達も、その場で片付ける**（2026-09-07）: 配送 handler は配送の途中で新しい行を積むことがある（移動の配送が入室通知と部屋の様子を積む形）。同じ persona 宛ての行は、一覧を配り切った後に問い合わせ直す**配り直し**が同じ flush の次の周回で配る。**別 persona 宛て**の行は、handler がその場で配送を頼んでも同じスレッドが配送の鍵を握っているので配れない — 依頼された persona を到着順の控えに残し、外側の配達が鍵を離した後に控えが空になるまで順に配る（**引き継ぎ**）。引き継ぎの最中に生まれた依頼も同じ控えに載って続けて片付く。上限（50 件）で打ち切った残りだけが回復処理送りになるので、60 秒の回復 tick は本来の役目（誰も来ない間の掃除）に戻る。
  - 直す前は、別 persona 宛ての依頼が再入検知で退場したきり誰にも引き継がれず、回復 tick まで待たされていた（まはーの実機で入室通知の配達が 38 秒遅れた正体 — [issues/perception_arrival_order_differs_from_event_order.md](../issues/perception_arrival_order_differs_from_event_order.md)）。
  - **引き継ぎが走った回は、依頼元をもう一度配ってからその結果を返す**: 引き継ぎで配った handler が、依頼元の persona 宛ての行を新しく積むことがある（別 persona の移動の後処理が依頼元への知覚を積む形）。最初の配達の結果をそのまま返すと、自分が汚した後の古い判定を関所に渡してしまい、pending が残ったまま Pulse が始まる。配り終えた最新の状態を返し、上限で打ち切ったときは安全側（pending 残存＝実行しない）に倒す。返した**後**に別スレッドが積む行は従来どおり対象外で、戻り値は返した瞬間の状態を指す。
...
### 11.2 metabolism.run の completed の退避 (2026-09-08)

§11.1 と同族の罠を completed について決めた。claim が completed 行に当たったら、その行のキーを `{key}#superseded-{id先頭8字}` へ退避し (`supersede_completed` — 状態は completed のまま動かさない。走った事実の記録は残る)、同じキーで新しい claim を取って走る。

- **なぜ必要か**: claim の鍵は「persona:編纂範囲の末尾メッセージ ID」で、「編纂が進めば鍵は自然に前進する」前提に立っていた。だが鍵が前進しない操作の形が二つある — Chronicle 全削除 (成果物だけ消えて台帳の completed 行が残る) と、古いログのインポート直後の補修 (末尾メッセージが変わらない)。この形では completed のブロックが「二重編纂の防止」ではなく「再編纂の永久封鎖」になる (2026-09-08、テリスの全削除→補修が実機で永久に `window_claimed` になった)。
- **なぜ安全か**: completed が守ろうとした「同じ提示コンテキストを二度 LLM にかけない」は、成果物が存在する限り arasuji の source_ids スキップ (確定済みチャンクは LLM を再発火しない) が既に守っている。成果物がユーザー操作で消えた場合の再実行はユーザーの意図そのもの。並行の二重実行は running 行の claim ブロックと Beat ロックが引き続き塞ぐ — 退避が解くのは「過去に完走した」の封鎖だけである。
- **責務分界は §11.1 の照合と同じ**: `supersede_completed` は台帳の汎用メソッドだが、「この kind は退避してよい」の判断 (成果物が外部証跡で観測でき、再実行が冪等であること) は kind の意味論を知る呼び出し元の責務。退避そのものが失敗したらその回は `deferred` で見送る — claim なしの追跡外で走らせない。
- **削除ルート側で直さない理由**: 全削除 API が台帳の鍵を退避する案も検討したが、同族の穴 (インポート直後の補修) は削除を経由しないため塞がらない。claim の側の一点で受ければ「鍵が前進しない形」全部に効く。
