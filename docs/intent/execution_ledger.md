...
**ステータス**: 設計中 (v0.1, 2026-07-16) — まはー承認済みの方向 (基盤化 + world DB 配置)。スキーマ・移行順のレビュー待ち
...
- 回復点は 3 つ: (a) 同一プロセス内の定期 tick、(b) プロセス起動時、(c) 対象 persona の次 Pulse 前。
- 同一 persona 宛の配送は **FIFO** を保証する（時系列の乱れを最小化）。
- 配送先の書き込みは execution_id を冪等キーとして刻み、再配送しても二重にならない。

「**適用済み・記録待ち**」は恥ずかしい事故ではなく、正式な状態として認める。

### 2.3 大原則 — LLM は自動再実行しない
...
| outbox 配送のタイミング・順序 | **基盤**（即時→tick→起動時、persona 内 FIFO） |
...
- **Phase 0 — 基盤**: テーブル 2 つ + migration、ヘルパー（状態機械の強制・冪等 INSERT・outbox 配送器・回復 tick）、回帰テスト。
...
5. **配送遅延の時系列** — outbox の遅延配送分は SAIMemory 上で実行時刻の位置に入るため、本人が既にその後の会話をした後だと、コンテキスト再構築時に「間に挟まって」見える。通常は即時配送されるため稀だが、ゼロにはならない（実行時刻で刻む誠実さを優先する）。
...
2. 回復 tick の住処（AutonomyManager の watchdog に相乗りか、独立 tick か — 完全手動モード（柱 7）の execution gate 設計と揃えて決める）
3. `unknown` の UI 表出（ライフビューに出すか、デバッグパネル止まりか）
4. 保持期間の既定値（30 日仮置き）
