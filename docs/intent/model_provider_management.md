...
**ステータス**: 実装完了（Phase 1-4 完了、2026-05-11）。実機検証はまはー側で実施。§10（アプリ帰属ヘッダー）と §12（プロバイダタブの API キー入力）は実装済み・実機検証待ち。
...
- **経路の網羅**: SDK を通らない経路を作ったら、そこにも明示的に乗せる。NVIDIA NIM の structured output は生 HTTP でヘッダーを手組みしており、放置すると「structured output を頼んだときだけヘッダーが変わる」形になっていた。**同じ経路は body 側でも同じ穴を開けていた** (モデルの `extra_body` が落ちる — [issue](../issues/archive/nim_structured_output_drops_request_kwargs.md))。2026-09-11 に SDK 経路との組み立て共有で根治した: 生 HTTP 経路も `build_request_kwargs` の結果を `llm_clients/openai_runtime.py: split_sdk_request_options` で SDK と同じようにボディと SDK 専用の指定に分け、`extra_body` はボディへ、`extra_headers` はヘッダーへ、`extra_query` は URL へ写す。構造化出力が持ち主のキー (`model` / `messages` / `n` / `tools` / `tool_choice`) だけは設定で上書きできない — SDK 経路では `extra_body` が勝つので、ここだけは意図した違い。`request_kwargs.timeout` はこの経路でも効く (正の数ならその秒数、未設定なら 120 秒。数値でない値や 0 以下は送る前に失敗する)。未設定のときの待ち時間だけは SDK 経路の既定と異なる。LLM 呼び出しではない補助 HTTP (llama.cpp の slot cache 制御) はヘッダーが乗らないうえ認証も付かず、認証付きサーバーで cache が黙って死ぬ ([issue](../issues/llama_cache_control_requests_unauthenticated.md))。
...
- **反映の範囲**: プロバイダやモデルの設定を保存・削除・再読込すると、その場で全ペルソナの話すモデルを決め直し、作ってあった LLM クライアント（通常用と軽量用の両方）を捨てる（`saiverse/model_configs.py` と `saiverse/provider_configs.py` の `reload_configs` から `saiverse/persona_model_selection.py` の `reapply_after_config_reload`）。**次の返事から新しい設定で動く。** 書いている途中の返事は、返事の始まりに決めたモデルと接続で最後まで書く（`ReplyModelBinding`）。削除したモデルを選んでいたペルソナはその場で止まり、追加し直せば再起動しなくても話せる。決め方と反映の全体は [`persona_model_selection.md`](persona_model_selection.md)。2026-09-11 までは、すでに動いているペルソナが作成済みのクライアントを持ち続け、再起動まで前の接続先へ送っていた（[`docs/issues/provider_change_does_not_reach_live_personas.md`](../issues/provider_change_does_not_reach_live_personas.md)）。
...
### 12. プロバイダタブの API キー入力 (2026-09-27)

**ステータス**: 実装済み・実機検証待ち。

**問題**: API キーの値を親切な形で入力できるのは初回チュートリアルの API キー画面だけで、そこは `api/routes/tutorial.py` の手書きリスト (7 サービス) に固定されている。後から増えたプロバイダ (例: TypeSafe) のキーは、環境設定の生の環境変数リストまで自力でたどり着かないと入れられなかった。プロバイダの編集画面にあるのは「どの環境変数からキーを読むか」という変数名の欄だけで、値の入り口が無い。

**設計**: プロバイダ一覧の各行に「キー設定」ボタンを置き、行の下にインラインの入力パネルを開く。

- 出す条件は「`api_key_env` を持ち、protocol が `openai_codex` でない」。プロバイダ定義から機械的に導くので、**今後プロバイダが増えても手書きリストの追従が要らない** (これがチュートリアル側との本質的な違い)。Codex はキーでなくログインで認証するので既存のログインボタンのまま。
- 保存は環境設定画面と同じ口 (`POST /api/admin/env` → `write_env_updates`) に流す。`.env` への書き込み・`os.environ` の即時反映・全ペルソナの LLM クライアント破棄まで既存処理がやるので、**再起動なしで次の返答から効く**。
- キーの値は取得も表示もしない。設定済みかどうかは既存の KEY OK / キー未設定バッジと placeholder (「設定済み - 変更する場合のみ入力」) で示す。
- 取得方法ガイド (`docs/api-keys/*.md`) があるプロバイダにはリンクを出す。対応表は環境変数名をキーにするので、同じ変数を共有するプロバイダ (OpenRouter 系 2 種) にも自動で効く。TypeSafe のガイドは本件で新設した。
- 実装はフロントエンドのみ (`ProviderManagementPanel.tsx`)。バックエンドは既存 API で足りた。

**検証済み (2026-09-27、隔離テスト環境)**: ボタンの出し分け (キー不要・Codex に出ない) / ダミー変数での保存 → `.env` へ追記 1 行のみ・バッジ即時更新・知らせ表示 / ライト・ダーク両テーマの表示 / 取得方法リンクの出し分け。実機 (本番環境) での確認はまはー待ち。
