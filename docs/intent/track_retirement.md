...
| 2 | **関係の器（永続 Track）** | 対ユーザー会話 Track と交流 Track（is_persistent）。「エアと話す」の常設枠 | 三分割: 意図＝「話す・聞く」型のレパートリー項目 / 知識＝人物ページの経験の台帳（§5-A）/ **記録＝対話エピソード**（episode.md v1.2 §3.3 — 相手ごとに 1 行・完了しない。永続会話 Track が持っていた「会話記録の常設の器」という役割の、実行の状態機械を伴わない再配置）。会話の実行面（待ち・「いま会話中か」）は待ちタイマーへ移管済み |
