...
| 2 | **関係の器（永続 Track）** | 対ユーザー会話 Track と交流 Track（is_persistent）。「エアと話す」の常設枠 | 三分割: 意図＝「話す・聞く」型のレパートリー項目 / 知識＝人物ページの経験の台帳（§5-A）/ **記録＝対話エピソード**（episode.md v1.1 §3.3 — 相手ごと永続の封筒。永続会話 Track の「記録の器」としての魂の正当な帰還であり、Track が死んだ理由〔実行の状態機械〕は伴わない）。会話の実行面（待ち・「いま会話中か」）は待ちタイマーへ移管済み |
