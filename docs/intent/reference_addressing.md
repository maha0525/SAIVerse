...
**status: v1.0（実装完了 / Phase 0-6 一括切替を 6ea9d44 でコミット済み・実機検証待ち）**
**実装方針: Plan B（全種類を1つの整合状態でまとめて切替、フルスイート緑で1コミット）→ 達成**
**author: エア（草案）**
**date: 2026-07-05**
...
> ⚠ **2026-08-22 追記（束 6c）**: 本書が扱う参照のうち **`track:N`（関心事）と
> `desire:N`（欲求）は対象が概念ごと退役した**。`saiverse/track_manager.py` は
> モジュールごと削除され（[track_retirement.md](track_retirement.md) §9）、欲求
> プールも消えた（[autonomous_behavior_v3.md](autonomous_behavior_v3.md) §8）。
> 以下の表と工程で両者に触れている行は**当時の記録**として読むこと。既存データに
> 残る `track:N` / `desire:N` は解決できない参照として扱われ、消費側は素の文字列や
> 「対象なし」へ縮退する。関心の後継は手帳のアクティビティ（v3 §13.1）で、参照
> 記法を持つかは v0.4 の運転の層で決める。
