...
**ステータス**: 実装中 (v0.2, 2026-07-17) — §6-1 ExecutionContext 導入 (挙動不変) 実装済み (77e81e6)。**§6-2 Beatロック+関所+main/META解体も実装済み (2026-07-17)**: `sea/beat_gate.py` (persona 単位 RLock + 最外周のみ関所 flush + Beat 境界の解放/再取得) + 取得点4系統 (run_meta_user / work_session / metabolism / gold_panning セッションクローズ) + spell ループの周境界結線 + PulseController 並列レーン意味論の解体。スレッドモデルの実測帰結は §3.4 末尾を参照。**§6-3a anchor/TTL/watchdog の行分離 + 実 model 記帳も実装・検収済 (2026-07-17)**: 新テーブル `session_anchor` (PK=(PERSONA_ID, MODEL_KEY)) + 冪等 backfill + 旧 `AI.METABOLISM_ANCHORS` 読み書き経路の撤去 + `touch_anchor_after_llm_call` の記帳先を usage.model に (S1/S8 根治) + TTL watchdog 予約 key の `ttl:{persona}:{model}` 化 + ライフ終端 cancel の prefix 一括化 (`EventScheduler.cancel_prefix`)。**§6-3b head snapshot + last_notified の (persona, model) キー化も実装・検収済 (2026-07-17)**: 新テーブル `session_head_snapshot` (PK=(PERSONA_ID, MODEL_KEY)) + 冪等 backfill (衝突は line='main' 優先→UPDATED_AT 最新) + in-memory キー・store 全経路の切替 + model_key を prepare_context 経由で head render まで運ぶ結線 (LLM node / work_session / gold_panning / keepalive)。**実装時の発見: 旧 line_head_snapshot の MODEL_KEY は全行 'default' に落ちていた実バグ** (integration.py が存在しない `persona.default_model` を引いていた) — フォールバックの正は `persona.model`、backfill は 'default' を ai.DEFAULT_MODEL へ解決して移行する。副産物として anchor-TTL 失効時の snapshot 再 capture が実ペルソナで初めて機能する。**§6-4 head 操作の内容型通知も実装・検収済 (2026-07-17)**: `sea/head_pipeline/notify.py` (通知本文 = `section.capture→render` の text そのもの = 「寸分たがわず」の構造保証、outbox 配送 kind="head.mutation_notify"、push 確定後に B を全 Session 行前進) + 4 section のツール成功点結線 (memory_write/delete/clip → core_memory・memopedia_index、memory_open/close → desk、life_purpose_set → life_purpose — META finalize 自動カバー、issue head_mutation_notification_gap 解消) + **S5/S3 修正**: 既存 backstop diff 通知 (world_state) を outbox 経由化し、B 前進を outbox durable 確定後に移設 (flush_diffs の advance=False 分離、配送失敗時は dirty 据え置きで次回再検出) + life_purpose diff ラベルへ render 断片同梱。**§6-5 Metabolism 二層分離も実装・検収済 (2026-07-17)**: 編纂の冪等 claim (実行台帳 kind="metabolism.run"/"metabolism.run_track"、key=persona:提示コンテキスト末尾ID、全入口が合流する generate_chronicle 内で claim — M1 根治) + S2 ガード (generate_chronicle が status ok/disabled/failed/deferred を返し、anchor 退役は ok/disabled のみ。**非対話の確認スキップは disabled** = 自律 Pulse のみの persona を永久据え置きにしない設計判断。failed/deferred は watermark 超過が残るので次回自然再試行) + `history_manager.metabolism_anchor_message_id` 全廃 (prefix anchor は `state["_prefix_anchor_id"]` で call-local に運ぶ — 記憶監査第4片の根治。work_session の WORKER 実行中 main-line 誤 touch も同時に消滅) + 閾値/退役/発火判定の model 化 (maybe_run_metabolism は Pulse の実行 model の提示コンテキストだけ動かす) + 可視化 = model の節目 (on_metabolism は anchor を進めた model の snapshot だけ再 capture) + chronicle_index の件数ラベル diff 退役 (可視化は節目の構造交換が担保)。**§6-6a thread の push/pop 化も実装・検収済 (2026-07-17)**: PulseContext に thread スタック (`push_thread`/`pop_thread`/`unwind_threads`) + Stelis / subagent の切替を全経路これ経由に + **graph 実行の finally で入口深さへ巻き戻し** (例外/cancel で end ノード不達でも「Beat が終われば必ず親 thread へ戻る」= 不変条件6 を保証、S4 根治) + `active_state.json` の `pulse_scoped_parent` マーカーでクラッシュ孤児を起動時復旧 (復旧はペルソナ登録経路の opt-in — 使い捨て adapter が走行中 Stelis を誤って巻き戻さない門) + `PulseContext.thread_id` (死に値) 廃止。裁定: `active_state.json` は「直列に一人ずつ触る台帳」として存置 (Beat 直列化下では保証された復元で実行属性の意味論が成立、読み口の contextvar 化は追わない)。恒久切替 (thread_switch) はマーカーを自然に消す = クラッシュ復旧が恒久選択を打ち消さない。**§6-6b (Beat ロックの実行トークン化) は分離して後続へ** — 現状の劣化は軽微 (レガシー分岐で META の待ち最大1 Pulse、直列性は無傷) に対し、トークン伝播漏れ=デッドロック新設のリスクを持つため。材料は handoff §6 に記録済み。**§6-7 正典改訂も完了 (2026-07-17)**: session.md の「line で head を分ける」残滓2箇所 / dynamic_state_sync.md の操作ラベル型・差分駆動前提 (改訂注記+3箇所) / concepts/metabolism.md の anchor 記述。統合工事の残 = §6-6b のみ。まはーレビュー 1 巡で全論点解決済み（知覚バッファ経由・常時通知・割り込みは Beat 境界待ち・Metabolism 閾値はモデル依存・未整理 section も包含）
...
1. **柱 2: Session の (persona, model) 正典に実装が追いついていない**。head snapshot の物理キーに model が無く、anchor/TTL/Metabolism 提示コンテキストは `persona.model` 固定で、lightweight 呼び出しが standard の cache 寿命を偽延命する（S1）。まはーの診断どおり「設計はフルキー、実装が途中で止まっていた」。
...
4. **head 操作の通知の抜け**: 「本人の操作は通知不要」という単一提示コンテキスト前提が、line 隔離・model 分離で破れている（issue 参照。META の生きる目的書き換えは現行で内容不達）。
...
- **編纂（Chronicle 生成）**: persona 共有の土地の仕事。冪等キー `(metabolism.run, persona:提示コンテキスト)` で実行台帳に claim し、全入口（各 model の Metabolism・手動整理・API）が同じ排他を通る（M1 の解）。※ session close の入口は 2026-08-24 に撤去された（[gold_panning.md](gold_panning.md) §3.6）。
- **退役（anchor 前進）**: model ごと。自 model の提示コンテキストが節目を迎えた時だけ、**退役範囲の編纂が済んでいることを確認してから** anchor を進める（S2 の解 — 編纂失敗時は据え置いて次回再試行）。
- **可視化は model の節目ごと**: 各 model のコンテキストに入る Chronicle 集合は、その model の anchor 更新時に確定する。他 model の編纂で新しい Chronicle ができても、自分の節目までは prefix に入れない（prefix cache 保護）。提示コンテキストが生ログでカバーしている間は情報欠落はなく、退役の瞬間に退役分の Chronicle が入れ替わりに見える。
- **Metabolism の閾値（watermark / token threshold）はモデル依存**（まはー確認 2026-07-16）: standard/lightweight という区別ではなく、実行 model のモデル設定（context_length 等）から導出する。各 Session は自分の model の閾値で自分の提示コンテキストを管理する。→ **2026-07-25 上書き** ([chronicle_eviction.md](chronicle_eviction.md) §4): 既定値は**全モデル一律・文字数の三水位** (低4万/目標6万/高12万) に改訂 — モデルごとにバラバラな数値・単位が切り分けの混乱の元だったため。置き場所は従来どおりモデルファイル (ユーザー設定可) で、「各 Session が自分の閾値で自分の提示コンテキストを管理する」構造は不変。高水位未設定なら `token_triggered` のみで発火。→ **2026-09-04 再改訂**: 低水位を廃止し二水位 (残す量4万・上限12万) へ ([chronicle_eviction.md](chronicle_eviction.md) §4)。
...
> **head の元データへの操作は、常に、その persona の全 Session 提示コンテキストへ「head に入るときと同一の render 断片」を内容型通知として配送する。**

- **内容忠実性の構造的保証**: 通知本文は section の render と同一の関数から生成する。「head で見える内容」と「tail 通知で届く内容」の文面を別々に書かない（書き分けた瞬間からドリフトが始まる）。
- **通知要否の判定は持たない — 常に通知する**（まはー裁定 2026-07-16）: 「読み手の提示コンテキストに操作の生ログが残るか」という判定は作らない。main 以外の line での操作は、操作した本人にも結局通知が届く — そのペルソナ体験を判定ロジックで完璧に対策しきれない以上、無理に効率化しない。安全と単純を優先する。main line で撃った直後は spell 結果の生ログと通知が二重に見えるが、これも引き受ける。
...
- **実装帰結（§6-4 設計 2026-07-17）**: (a) 操作起点 push の対象は tool 書き込み経路がある 4 section（core_memory / desk / life_purpose / memopedia_index）。**memory_weave / chronicle_index は書き手がシステム編纂のみ**（tool 書き込み経路が存在しない）ため、その内容型可視化と diff 整理は §3.2 の「可視化は model の節目」と一体で **§6-5 で扱う**。(b) 通知本文の構造保証は「操作後に `section.capture(ctx)` → `section.render(snapshot)` を呼んだ text」— render は snapshot のみに依存（DB 非参照）するため、head 本体の凍結を破らずに寸分たがわぬ断片が得られる。(c) 二重通知の防止は diff の削除でなく **push 確定後に last_notified (B) を該当 section だけ全 Session 行で前進** — 知覚バッファ→SAIMemory は persona 共有の履歴ストリームで push は全 Session の提示コンテキストに届くため。tool 外の変化（UI 編集・migration）は backstop diff が引き続き拾う。
...
4. **head 操作は常に内容ごと届く** — 操作ラベルではなく render 同一断片を、判定なしで全 Session 提示コンテキストへ。「head に入ったときに見える内容と寸分たがわず同じ内容」（まはー 2026-07-16）。経路は outbox → 知覚バッファ → tail。
...
| どの model の提示コンテキストをいつ縮めるか | **基盤**（各 model の閾値。ペルソナの意志は関与しない） |
...
2. **通知の重複ノイズ** — 常時通知のため、操作した本人の提示コンテキストにも通知が届く（main line で撃った直後は spell 結果の生ログと二重に見える）。判定ロジックの複雑さより、欠落の無さと単純さを取った（まはー裁定）。
