...
**ステータス**: 確定 (§I のみ 2026-08-10 設計確定・実装待ち)
...
### I. ツール一覧の所有と鮮度 — 起動時 discovery の廃止 (2026-08-10 設計確定・実装待ち)
...
- spell_list Section capture の参照先の付け替え方 (グローバル登録簿前提の `is_tool_available_for_persona` → ペルソナの live 一覧照会)
- `visible: false` ツールと `addon_spell_help` の列挙元との整合
- プロンプトキャッシュとの関係: 一覧が「変わった時だけ変わる」形を守る (毎コールの組み直しで head を揺らさない)
- per_persona 接続の Pulse 終端での扱い (切断か短期 keep-alive か) と参照カウントの整合
- Beat 頭の検知を spell_list に限定するか (全 Section の capture を毎 Beat 走らせると DB 負荷が Beat 頻度で掛かる)
