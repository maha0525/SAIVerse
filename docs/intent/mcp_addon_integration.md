...
- **ready の合図は「1 つでも生きているか」**: `_fire_server_ready` は全 instance の成功ではなく生存 1 件で立てる。複数 instance (ペルソナごと / vessel ごと) の 1 つが失敗しただけで、生きている instance の購読者を待たせる理由がない。戻り値の `success` は従来どおり「全 instance が繋ぎ直せたか」。
