...
- **spell_list の鮮度検査の除外**: 再起動直後は per_persona ツールが登録簿に無いが、これは「まだ今セッションで誰も取得していない」だけで消えたのではない。head snapshot の鮮度検査 (`_check_spell_freshness`) が per_persona 名の欠落だけを理由に失効を出すと、Pulse 前の head 構築 (プレビュー等) が再キャプチャ → 通知既読 (B) のリセットを起こし、初回取得時に全ツールぶんの「使えるようになりました」が知覚へ流れ込む。per_persona サーバーの名前空間に属する名前の欠落は失効にしない。**同じ除外は差分の計算 (`diff_to_notifications`) にも効かせる** (2026-09-07 追記): 剥奪の候補のうち「per_persona の名前空間に属し、かつ今セッションで未取得 (= 登録簿に無い)」名前は剥奪と数えない。判定は両者で一つの関数 (`_is_unfetched_per_persona_spell`) を共有する。鮮度検査だけに置いていた間は、Pulse 前のコンテキストプレビューが未取得のツールを「使えなくなりました」と偽表示していた (まはーの実機検証、`docs/issues/perception_state_pushed_at_event_time.md`)。取得が成功した回は名前が登録簿にあるので除外は不発で本物の剥奪はそのまま出るし、取得が失敗した回は上の「fetch 失敗の扱い」が直前の一覧を維持して剥奪を発火させない。
...
- **Pulse root は 2 本ある**: 会話・schedule・auto・メタ判断の Pulse (`run_meta_user`。`pulse_controller` / `manager/runtime` / `day_scenario` の全入口がここを通る) と作業・暮らしセッション (`sea/work_session.py`。コマ発火 = `day_plan` から呼ばれる)。取得を前者にだけ置いていたため、スペルを一度も撃たない作業セッションは接続ゼロのまま head を組んでいた。頭の一手は `sea/mcp_tool_refresh.refresh_mcp_tools_at_head` に切り出し、両 root と Beat 境界の 3 箇所がこれを呼ぶ (Pulse root を増やすときはこの 1 行を頭に置く)。**`tell` スペル (`builtin_data/tools/tell.py`) は自分の PulseContext を作るが root ではない** — スペルは定義上つねに親 Beat の内側で唱えられる子ラインなので、Beat ロックも知覚消費も取得も親が済ませている (同ファイルが既に同じ理由で消費を持たない)。
...
#### 2 巡目の消し込みで確定した点 (2026-08-10、Codex 再レビュー後)

- **畳む理由によって忘れる範囲が違う** (これが設計の芯)。`_shutdown_instance` は常に「接続が無くなったら真になること」= 登録簿からツールを外す / 所属を無効化する、だけをやり、**何を忘れるかは引数 `recoverable` が運ぶ**:
  - **恒久的な撤去** (手動停止・アドオン無効化・refcount 0) = 参照も落とす。
  - **一時的な失敗** (設定の一時的な欠落・再接続の失敗) = 参照を残す。「まだ在るべきもの」なので、落とすと refcount 0 の不変条件を壊したまま復旧させることになり、再接続の対象からも消える。
  - **`${instance.*}` の context はどちらでも忘れない**。忘れるのは「もう使わない」と決めた `stop_instance` (vessel の解除など) の責務で、そこが明示的に捨てている。ここで一緒に捨てると、設定が戻っても名前付き instance が再登録なしには復旧できない。
- **再接続は「繋ぎ直す」だけでなく「立て直す」**: 対象を `_connections` だけから採ると、一度落ちた instance は再接続ボタンから見えなくなり、自己修復の頭を持たない global / 名前付き instance はプロセス再起動まで戻れない (**私の畳み方が作った裏返し。Codex の指摘ではなく、指摘を直した後に自分で歩いて見つけた**)。参照が残っているか失敗記録があるものも対象に含め、接続オブジェクトが無い instance は `_start_instance` で立て直す (設定の再解決と未解決検査は `connect()` の関所が済ませる)。
- **再接続の失敗も instance を畳む**: 掴んだままにすると、ツール wrapper の遅延起動が「`_connections` にキーがあるか」しか見ないため切れた接続へ送り続け、しかも失敗記録が無いので UI にも backoff にも出ない。畳んで失敗を記録すれば、per_persona は次の Pulse 頭が、名前付き instance は保持された context で遅延起動 / retry がやり直す。再接続を `_start_instance` の共通経路へ寄せる構造的な統合は issue 側 (1) に残す。
- **ready の合図は「1 つでも生きているか」**: `_fire_server_ready` は全 instance の成功ではなく生存 1 件で立てる。複数 instance (ペルソナごと / vessel ごと) の 1 つが失敗しただけで、生きている instance の購読者を待たせる理由がない。戻り値は従来どおり「全 instance が繋ぎ直せたか」を表す。
- **「繋ぐ相手がいない」は失敗ではない (2026-08-25)**: 再接続の結果は `reconnected` / `failed` / `no_instances` の 3 つ (`RECONNECT_*`)。per_persona はペルソナが動き出すまで接続を持たないので、**接続が無いのが常態**であり、それを失敗と同じ値に潰すと常態が異常の顔で表に出る。実機ではこれが三段重ねになって「再接続ボタンが無反応」に見えた — ① 対象なしと失敗が同じ `False`、② API がその `False` を HTTP 200 に包む、③ 画面が HTTP ステータスしか見ずに本文の `success` を読まない。**それぞれ独立した欠陥で、どれか一つを直しても症状は消えない**。加えて画面側は操作の直後に一覧を取り直しており、その取り直しがメッセージを無条件に消していた (4 つめ)。「失敗」と「空振り」は `error` と `message` に分けて返し、画面も赤と青で分ける — 正常な状態をエラー色で見せるのは、ログで正常を警告に混ぜるのと同じ誤りである。
- **版の検査は登録より先**: 登録を先にやると、一覧は捨てても `TOOL_REGISTRY` / `SPELL_TOOL_SCHEMAS` へ足した wrapper が残り、未評価の別ペルソナが config 近似でそれを見る。無効化を跨いだ取得は所属も登録も触らない。
- **手動停止の説明は実装に合わせた**: `manual_stop_instance` と `/api/mcp/instances/stop` の説明が「次の tool call まで停止」のままだったのを、「per_persona は次の Pulse 頭で復活する / 止めたければアドオン無効化」に直した (画面と docstring に書いた文は契約なので、実装が変わったら同じコミットで直す)。

#### 3 巡目の消し込みで確定した点 (2026-08-10) — 遷移中の instance を見えるようにする

**`_connections` は「今繋がっているもの」しか表せない。** 接続の確立と切断には await があり、MCP ループが単一スレッドでもその隙に別経路が同じ instance を触る。**Pulse ごとに接続を張る §I の設計は、この窓を大幅に広げた** (従来 per_persona の起動はツール呼び出し時だけだった)。窓が開いている間、遷移中の instance はどの経路からも見えず、二重起動と孤児 subprocess を生む。

そこで遷移そのものを状態として持つ:

- `_starting` … 起動処理が走っている (まだ `_connections` に入っていない)
- `_stopping` … 停止処理が走っている (もう `_connections` から出ている)
- `_stop_requested` … 起動中に停止を頼まれた。**起動は途中でキャンセルせず、着地した瞬間に起動側が自分で畳む**

規則:

1. **起動は遷移中を拒む** (`MCPInstanceBusyError`)。既に繋がっているなら何もしない (別経路が先に張り終えていた場合)。待たずに断るのは、待つと停止側が数十秒ブロックされるため — per_persona なら断られた回の実害は「その Pulse はツール無し」だけで、次の Pulse 頭が張り直す。この状態は `busy` として分類するが、**故障ではないので失敗記録には載せない** (backoff を焚くと「混み合っただけ」で使えなくなる)。
2. **停止は起動中にも届く**。`_connections` を列挙する掃除 (全停止・アドオン無効化) は `_starting` も併せて見る。見落とすと、無効化・プロセス終了のあとに subprocess とツール wrapper が残る。
3. **停止要求はその起動に向いたもの**。接続が失敗した回に旗を残すと、次に成功した起動が着地した瞬間に自壊する — 旗は成否にかかわらず降ろす。
4. **復活の条件は「まだ望まれているか」= 参照があるか**。失敗記録を根拠に加えると「誰も要求していない instance を復活させる」道が開き、参照が無いので refcount 0 の live instance ができる。

⚠️ **ここで作っていないもの** (`docs/issues/mcp_remote_connection_recovery_gaps.md` へ): 起動タスクの明示的なキャンセル、望ましい状態 (desired state) の tombstone / 世代管理、`reconnect_server` の生存接続経路を `_start_instance` へ寄せる統合。今回入れたのは「遷移中を見えるようにして、重なりを断る」だけで、状態機械としては未完成。
