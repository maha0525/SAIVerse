...
**ステータス**: 実装中（2026-07-12 コードレビューで編纂 P4-a に P1×3 指摘 — fold の refs 契約不一致・split の本文保存則違反・merge/split 非原子性。修正後に実機検証へ。詳細は `docs/handoff/2026-07-12_concept_consolidation_code_review.md`）。これは⑥「概念要素の整理」の umbrella（親）。個別の再編は各々の doc / この文書のメモから育てる。
...
2. **裁定**（就寝判断相乗り）: desire_reviews と同じ3点セットで `curation_reviews` を追加。**ペルソナが何を見るか**（状況テキスト、候補1件＝1行＋根拠）:
   ...
   **編纂の本文保存則（まはー裁定 2026-07-11・原則）**: **編纂は本文を生成しない——移動と結合のみ**。LLM に統合文や節の本文を「生成」させると必ず漏れが出る。既存の記述を新たな自然文で置き換える操作は絶対に許さない:
   - **統合＝完全決定論**（LLM ゼロ）: 残る側本文＋区切り（`## 統合: 旧「<タイトル>」より`）＋消える側本文の逐語連結。キーワード＝和集合、タイトル＝残る側、summary＝残る側（消える側の summary は区切り見出し直下に逐語で残す）
   - **分割＝LLM はラベル付けのみ**: 既存段落ブロックの「どの子に割り当てるか」だけを LLM が返し（本文は出力させない）、移動はコードが逐語で行う。**保存則の機械検証**: 子ページ全部＋親の残り＝元の本文（文字レベル一致）でなければその分割を棄却（fail-safe、WARN）
   - この原則により、編纂の失敗の最悪ケースは「不格好な配置」であり、全て可逆
