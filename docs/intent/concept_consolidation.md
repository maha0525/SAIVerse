...
**P4-d 追補: 既定を OFF から ON へ**（2026-09-01 まはー裁定）。P4-d 当時の「opt-in・既定 OFF」（上記 2 段落）は、索引の常時表示を自動想起（記憶アーキv2 ゾーンC）が置き換える前提に立っていた。その前提がまだ満たされていない——自動想起は索引の常時表示を置き換えられる完成度に達していない——ため、**`MEMOPEDIA_INDEX_ENABLED` の既定を ON へ変更**する。実装: `database/models.py` のカラムデフォルトを `True` に変更（新規ペルソナ）＋ v0.3.1 のアップグレードハンドラ `v0_3_1_memopedia_index_default_on`（`saiverse/upgrade_handlers.py`）で既存ペルソナも一括 ON（SPELL_ENABLED の dev3 と同じ構図——カラムデフォルトは既存行に効かず、スキーマ差分も出ないため）。「明示的に OFF を選んだ」と「触っていないだけの OFF」は DB 上で区別できないので一律 ON に揃え、OFF に戻したいユーザーは設定 UI から個別に戻す。トークン消費が増える点は変わらないので、自動想起が育った時点で既定を再検討する。
