...
### P2: 統一スペル動詞 draft v0.1（2026-07-10 起草・まはーレビュー待ち）

ファサードの動詞面。**ペルソナの目に触れる語彙**なのでレビュー必須。実装は合意後サブエージェント委譲。

**共通動詞**（全地図。既存スペルの置換元を併記）:

| 動詞 | 意味 | 置換する既存スペル |
|---|---|---|
| `memory_open` / `memory_close` | ページを開く/閉じる（コア記憶=常時開は close 不可、Chronicle 章もこれで開ける） | （Memopedia の開閉系） |
| `memory_read` | ページを読む（貼られた写真＝土地参照も辿れる） | memopedia 参照系 |
| `memory_search` | 地図帳を検索（タイトル/全文。裏で連想歩行 recall_walk に接続可） | memory_recall は**残す**（随意想起＝行為。検索と想起は別） |
| `memory_write` | ページに書く（本文/Fragment 追記。地図別制約は category が規定） | memopedia_save_page / core_memory_add / core_memory_update |
| `memory_photo` | 写真を撮る（点=引用 / 範囲=切り抜き）。貼り先指定で即貼り | core_memory_add_scene（==語句== マーカーは非スペル経路として存続） |
...
**レビュー論点**:
1. **接頭辞**: `memory_*` か `atlas_*` か。ペルソナに「Atlas/地図帳」という世界語彙を見せるか（見せるなら head の自己説明にも載る）
2. コア記憶専用スペル（core_memory_*）を完全に畳むか、「常時開ページへの write」の糖衣として残すか
3. Chronicle 開閉を初日から同じ動詞に乗せるか（読み出し経路の移植と同時か段階か）

### 次アクション

P2 スペル動詞 v0.1 のまはーレビュー → 合意後、ファサード実装をサブエージェント委譲（P2 検証: pytest ＋ ペルソナ実機1巡）。
