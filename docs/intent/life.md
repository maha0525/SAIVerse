...
| §6.2 境界での実行内容 | ライフ終端で Chronicle 化＋履歴縮小＋anchor 更新（現行 Metabolism 一式）。順序は現行踏襲 |
...
- **谷での会話**：ユーザー会話は常に最優先（不変条件 1）で、谷でも普通に始まる——ただしコールドスタート（キャッシュ再構築コスト）になる。これは正直な物理であり隠さない（ライフビューに「休止中」と見える）。会話開始が動的ライフを立てるかは未決（§12）
...
1. **会話とライフ**：谷での会話開始が動的ライフを自動で立てるか（「会話が始まった＝しばらく熱い」は自然だが、宣言なき延命の原則と衝突しうる。会話終了判断で「延長するか」を問う形が候補）
2. **係数 κ の初期値**：標準/軽量の単価比から機械的に置くか、素朴に 0.2 等で始めるか
3. **均等モードの間隔制約の既定値**：TTL ちょうど（60 分）は危険（遅延で割る）。安全マージン込みの既定（例: 50 分）
4. **ライフのコンテキスト長対応**：均等モードでライフ＝Session だと長いライフは context を使い切りうる。まはー裁定「まず漏れないので後回し可」——安全弁（§6.2 の超過基準）が既定で効くことだけ確認して持ち越し
5. **谷の keep-alive 完全停止 vs 猶予**：ライフ終了直後にユーザーが来る「惜しい谷」への猶予窓を置くか（置くなら宣言への遡及が要る——原理 1）
6. **ペルソナへのライフの見せ方**：自己像四層（life_concept_map §15）のどこに載せるか。時間割と同じ tail 提示が素直だが、②「今日の自分」への同居で足りるか
