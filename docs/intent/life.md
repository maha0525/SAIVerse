...
> Track の状態そのものが無い。**② 会話の出来事（Episode）**（[autonomous_behavior_v3.md](autonomous_behavior_v3.md) §7）—
> 「いま会話中か」の正典はプロセス内のメモリ状態へ移り、会話終了判断
> （`post_conversation`）も 2026-08-16 に退役した。
> **案 Y の芯（「いま」の真実は Track の状態ではなく別の器が持つ）は生きている**が、
> その器は三代目になっている（Track の status → 開いている出来事 → メモリ内状態）。
> §7.3 の経路表・§10.1・§11.2 で Track と post_conversation に触れる行は、当時の
> 記録として読むこと。ライフ（活動区間）の設計そのものは v0.4 のティック設計
> （v3 §5）が引き取る。
...
1. **コマの標準パルス**：暮らしコマは presence 記録だけのスタブをやめ、**コマ開始時に標準モデルの短い暮らしの一手（暮らし Pulse）を実際に 1 回撃つ**（v2 intent §4.1 (b) の前倒し実装）。これでコマ＝AI 呼び出し 1 回＝予算 1 消費＝キャッシュ延命 1 回が一直線に揃う。v1 の充填独白と分けるガード: 低頻度（コマとして選ばれた時だけ）・世界の材料つき（部屋の様子・机メモ・直近のできごと）・成果ゼロ許容。作業コマはセッション終了時の post_session 判断（標準）が同じ役を果たす
   ...
   > ⚠️ **実装とのズレ（2026-08-09 に発見、まはー裁定待ち）**: 暮らしコマは 2026-08-08 に実体化されたが、[器の統合](autonomous_pulse_vehicle.md) で作業セッションと同じ **WORKER アスペクト＝軽量モデル**で走る。したがって **暮らしコマは標準モデルの Session（＝会話が使うキャッシュ）を温めない**（唯一の例外は、軽量クライアントの生成に失敗して `select_llm_client` が標準クライアントへフォールバックしたとき。障害時の副作用であって設計された供給ではない）。上の「コマ＝キャッシュ延命 1 回」は成立しておらず、均等モードで標準キャッシュを繋ぐ実体は現状 keep-alive touch（下の 3）だけになっている。最低予算の式 `ceil(窓長 / 50 分)` も「標準パルスがその間隔で撃たれる」前提で立てられていたので前提が変わっている。取りうる方向は (a) 暮らしコマを標準アスペクトへ戻す（器の統合の利点と衝突）、(b) キャッシュ延命は keep-alive に任せ、この節の「意味のあるパルスが本体」という順位を書き換える、(c) 暮らしコマの中で標準を 1 回だけ触る仕掛けを足す — いずれも設計判断なのでコードは現状のまま置いている。
...
### 5.3 予算が数えるもの — 自発活動の回数（2026-08-08 追補で単位を改訂）

予算のカウントは**ペルソナが自分から動いた 1 回**を単位にする。ライフビューの数字（used / budget）が利用者に対して意味するのは「その日の自発活動の回数」。コマの発火そのもの（開始時刻が来ただけで AI を呼ばないもの——presence 記録・施設への移動・keep-alive）は何回起きても予算を減らさない（原理 4）。

> **追補（2026-08-08、[autonomous_pulse_vehicle.md](autonomous_pulse_vehicle.md) §A）**: v0.5 はこの単位を「標準（DEFAULT_MODEL）の LLM が実際に呼ばれた瞬間」と書いていた。器の統合で暮らしコマが WORKER アスペクト（＝軽量モデル）のセッションとして走るようになったため、その文字どおりの読みでは暮らしコマが 1 回も数えられなくなる。§A の裁定は**計上単位をコマのまま維持**——利用者が数えているのは呼ばれたモデルの階層ではなく活動の回数だから。「標準モデルの呼び出し」は単位ではなくコストの目安として残る。
> **未裁定**: tell スペル（`builtin_data/tools/tell.py`）が起動する標準モデル 1 呼び出しを予算に数えるか。現状は数えていない——tell は会話の声であってコマの活動ではない、というのが実装時のメティス見解で、まはー裁定を待っている。

- **数える（実装済み）**: 暮らしコマ（出かける／自室で過ごす）1 つにつき 1（`day_plan._run_slot_life_session` がセッションを走らせた後）。**2026-08-09 時点で `used_pulses` を積むのはこの経路だけ**
- **数える予定（未実装）**: セッション系コマ内の標準パルス。現在の作業セッション経路（`_fire_slot` の予算ゲート・自由時間からの委譲）が積むのはラウンド台帳（`consume_life_rounds`）だけで、`used_pulses` へ入る口がない。作業コマの標準呼び出しを活動として数えるかは、数え方（セッション 1 本＝1 か、内部の標準呼び出しごとか）ごと未確定
...
消費は**自発活動 1 回ごと**に記帳する（実装済みは暮らしコマ 1 つ＝1 のみ。セッション系コマの計上は未実装 — §5.3 の数える/数えないの表と 2026-08-08 追補が正）。台帳はライフ（＝営業日の覚醒窓）単位で `used_pulses` を持ち、予算ゲートは残高ゼロでコマの標準パルスを skip する。判断点はゲートの対象外（止めると就寝判断すら撃てなくなる）——判断点の回数は観測値として別枠表示。
...
- v0.5 追補「予算の単位」(2026-08-08): §5.3 / §8.2 の計上単位を「標準 LLM が呼ばれた瞬間」から
  **「自発活動 1 回」**へ改訂。自律 Pulse の器統合 (autonomous_pulse_vehicle.md §A) で暮らしコマが
  WORKER アスペクト = 軽量モデルのセッションになり、旧文言のままでは暮らしコマが 1 回も
  数えられない状態になっていた (実装は 1 コマ 1 消費で先行、本書が追従)。
  未裁定として tell スペルの標準呼び出しを予算に数えるかを §5.3 に明記。
  指摘元は Codex レビュー (2026-08-08) の「予算の単位が分裂している」。
