...
- **却下は黙って捨てない**：置換が適用されなかった場合（空・全コマ無効・検証失敗）、finalize は「時間割の変更は適用されませんでした（理由）」をペルソナの文脈に乗る適用サマリ（lines）に必ず載せる。warnings（ログ）だけに落とすと、ペルソナは「組み替えた」つもりのまま一日を続け、就寝判断が実態とズレた総括をする（接地原則違反）。一部コマのみ棄却して適用した場合も除外件数を明示する
