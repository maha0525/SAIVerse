...
- **構想**:
  - **ユーザーアバター**: ユーザー自身をSAIVerse内を移動できる「アバター」として扱い、DBで現在位置を管理する。
  - **UI拡張**: 「ユーザー対話」タブが、ユーザーアバターがいるBuildingのログをリアルタイムに表示するように変更する。UIからアバターを好きなBuildingに移動させられるようにする。
  - **どこでも会話**: ユーザーがメッセージを送信すると、アバターがいるBuildingの会話履歴に追加され、そこにいるAIたちがリアルタイムに反応する。
