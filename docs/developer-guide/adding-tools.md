...
ツールは以下のディレクトリに Python ファイルとして配置します：
- `user_data/tools/` - カスタムツール（優先読み込み）
- `builtin_data/tools/` - 組み込みデフォルト
- `tools/defs/` - レガシー（後方互換）

Function Calling の仕様に従ってツールを定義すると、ペルソナがそのツールを呼び出せるようになります。サブディレクトリに `schema.py` を配置して複数ツールをまとめることも可能です。
