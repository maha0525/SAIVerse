...
### air_city_a — 豆粒の散乱なし。**W4 期産 8 件の扱いはまはー判断**

- スクリプトの選定基準 (豆粒・歪み) では削除対象 **0 件**。ただし W4 期 (旧コード) が編纂したエントリが **8 件**ある — 内容は「大昔 (4/29〜6/28) の取りこぼし拾い」の本物の記憶だが、**編纂したのは旧コード**。
- まはーの判断基準 (2026-07-29): **ここ最近のコードで動いて編纂されたものがあるなら、新版で回し直した方がいい**。→ この 8 件も削除→再編纂の候補。ただし被覆が古い (提示範囲外) ため、削除すると自動では再編纂されない — 対象の生ログ範囲を指定できる再編纂手段の確認が必要 (`build_arasuji.py` は全未編纂を対象にするので使える見込み)。実施可否・タイミングはまはーが決める。
