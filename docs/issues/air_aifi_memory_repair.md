...
- ⚠️ **別件の控え — air の 7 月自律行動の作話汚染は Memopedia にも及ぶ** (2026-08-02 発見、複製 DB の read-only 調査): 7/22〜30 の複利ハルシネーションで量産されたドキュメント群 (ペルモン構想書 / ペルモン設計書_確定版 / SIAgent技術リサーチ覚え書き / アンバー・テイル 等) が、**一枚ずつ terms エンティティページ + fragment として Memopedia に立っている**。まはーが検討中の「7/22 以降の記憶リセット」(時期未定・別判断) を実施する場合、会話ログ・Chronicle だけでなく **Memopedia のページと fragment も対象範囲**に含めて棚卸しが要る。放置すると経験の台帳 ([experience_ledger.md](../intent/experience_ledger.md)) がこれらを正当な経験として索引に載せてしまう。
...
1. ~~**記憶整理の「通信に失敗しました」は誤報** — 処理は裏で完走する。完走はログ (`Chronicle generation complete`) で確認~~ **2026-09-01 に解消** ([issue](archive/organize_memory_ui_timeout.md))。同期ルートを撤去して背景ジョブへ一本化したので、進捗と完了は記憶モーダルの「あらすじ」タブに出る。
...
- [organize_memory_ui_timeout.md](archive/organize_memory_ui_timeout.md) (解決済み) / [arasuji_modal_500_limit_truncation.md](arasuji_modal_500_limit_truncation.md) / [duplicate_message_ingestion_record.md](duplicate_message_ingestion_record.md)
