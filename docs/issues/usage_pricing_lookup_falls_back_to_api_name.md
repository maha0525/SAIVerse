...
判定条件は「渡された値が設定キーとして実在せず、かつ**その値を API 名に持つ別の設定が存在する**」。誤課金が実際に起きる条件そのものに合わせてあり、動的に組んだ設定やテスト用の架空キーでは鳴らない。`MODEL_CONFIGS` はモジュール属性として都度読む (`reload_configs()` が新しい辞書へ再束縛するため、import 時の辞書を掴むと再読込後に古い一覧で判定してしまう)。

**この境界では原理的に塞げないもの**: API 名がそれ自体別設定のキーでもある場合。`gpt-5.6-terra` は Codex 設定の API 名であり、同時に従量課金版設定のキーでもある。factory から見れば有効な設定キーを渡されただけで、呼び出し側がどちらを意図したかは判定できない。**つまり今回の当該ケースそのものは factory では検出できない**。防げるのは呼び出し側と保存境界だけ。
