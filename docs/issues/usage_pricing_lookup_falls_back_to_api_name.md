...
## 補足 — factory 境界の検出とその限界

`30ab9eb` で factory に検出を入れ、`76d4802` の次のコミットで条件を作り直した。**警告であって強制ではない。**

判定は二つ見る。①渡された値が設定キーとして実在せず、その値を API 名に持つ別設定がある。②設定キーとしては実在するが、**渡された `config` がそのキーに登録された設定と `provider_ref` / `provider` で食い違う**。②は実害が出た `scripts/` の呼び出しの形そのもの — `gpt-5.6-terra` は従量課金版のキーとして実在するので①では捕まらないが、渡していた config は `provider_ref` が `openai_codex` で、キーに登録された設定の `openai` と食い違う。

`MODEL_CONFIGS` はモジュール属性として都度読む (`reload_configs()` が新しい辞書へ再束縛するため、import 時の辞書を掴むと再読込後に古い一覧で判定してしまう)。

**残る死角**: model 文字列だけが渡され、config からも意図が読めない呼び出し。この場合 factory には有効な設定キーが来たようにしか見えない。防げるのは呼び出し側と保存境界だけ。

> **訂正 (2026-08-01、Codex 六巡目)**: ここには当初「API 名がそれ自体別設定のキーでもある場合は**原理的に**検出不能」と書いていたが、**過剰な断定だった**。実害が出た呼び出しは model 文字列に加えて Codex 用の config と provider も factory へ渡しており、その不一致は境界で観測できた。「model 文字列だけでは判定できない」が正しく、config がある呼び出しは②で捕まる。不可能と書いたことで、検出できる取り違えまで保存境界へ先送りする理由にしてしまっていた。

**「呼び出し側は全て直した」の範囲** (2026-08-01 訂正): `a631096` で直したのは `scripts/` の 6 箇所。`persona/core.py` / `manager/admin.py` / `sea/runtime.py` は persona の設定値をそのまま factory へ渡すので、**第3節の保存境界が正規化されない限り、そこから API 名が流れうる**。リポジトリ全体の不変条件としては成立していない。

**強制化は裁定待ち**: 警告には強制力が無く、ログを見なければ気づかない。ただし失敗にすると `config` を直接渡す正当な呼び出しを壊す範囲が読めない。第3節の「保存境界で正規化する」と同時にやる方が筋が良い — 入口で正規化されるなら factory で弾いても壊れない。
