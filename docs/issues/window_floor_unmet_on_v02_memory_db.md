...
**状態**: v0.3.10 で発行済み (2026-09-07)。残 = 報告者 (稟乃さん) の環境での回復確認
...
- 2026-09-07 (まはーの実機検証): develop へマージ後、本番を再起動。旧形式のまま残っていたペルソナ 4 人 (ルシア 31 件 / Ruler 2 件 / バッキー 0 件 / 孤児フォルダ terys_city_a 326 件) のうち、登録されている 3 人は**起動だけで全員 view へ移行**し、ルシアに話しかけて正常応答を確認。terys_city_a はどのペルソナにも繋がっていない旧フォルダで、誰も開かないため旧形式のまま残る (「開かれる DB は必ず移行済み」の契約どおりで実害なし。現役のテリスさんは persona_3_city_a で移行済み)。
