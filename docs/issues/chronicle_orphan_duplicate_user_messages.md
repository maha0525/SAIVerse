...
W4 (`277bd8d`, 2026-07-21 07:48) で Chronicle の未処理判定が「進捗ポインタ」から「級 1 エントリの `source_ids` 全部の集合との引き算」に変わった ([`session_lifecycle.py:1162-1174`](../../sea/session_lifecycle.py))。取りこぼしを拾う正しい変更だが、その結果:

- 旧ポインタ方式では見えなかった複製行 (#9238 / #9243) が「未編纂」として発見された
- 前後を編纂済みメッセージに挟まれた孤立 1 件なので、設計どおり恒等圧縮 (identity) として確定した
- 要約ではなく `[2026-07-18 21:04] [user]: …` という**生ログが本文の級 1 エントリ**になった
