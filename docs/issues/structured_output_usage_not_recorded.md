...
**状態**: 実装済・実機検証待ち (2026-08-01 起票 → 同日修正)。Codex レビュー一巡目の指摘3・二巡目の指摘4・七巡目の指摘6/7。

**当初「今回の変更とは独立の既存欠陥」としてスコープ外に置いたが、判断を変えて修正した。** Codex が三巡連続で high として挙げ、七巡目で「使用量帰属の scope 外にできない直接違反」と反論してきたため。不変条件は「使用量と費用を設定キーに帰属させる」ことだが、**usage がそもそも記録されなければ帰属以前**であり、分離の理由が目的から見て成り立っていなかった。
...
- **経路1** (`llm_clients/openai.py`): `_stream_text_mode` の非ストリーム分岐で `openai_runtime.store_usage_from_response` を呼ぶ。ストリーム分岐が使っている保存処理と同じもの。
- **経路2** (`llm_clients/nvidia_nim.py`): `_create_nim_structured_output_via_tool` のレスポンス JSON から `usage` を読み、`_store_usage` を一度だけ呼ぶ。抽出が失敗して例外になる分岐でも API 呼び出しは成立して課金されているため、抽出の成否より手前で記録する。
- 回帰テスト `TestStructuredOutputRecordsUsage` を追加。修正を外すと落ちることを確認済み。

**残り**: NIM 経路は実レスポンスでの確認をしていない (テストは OpenAI 側の非ストリーム分岐のみ)。NIM の構造化出力を使う実機実行での確認が要る。
