...
報告者の壊れた状態を手元で再現して、修正後のコードで通した。まはーの環境で旧形式のまま残っていたペルソナ (ルシア、旧形式 31 件、13 MB) の memory.db を読み取り専用で複製して隔離環境 (test_data) のテスト用ペルソナに置き、複製の上で「31 件全部をページにした後 DROP TABLE で倒れ、さらに 5 件分だけページを失った」状態 (26 / 31 がページ、旧テーブルは残存、先頭の旧 id は既にページ = 旧コードなら初手で UNIQUE 違反) を作った。その上で隔離環境のバックエンドを起動し、API を叩いた結果:

- `GET /api/people/<id>/arasuji` (Chronicle 一覧): **200、0.08 秒** — 旧コードならここで倒れていた
- 直後の `GET /api/people/<id>/memopedia/tree`: **200、0.009 秒** — 報告者の環境で「読み込みが終わらない」だった経路
- 二度目の Chronicle 一覧: 200、0.018 秒 (もう移行は走らない)
- ログ: `legacy migration resumed: 26 of 31 rows were already pages ... migrated the rest` が出た
- DB: 旧テーブルは消え、互換 VIEW があり、Chronicle ページは 31 件、short_id は 31 個すべて一意

本番の DB には読む以外で触っていない。隔離ペルソナの DB は元に戻した。残るは報告者の環境での確認だけ。
