...
**この時点の収束状態**: 指摘ゼロでは終わっていない（次の巡を投げれば同じ 4 件が別の顔で出る見込み）。ただし残りは実装の粗ではなく**設計の欠落**なので、巡を重ねても埋まらない。

**まはー裁定（2026-08-14）: ここでレビュー完了。** 根拠は「指摘が今回の実装範囲を外れた根っこまで到達している」こと。近似の歯止めのまま実機検証へ進み、receipt は別セッションで設計する（受け入れたリスクは切り出し先 issue の「近似のまま進めた裁定」節）。
