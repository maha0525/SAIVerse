...
**ステータス**: ✅ 対象消滅 (2026-08-25) — **re-speak 機構そのものを撤去したため、分裂の起点が無くなった**

2026-08-25 のまはー裁定で「追加の推論は必ずユーザーの一押しの後ろに置く」が原則になり、
`_respeak_after_stream_timeout` は削除された。ストリームが中断されたときは、部分文に
「言い切っていない」印 (`_interrupted`) を立ててその場で Beat を閉じる — 二回目のコールを
打たないので、**一つの発言が二つのメッセージに分裂する経路が構造から消えた**。
続きはユーザーがボタンで起こす形になり、そのとき何メッセージで表すかは新しい設計の管轄
([`user_utterance_path_failure_inventory.md`](user_utterance_path_failure_inventory.md))。

なお本 issue の起草時 (2026-05-20) の調査でも「残存 session log 61 件中、発火の記録は
1 件のみ」と書かれており、2026-08-25 の全数検索 (654 セッション) でも発火はゼロだった。
以下は撤去前の記録。
