...
## 同族 (2026-08-25 発見、実データ付き)

同じ「例外が誰にも届かない」型が、API ルート側にもある。**ログにすら残らない**ぶん、こちらの方が調査が詰む。

- 現物: 2026-08-25 19:18:01、まはーが Chronicle タブを開いた 1 回だけ
  `GET /api/people/eris_city_a/arasuji/stats` が **500** を返した (同じ呼び出しは 34 秒後に 200、
  読み取り専用で同じ 2 クエリを叩き直しても正常 — 一過性)。
- 原因が追えない理由: `api/routes/people/arasuji.py` の `get_arasuji_stats` は
  `except Exception as e: raise HTTPException(500, detail=f"...: {e}")` だけで、**LOGGER を一切呼ばない**。
  例外の型もスタックも `backend.log` に残らないので、後から何が落ちたのかを再構成できない。
  当該セッションのログを全走査しても、この 500 に対応する行は uvicorn.access の一行だけだった。
- 族の大きさ (2026-08-25 実測): `api/routes/people/arasuji.py` の `except Exception as e:` は 12 箇所あり、
  そのうち **9 箇所がログを一切残さずに 500 へ詰め替えている** (L169 stats / L459 一覧 / L510 単体取得 /
  L544 削除 / L570 更新 / L592 一括削除 / L638 元メッセージ / L676 断片 / L711 メッセージ)。
  ログを通しているのは 3 箇所だけ (L370 diagnosis / L753 regenerate / L875 generate)。
  **削除・更新 (L544・L570・L592) が無言で落ちる**のがいちばん重い — 記憶の操作が失敗したことが
  どこにも残らない。
- 直す形: 詰め替える前に `LOGGER.exception(...)` を通す。他の `api/routes/` にも同じ書き方が
  無いかを同じ数え方で確認する。

規律の出どころ: 「エラーは、握り潰した場所ではなく**届く場所**を決めてから握る」。UI への通知 (本 issue の本体) と
ログへの記録 (この同族) は別の受け手で、片方だけでは調査も救済も成立しない。
