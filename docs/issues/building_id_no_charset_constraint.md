...
**ステータス: 実装済み・レビュー待ち** (2026-08-08 起票 → 2026-08-09 論点 1 を実装 (v0.3.0 の門 Wave 0)。作成の口 `manager/admin.py create_building` (API・ツール・manager 委譲の全経路が集約) に文字種契約を追加: カスタム ID は `[A-Za-z0-9][A-Za-z0-9_-]*` 以外を拒否、自動生成は ASCII slug 化 + 日本語名は `building_<連番>_<city>` フォールバック。読み変換 (ローマ字化) は導入しない。**論点 2 (既存 3 件) は裁定どおり放置**、論点 3 (City/ペルソナ ID の同族点検) は未着手のまま残る)
