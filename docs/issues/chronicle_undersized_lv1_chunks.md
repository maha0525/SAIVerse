...
**既存の小粒ノードの掃除方針は決着 (2026-07-27)**: [chronicle_consolidation.md](../intent/chronicle_consolidation.md) の新束ね機構が自動回収する — 質量の近い塊は丸ごと束ね、束ね不能ノードは治療 (隣人への合流) で拾う。手動の削除・再編纂はしない。archive 判断は「本 issue の実機検証 (a)」と「同 intent の実装」の両方が済んでから。
...
歯止めは二つ。(a) U 以上の候補が一つでもあればそちらを常に優先する（[`chronicle_eviction.md`](../intent/chronicle_eviction.md) §5-5 の二段構え）。(b) 二段目は一回の Metabolism で一回だけ。小粒が出た回は `used_last_resort_fold` でログに残るので、定常運転に堕ちていないか観測できる。
...
- 実装は `plan_alignment` に「末尾の端数を持ち越すか」のフラグを足し、自動経路 (`evict_boundary_epoch` あり) では持ち越し、全量整理 (force / 手動整理) では従来どおり吐き切る、が素直。
