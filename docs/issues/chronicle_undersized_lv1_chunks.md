...
> 5. **残る問い (裁定待ち)**: 被覆の**字数**は今後も U の半分程度に見える見込み。長いスペル結果は決定論の一行 (数十字) に縮んで材料に入るため、10,000 字 (生) の fold の材料は約 5,000 字にしかならず、fold の刻み (生の字数の水位) とチャンクの目標 U (材料の字数) の**物差しが違う**まま残る。一次あらすじを「材料 10,000 字級」で立てたいなら、退場の刻みを材料の字数で測るよう物差しを揃える改修が要る (小さめのダイヤル調整)。現状のままでも嘘は無い (被覆 ID は完全、字数は材料の実量) ので、揃えるかはまはーの裁定。
