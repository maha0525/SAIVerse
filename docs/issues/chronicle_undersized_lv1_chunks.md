...
**ステータス**: 🟡 実機検証の結果が出た (2026-08-29) — 半分成功・残り半分は原因特定済み。裁定待ちの問いが一つ残る (下の「2026-08-29 実測」)。

> **2026-08-29 実測 (まはーの観察「7 件で 1 あらすじができてる。本当に 1 万字あるのか」を受けた検分)**:
>
> 1. **豆粒 (9 字・20 字の identity) は消えた** — 直近 20 件の最小は 1,625 字で、identity の新規はゼロ。7/25 の修正はこの点では効いている。
> 2. **しかし U 級 (10,000 字) には立っていない** — 直近の一次あらすじの被覆は実測 2,000〜5,400 字が大半 (帳簿と実測は一致 — 勘定の嘘ではない)。
> 3. **原因を特定した**: 各 entry の期間内には、当時編纂から除外されていた機構名義の行 (朝の Elyth 読みのスペル結果 6,000 字級 + 通知) が **6,000〜10,400 字**ずつ実在する。つまり退場 (fold) は毎回 10,000〜14,000 字ぶん動いていたが、**あらすじに写っていたのは会話の半分だけで、残り半分は §7-1 の旧例外 (2026-08-29 に撤回した「被覆外退場の割り切り」) として黙って被覆の外へ出ていた**。小さい一次あらすじは、あの除外の目に見える傷跡だった。
> 4. **8/29 の編纂材料の一本化で、ID の網羅は解決** — 今後の整理では機構名義の行も被覆 (source_ids) に入り、「fold の中に覆われない行がある」状態は構造的に消える。
> 5. ~~残る問い (裁定待ち)~~ → **裁定済み (2026-08-29 夕、まはー)**: U 到達の判定は圧縮後 (材料) の字数で測る。「あらすじを作る理由はあくまで圧縮にあって、圧縮の意義が薄いあらすじ区間を作るべきじゃない」(スペル結果が 1 万字あっても、その流れだけであらすじが立つのは目的に反する)。退場の fold が U で閉じる判定と手動確認窓の「畳めるか」を材料字数へ揃え、提示コストの水位 (残す量・上限) は生のまま、非常経路だけが材料 U 未満の fold を許す — 実装は同日 (arasuji_levels.md §7-1 の U の項)。これで本 issue の「U 級で立つ」は材料の物差しで成立する見込みになり、残りは実機検証のみ。

旧ステータス: 検証待ち — 当初の借用実装は撤回、[`../intent/chronicle_eviction.md`](../intent/chronicle_eviction.md) で再設計。**本 issue の範囲 (生ログ → 一次あらすじ) の実装は完了** (`e8c061d` + `3fa2711` + `53448ef`、レビュー Codex 3 巡 + サブエージェント 1 巡を消し込み済み)。残るのは**まはーの実機検証** = intent §9 の (a)「一次あらすじが U 級で立ち、9字・20字の小粒が消える」。
