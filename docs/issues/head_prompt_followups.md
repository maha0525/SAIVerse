...
### 2026-09-09 追記: ユーザー実害としての再提起（まはー）

v0.2 系と比べたシステムプロンプトの肥大の主因がこの一覧で、普段使わないスペルのための 1 万字超が全ペルソナ・全送信の固定費になっている。アップデート後のトークン増というユーザー報告（2026-09-09、複数件）の一因であり、優先度が「積み残し」から一段上がった。二つの接続先が増えた:

- 操作通知（スペル付与の通知）は head と同一の描画を同梱する設計なので、**head 側の一覧が痩せれば通知も一緒に痩せる**（[presented_context_reduction.md](../intent/presented_context_reduction.md) の隣接別件）。
- キャッシュあり運用では「一覧を絞って、必要時に引かせる」方が効率的になりうる — 常時全見せを既定にし続けるかは、ideas 帳のスタイル構想（キャッシュの使い方別の方針プリセット）と接続する。
