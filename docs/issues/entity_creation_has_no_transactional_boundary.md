...
- **現 (DB 先)**: DB に行はあるがメモリには**部分的にしか**載らない = *孤児*。
  失敗した位置によって「DB のみ」「DB + Building キャッシュ」「DB + キャッシュ +
  personas への登録まで」と残り方が変わる (起草時に「メモリに載らない」と一律に
  書いたが不正確だった)
...
## 隣接して確認された共有状態の問題 (2026-08-09、レビュー第 7 巡)

作成だけでなく **reload でも** 同じ「誰が正本を持つか」の欠落がある。

- `_reload_buildings` は `buildings` / `building_memory_paths` を新しいオブジェクトへ
  差し替えていた。`CoreState` はこれらを参照で共有する設計なので、差し替えると
  manager 以外が古いまま取り残される。**同日 in-place 更新へ修正済み。**
- ただし `PersonaCore` は構築時に `{b.building_id: b for b in buildings}` で
  **自分の dict を作る** (`persona/core.py`)。参照共有ではないので、in-place に
  してもここには届かない。既存ペルソナからは新しい建物が見えず、削除された建物が
  残り続ける。**未解決** — 建物の増減を既存 PersonaCore へ伝える経路が無い。

この二つは「作成」ではなく「世界状態の正本と同期」の話だが、根は同じ (共有状態の
オーナーシップが決まっていない) なのでここに併記する。
