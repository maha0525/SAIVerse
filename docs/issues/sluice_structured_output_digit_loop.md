...
## 22 欄の隔離実験 — 現行の型では再現なし (2026-08-24、まはー承認のもと API 90 回)

危ない並びに当てはまる 5 型を選んで、壊れた 3 モデル (3.7-flash / 3.6-flash / 3.5-flash-lite) に合成の場面で叩いた。選んだのは: 予定管理の削除 `schedule_id` (一覧の ID を数字で書かせる — 壊れた memory_id と同族) / メール送信 `user_id` (数字の直後に件名・本文) / memory_clip `rounds` (任意の逐語引用欄を飛ばした隣の任意整数) / 建物作成 `capacity` (長文欄に挟まれた任意整数) / 予定管理の追加 `days_of_week[]`+`interval_seconds` (整数の配列 = フォーラム報告の形)。

本番の写しとして再現した条件: `_schema_from_json` と同じ変換 (`default`/`minimum` は wire に載せない・宣言順を `propertyOrdering` として送る・properties の無い object は string に化ける)、thinking はモデル設定の既定 (medium / medium / minimal)、REST 直叩きで生の応答を全部保存。出力上限 2,000 は実験の安全弁 (本番のこの経路に上限は無い)。

- **短い場面** (入力 170〜353 トークン) × 5 型 × 3 モデル × 3 回 = 45 回: 全回 STOP・JSON 妥当・値も正解。最長の数字列は 3 桁 (`m:103` の写し)。
- **本番相当の長い文脈** (入力 10,268〜10,693 トークン。日付・時刻・金額・ページ数を自然に含む合成会話で、スルース再現実験と同じ入力規模) × 同じ 45 回: 同じく全回正常。任意欄の省略 (capacity を書かない回など) も正しく起きた。

**90 回で数字ループ・欄の欠落・本文の流れ込みは一度も出なかった。** スルースの旧 schema が同じ入力規模で 5 モデル中 3 つ一発で壊れたのと対照的で、壊れの要因は「数値欄がある」ことそのものではなく、旧 schema の形 (操作の配列 + 任意欄を飛ばした先に、飛ばした中身を吐き出せる参照欄が来る文法) に強く依っていたと解釈できる。ただし 1 型あたり 18 回なので、数 % 級の低頻度の壊れまでは否定できない。

副次の観察: 予定追加の `args` 欄 (properties の無い object) は本番の変換で string になり、モデルは空文字を返す。既存挙動で本件とは別。

実験スクリプトはスクラッチパッド (`numeric_probe/`) にあり、セッションと共に消えるが、要点は全部この節に写した。

**まはー裁定 (2026-08-24): 現状維持。** 再現できなかった危険のために境界の一括変換 (数値欄を文字列で訊いてこちらで戻す機構) を新設しない。守りは機械検査 (`KNOWN_NUMERIC_FIELDS` の両方向検算) が続け、本番でこの経路の数字ループが実際に観測されたら、この節の実験手順で叩き直してから型を変える。経路 B の 2 欄 (起床判断 `budget_rounds` は v0.4 の配線前、夜間編纂 `block_indices[]` は編纂の再開前に直す) だけは従来どおりの宿題として残る。

## 別 issue との関係

[cold_anchor_advance_bypasses_sluice.md](archive/cold_anchor_advance_bypasses_sluice.md): スルースが失敗しても起点が進んでいた。本 issue が直っても、あちらが直らなければ退場の不変条件は守れない。両方直してからリリース。
