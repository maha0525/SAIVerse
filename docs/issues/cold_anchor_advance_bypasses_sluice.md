...
## 被害の範囲 — 実測で訂正 (2026-08-24)

**エリスには被害が無かった。** 読み取り専用で数えた結果: パンマーカー (スルースが最後に見た位置、`sluice_last_pan_id` = gold_panning 時代の 2026-08-16 17:35 のメッセージ) は、今日前進した起点 (2026-08-14 ごろの `3b29bc72`) より**先**にある。つまり今日押し出された範囲は全部「旧スルースが 08-16 に読んだ範囲の内側」で、読まれていない範囲 (08-16 以降) は一つも退場していない。次にスルースが成功すれば、08-16 以降の全部 (約一週間分) を本人が読む。**採取し直しは不要。** 今日入れた頭打ちの修正でも、この前進は同じく通る (最前線がマーカー以前のため)。

下の「読み」は訂正前の記述 (機構の穴自体は実在し、修正も正しい。被害の見積もりだけが過大だった)。

## 被害の範囲 (2026-08-23 時点の読み — 上の実測で訂正済み)
