...
API キーが必要な場合は、保存後にプロバイダ一覧のその行の **「キー設定」** ボタンから
API キーの値を入力します（プロバイダ JSON には環境変数名のみ書く。値は `.env` に
保存され、再起動なしで次の返答から使われます）。グローバル設定 > 環境タブ >
サーバー環境変数から設定しても同じです。
...
保存するとプロバイダ一覧に `kimi` の行が現れるので、その行の **「キー設定」** を押し、
Moonshot 発行の API キーを入力して保存します。再起動は不要です。
