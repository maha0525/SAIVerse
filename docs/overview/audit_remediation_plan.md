...
実装 wave    ███████▶ W1〜W8 + W10 + W13 + W14 実装済み / W4 は検証を世代交代先へ移管 / W9 は v3 待ち凍結 / 残 = W11 + A3 裁定 + W12
...
**次にやる wave**: **W10 は 2026-08-16 実装完了** (Spell 監査残 3 件消し込み + auto_mode 常時 False の実バグ根治。明細は W10 行とレビュー台帳 Spell 行)。実働の残りは **A3 の着手裁定 (まはー)** と W11 (W14 は 2026-08-19 実装完了 — 明細は W14 行)。全 wave 決着後に **W12 (レビュー台帳の全行最終照合)** で監査を閉じる。実機検証は統合検証手順の走行の続き (Step 3 一日の走行〜) が本線。
...
- **切り出した 2 件は同日裁定済み**: [許可ゲートの被覆](../issues/archive/playbook_permission_gate_coverage.md) — ① `user_only` は「ペルソナに対する禁止」と確定し、ユーザー本人の指定は通す形へ実装 (確認ダイアログのボタン文面も同時に是正) / ② SUBPLAY が許可判定を通らない件は放置と確定 (再開条件 = ペルソナが Playbook を書けるようになったとき)
...
### W14 ☑ 体験の構造 工程(4) — 知覚レンダリング (監査外) — 実装済み・実機検証待ち (2026-08-19)
...
- **実施 (2026-08-19)**: 消費バッチ設計 (perception_buffer.md §10 — 台帳正準・提示は未付記バッチの時刻順マージ・退場付記+付記印・読み口=検索包含)。flush の event_message 行書き込みを全廃、迂回直挿し 2 経路 (移動失敗・アップグレード通知) を移送。Codex 敵対レビュー 8 巡 30 件消し込み (既存欠陥 3 件の根治込み)、専用回帰 89 件。走行メモ = [handoff](../handoff/2026-08-19_w14_perception_rendering_handoff.md)
- **完了条件**: event_message 直挿しの段階的廃止が始まり、通知洪水が観測面で解消 — 廃止は本体経路で完了、観測面はまはーの実機検証で確認
