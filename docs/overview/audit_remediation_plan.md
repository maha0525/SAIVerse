...
実装 wave    ███████▶ W1〜W8 + W13 実装済み / W4 は検証を世代交代先へ移管 / W9 は v3 待ち凍結 / 残 = W10・W11・W14 + A3 裁定 + W12
...
**次にやる wave**: 実働の残りは **W10 (柱8 — S9 消滅済み、残は Spell 監査残)** と **A3 の着手裁定 (まはー)**。W11 は最後尾のまま、W14 は体験の構造 工程(4) として存続。全 wave 決着後に **W12 (レビュー台帳の全行最終照合)** で監査を閉じる。実機検証は統合検証手順の走行の続き (Step 3 一日の走行〜) が本線。
...
### W10 ☐ 柱8 — 独立小物 — 次の実装 wave

- **スコープ**: Spell 監査残 (realtime spell の SPELL_ENABLED 迂回・auto_mode 固定 / `_` 予約 namespace / 入力 contract)。着手時に柱8 の全量を現行コードで棚卸しして確定 (SPELL_ENABLED 機構の現存は 2026-08-16 に確認済み、迂回が残っているかは着手時に検証)
