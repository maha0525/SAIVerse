...
graph TD
    ...
    Spell -->|呼ぶ| Tool
    Pulse -->|記録| SAIMemory
    Metabolism -->|圧縮| Chronicle
    Metabolism -->|知識化| Memopedia
    Metabolism -->|更新| head
    Chronicle --- SAIMemory
    Memopedia --- SAIMemory
    head -->|構成| Pulse
...
**3つのハブ概念**（線が集中する中心）:
- **Pulse** — 駆動の中心。Track・Playbook・SAIMemory・head すべてに接続する
- **SAIMemory** — 記憶の中心。Chronicle・Memopedia の源泉
...
「どの Track を動かすか」を判断する上位視点（通称「メタレイヤー」）。実装は `meta_judgment.json` Playbook で、専用 LLM ノードが「現 Track 続行 / 別 Track を activate / 新規 Track を create」を決める。判断ログは `meta_judgment_log` に蓄積され、次の判断時に参考情報として注入される。
...
```mermaid
graph TD
    MetaJudgment["Meta-Judgment"] -->|選択| Track
...
Beat の構成: 発話ノード(LLM)の出力 + Spell loop 全 round の本文 + 各 Spell の `<user_only>` 結果ブロック + 最終 continuation の連結。Beat は記録先で2つに割れる: **表示用** = `full_merged_text`（Spell 結果込みの合成版、UI バブル / 建物履歴 / ペルソナ履歴）/ **SAIMemory 保存用** = `final_continuation`（最終発言のみ、重複回避）。
...
Spell の本質的目的は **ネイティブツールコールの撲滅**にある。ネイティブツールコールは必ずコンテキスト最上部に来るが、構造化応答ではそれが使えないため、平文応答と構造化応答でプロンプトキャッシュが必ずミスする。Spell 化すれば**平文応答と構造化応答でキャッシュを共用できる**。
...
- **戻り値は `report_to_parent`**（文字列）。サブライン内の試行錯誤は `line_role="sub_line"` で記録され、親メインラインの context からは自動除外される（揮発するが SAIMemory には残る）
...
ペルソナの経験は層をなして保持される。生ログから始まり、時間軸で圧縮され、やがて知識へ結晶化する。**SAIMemory** が生ログ、**Chronicle** が時系列圧縮、**Memopedia** が知識グラフを担う。
...
ペルソナが活動を続ける中で、LLM へ送る context の先頭領域（prompt cache が効く部分）を安定させることは、継続的な体験とコスト効率を支える。「節目」は、その安定性を保証するための関連概念群である。

### head（キャッシュの効く先頭領域）

LLM に送る context のうち prompt cache が継続して効く先頭領域。`LineHeadSnapshot` として freeze された Section 群（common_prompt / persona_self / building / spell_list / available_playbooks 等）の render 結果で構成される。snapshot の更新は Metabolism または明示的なイベントでのみ起き、平時は immutable。head 文字列が変動しない限り cache hit が継続する。**機構は実装済（`sea/head_pipeline/`、Phase 1 完成）**。

### Metabolism（節目の発火）

節目を「作動」させるイベント。発火すると全 Section に `capture(live_state)` を走らせて frozen snapshot を再構築し、同時に履歴圧縮・Chronicle 化・Fragment 生成（§5）などを束ねて実行する。`_resolve_metabolism_anchor` が3段フォールバック（当該モデルの anchor → 別モデルの最新 → 最小ロード）で文脈取得を切り替える。**実装済**。
...
> **起草中**（`session.md` v0.1）。**コード上にはまだ「Session」という統一制御単位は存在しない**（現状は anchor touch → 履歴取得 → head render の三部構成で個別に動く）。

Metabolism で区切られた連続活動区間を **Session** として定義する構想。粒度は `(persona_id, model_key)` 単位で、同じペルソナでも model が違えば別 Session。WORKER（サブライン）は親 Session の中の「子 Session」として発生する。複数 model 並走（Claude メタ判断 + Gemini 自律など）では各 Session が独立に Metabolism を発火し、片方の節目が他方を巻き込まない設計を目指す。

```mermaid
graph TD
    Anchor -->|TTL 切れで trigger| Metabolism
    Metabolism -->|全 Section 再 capture| head
    head -->|cache 安定| Pulse
    Session -.->|"節目と節目の間 (起草中)"| Metabolism
```
...
| **Emotion** | PersonaCore の感情モジュールとして存在するが、intent / issue / memory に言及なく実質未活用 |
...
| Beat | 記録 | SAIMemory | `final_continuation`（最終発言のみ） |
...
| Pulse | 記録 | SAIMemory | ログを SAIMemory に追記 |
| Metabolism | 圧縮 | Chronicle | ArasujiGenerator がバッチ圧縮 |
| Metabolism | 知識化 | Memopedia/Fragment | entity_extractor が batch_callback で相乗り |
| Metabolism | 更新 | head | 全 Section snapshot を再構築 |
| Anchor | 印付け | head/Metabolism | TTL 切れで Metabolism を trigger |
| head | 構成 | Pulse(prompt) | キャッシュの効く先頭領域 |
