...
- ✅ **自律行動 v2 (旧 Phase 3 の後継)** — 旧「自律バイオリズム (1 時間サイクル)」構想は v1/v2 の実機診断を経て**時間割モデル**に置き換わった: 習慣テンプレート (ユーザーとペルソナの合意で固定した一日の枠) + 判断点 (起床/就寝、専用 Playbook) + 予算付き作業セッション + コマ種別カタログ ([`timetable_redesign.md`](../intent/timetable_redesign.md))。骨格・時間割改修・実行台帳 (W1〜W8) まで実装済み — **v0.3 (develop) は止め具で運転を凍結した** (`saiverse/autonomy_wiring.py` の `AUTONOMOUS_DRIVING_SHIPPED = False`、[`autonomous_behavior_v3.md`](../intent/autonomous_behavior_v3.md) §11.1。2026-08-23 まはー裁定で関連 6 件を台帳から外して凍結)。**develop-v0.4 では 2026-09-25 に止め具を定数ごと撤去し、関連 6 件の凍結も解いた** — 自律 ON のペルソナで判断点・見張り・時間割のコマが動くようになったので、この領域の実機検証はここから再開する
