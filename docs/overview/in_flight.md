...
| 🟣 検証待ち | Memopedia 書き込みのロック漏れ (抽出器ほか) | adapter ロックの配線・握り潰し撤去に加え、失敗した抽出を付箋に残して次の Metabolism の頭で拾い直す機構 (上限 3 回) まで実装済み。次 = 実機で Metabolism を一度回し、抽出が通ることと失敗時の通知・拾い直しを確認。 | まはー (実機確認) | [issue](../issues/memopedia_writers_bypass_adapter_lock.md) | 2026-08-06 |
