...
| 🟣 検証待ち | 監査第二陣・共通境界hardening | 公式 Addon registry の署名対応が済み、カタログの fail-closed は解除済み。次 = 実機のカタログ UI で一覧が表示されることの確認。非 Git updater は署名済み release manifest 設計まで fail-closed のまま (別件)。 | まはー(カタログUI確認) | [第二陣Intent](../intent/audit_second_batch_hardening.md) / [レビュー台帳](code_review_ledger.md) | 2026-08-17 |
