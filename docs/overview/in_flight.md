...
| 🟢 実装中 | v0.3.0 の門 (リリース範囲と実施順序) | episode.md はレビューの往復で v1.4 に達し、概念はエピソードとチャンクの二つ + メッセージへの記録まで削れた。コネクション廃止と旧概念の残存棚卸しも済み。次 = まはーが v1.4 を通読レビューして Wave 1 を締め、Wave 2 (モデル格と保温、生ログ正史を含む) の設計議論を開く。 | まはー (v1.4 レビュー → Wave 2) | [v030_release_gate.md](v030_release_gate.md) / [episode.md](../intent/episode.md) | 2026-08-11 |
