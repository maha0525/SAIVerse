...
| 🟠 実装中 | 通話モード (Gemini Live でペルソナと音声通話) | intent v0.1 を図面に、backend (WS 中継・文脈積み込み・記憶書き戻し) と frontend (通話画面・マイク取り込み・再生) を実装する。次 = 実装 → レビュー → まはーの実機体感とフィードバック。 | 私 (実装) | [voice_call.md](../intent/voice_call.md) | 2026-09-16 |
