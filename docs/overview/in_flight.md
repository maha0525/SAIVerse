...
| 🔵 設計中 | 極小 run の隣人吸収 (豆粒編纂の捏造根治) | 方向と主要裁定は確定 (吸収先=後ろ / 閾値=材料 0.5U / 上位は最上位まで連鎖再生成)。帳簿の追随調査も issue に収録済み。次 = 設計の残り (印の書き直し方・Fragment 付け替え・Lv2+ 再生成の形) を詰めて実装へ。完了後に aifi の豆粒後始末 (削除→補修再実行)。 | 私 (設計→実装) | [issue](../issues/arasuji_tiny_run_absorption.md) | 2026-08-31 |
