...
| 🔵 設計中 | 知覚バッファ (Perception Buffer) | v0.1 draft。核=「主観時間は Pulse でのみ進む／知覚は Pulse 消費時のみ／未消費の間だけ型別 reduce／知覚イベントは起動力属性を持ち決定論で反応可否を決める」。差分通知・メタ記憶訂正・入室想起・**会話取り込み**を一本化する横断基盤。確定: 消費者=全Pulse・会話も経路統合・粒度=per-persona・未消費プレビューは ContextPreviewModal に載せる(ペルソナホーム連携は別途)・軽量LLM門番(SN)は将来余地のみ。残未決: 再起動耐性/検知方式/編集粒度(いずれも推し有り)。次: draft レビュー → Phase 1(器+起動力ディスパッチャ+「絶対反応する」フラグを現状§3.1全契機に付与+メタ記憶訂正載せ替え) | まはー(レビュー) → 私 | [perception_buffer.md](../intent/perception_buffer.md) | 2026-07-09 |
