...
| 🟣 検証待ち | v0.3.0 の門 (リリース範囲と実施順序) | アップグレード検証は段 3 (update.bat 経路) まで全段合格し、掃討チェックリストの消化が進行中。次 = 掃討の緑をまはーの報告で消し込み、リリース判定点の裁定をまはーと行う。リリース時の必須作業 = dev6→0.3.0 の空エッジ追加と、リリースノートへの「update.bat の後に start.bat を実行すれば残りは自動で完了する」の明記。 | まはー (判定) | [v030_release_gate.md](v030_release_gate.md) / [アップグレード検証](../handoff/2026-08-29_v0229_upgrade_test.md) | 2026-09-01 |
