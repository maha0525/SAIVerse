...
| 🟠 実装中 | 知覚が多いと「残す量」と「整理を始める量」を両方満たせない状況への対応と、既定値の見直し | 低水位の廃止・既定値 (残す量 4万 / 上限 12万)・逆転クランプまで実装とレビュー消し込みが済み、まはーの体感チェック待ち。次 = 並行して、知覚の二水位上限 + 省略の印 + 保存時検査と、供給側 (想起の門 / スペルのスキーマ同梱 / 部屋の差分+移管) を順に実装する。 | 私 (実装) | [issue](../issues/watermarks_unsatisfiable_when_perception_is_large.md) / [metabolism.md](../concepts/metabolism.md) | 2026-09-04 |
