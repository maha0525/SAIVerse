...
| 🟢 実装中 | v0.3.0 の門 (リリース範囲と実施順序) | v3 §9 の未決は 1〜8 が決着 (スキーマ具体形・ティック命名と単発制御・tell 引数式・システムタスク新設・移行は機械写しで済む、ほか)。次 = 残る §9-9 (UI 導出) と連続性グラフを詰め、机上完動の判定をまはーと行って v0.3 実装 (形の層) へ。 | まはー (設計議論の続き) | [v030_release_gate.md](v030_release_gate.md) / [autonomous_behavior_v3.md](../intent/autonomous_behavior_v3.md) | 2026-08-18 |
