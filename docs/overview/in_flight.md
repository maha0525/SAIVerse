...
|---|---|---|---|---|---|
| 🟡 実装待ち | 知覚供給側の修理 (再会の想起の門 / スペルのスキーマ同梱 / 部屋の様子の差分+移管) — v0.3.9 主案件 | 裁定は確定済み (watermarks issue の議論節が正史) で、本番実害の実測 (のろけゆきさんの 18 万字) も issue に記録済み。次 = 門の再配線から実装に入る。 | 私 (実装) | [issue](../issues/persona_recall_perception_unbounded.md) / [watermarks issue](../issues/watermarks_unsatisfiable_when_perception_is_large.md) | 2026-09-05 |
...
| 🟠 実装中 | 知覚が多いと「残す量」と「整理を始める量」を両方満たせない状況への対応と、既定値の見直し | 水位側の裁定 (低水位廃止・既定値・クランプ・スペル結果の勘定外し・内訳三分割) は v0.3.8 として配布済みで、供給側の修理は v0.3.9 主案件の行へ分離した。次 = 残る知覚の二水位上限 + 省略の印 + 保存時検査を、供給側の修理の後に実装する。 | 私 (実装) | [issue](../issues/watermarks_unsatisfiable_when_perception_is_large.md) / [metabolism.md](../concepts/metabolism.md) | 2026-09-05 |
