...
| 🟣 検証待ち | Chronicle の被覆の穴 — 詰まらない統合と確定を装わない記録 | 第一段の実装・レビュー消し込み・フルスイート合格まで完了し PR 作成済み。次 = Codex 三巡目が完走しなかったため四巡目の要否をまはーが判断 → PR 確認 → 報告者への配布と実機再測定。 | まはー (レビュー裁定と PR 確認) | [intent](../intent/chronicle_coverage_gaps.md) | 2026-09-08 |
| 🟣 検証待ち | スルースの被覆の穴 — 会話を止めない採取と後から拾える記録 | 第一段 (飛ばしと記録・安全装置・読み戻し規則・採取ジョブ二本立て・手帳の二つの時刻) と隔離検証 12 本まで完了、フルスイート合格。次 = レビューの範囲をまはーが裁定 → 消し込み → PR #287 の確認 → v0.3.11 発行。 | まはー (レビュー裁定) | [intent](../intent/sluice_coverage_gaps.md) | 2026-09-09 |
| 🟣 検証待ち | Windows の psutil 無し環境で UI セルフアップデートが必ず失敗する (v0.3.11 主案件) | 実装・レビュー収束・フルスイート合格まで完了。次 = PR のまはー確認 → develop へマージ → v0.3.11 の発行。 | まはー (PR 確認) | [issue](../issues/self_update_unsafe_without_psutil.md) | 2026-09-08 |
