...
|---|---|---|---|---|---|
| 🟠 実装中 | 部屋 ID に「/」が入った部屋の付け替え (過去ログが移らず毎起動で警告) | 設計はまはーの GO 済みで、付け替える場所・途中で止まったときの再開・確認と取り込みの探し方の一本化を issue に確定した。次は実装を委譲し、隔離環境で N さんの形の部屋が付け替え→取り込み→警告なしになるまで出力を読んで確かめる。 | 私 (実装と検証) | [issue](../issues/building_id_contains_path_separator.md) | 2026-09-11 |
