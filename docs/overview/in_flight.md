...
| 🟣 検証待ち | v0.3.0 の門 (リリース範囲と実施順序) | 実機検証で出た欠陥 2 件 (機構 1 の退場バイパス / スルースの数字ループ) は両方直してコミット済み。次 = まはーが再起動してスルースの実機検証 (Memory 窓 → Chronicle → 生成。送信量の横棒が残す量を超えていること)、ログの `[sluice]` と手帳タブで結果を見る。並行して、数値欄を持つ他の構造化出力 4 Playbook (建物作成 / 文書検索 / 予定管理 / Web 調査) を同じ手順で隔離実験してから直す。飛ばされた範囲の採取し直しは別途手順を決める。 | まはー (実機検証 / 採取し直しの手順) / 私 (4 Playbook の実験) | [v030_release_gate.md](v030_release_gate.md) / [裁定ハンドオフ](../handoff/2026-08-22_v3_shape_layer_rulings_handoff.md) | 2026-08-22 |
