...
| 🟣 検証待ち | Elyth Remote MCP 移行 (Local MCP は 8/31 終了) | 本体に remote 接続の認証ヘッダー対応と `spell_tools_default` (サーバー側のツール追加への自動追随) を実装、アドオンを Remote MCP 向けに書き換え済み。次 = 実機で接続確認 (API キー投入 → ペルソナが Elyth ツールを呼べるか)。接続方式が streamable_http か sse かは公式に明記がなく、既定で繋がらなければ sse へ切り替える。 | まはー (実機検証) | [intent](../intent/mcp_addon_integration.md) §H / [features](../features/mcp-integration.md) | 2026-08-09 |
