...
| 🟡 実装待ち | 会話の終わり方 (出口) と応答のやり直し | 設計確定 — 出口 7 通り / 片付けは未読なら差し戻し・既読なら残す固定 / 操作はやり直しと送信の 2 つ (retryable は取り下げ)。やり直しは門の内へ引き上げ済み。形は確定 — 追加の推論は必ずユーザーの一押しの後ろに置き、ユーザーのバブルに再送・ペルソナのバブルに続きの生成、失敗はダイアログで通知。途中終了は部分文の保存とフラグで処理を終える。次 = 既存の 504 自動継続を同じ形へ揃えるか (揃えると救済機構が丸ごと減る) の裁定を受けて実装。 | まはー (裁定) → 私 (実装) | [issue](../issues/user_utterance_path_failure_inventory.md) / [門 §2-17](v030_release_gate.md) | 2026-08-25 |
