...
| 🔵 設計中 | 依存関係の管理 (lock ファイルと上げる/止めるの裁定簿) | intent を起草済み。次 = §6 の未決 3 点 (lock を作る道具 = uv 案 / SDK 更新の順番 / google-genai の上限を外すか) のまはー裁定を受けて、lock 導入と 7 経路の切り替えから実装に入る。 | まはー(§6 裁定) | [dependency_management.md](../intent/dependency_management.md) | 2026-09-02 |
