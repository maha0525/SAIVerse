...
| 🔵 設計中 | 想起用タグ + Track の役割縮小 | 想起の読み書きの確定形は v3 §12〜§13 が持つ形になった (書き手 = 機構コール相乗り、読み側 = 糸の台帳と知覚の差し込み)。次 = v3 の文面レビュー通過後、本 intent の §9 以降を v3 参照で改稿し、Track 縮小部分だけを残して整理する。 | まはー (v3 文面レビュー) | [recall_tags_and_track_reduction.md](../intent/persona_cognition/recall_tags_and_track_reduction.md) | 2026-08-16 |
...
| 🟠 実装中 | コードレビュー運用・残存finding修正 | W10 (柱8) 実装済み・Codex レビュー 1 巡完了 — 判定 No-ship (high 1 = schedule の事前承認が届かなくなる確認済み回帰 + medium 4、全件受諾・裁定はハンドオフに記入済み)。次 = Opus セッションでハンドオフを入口に消し込みループ → 実機検証。A3 は着手裁定待ち。 | 私(Opus セッションで消し込み) / まはー(A3 裁定 + 消し込み後の実機検証) | [完了計画書](audit_remediation_plan.md) / [W10 走行メモ](../handoff/2026-08-16_w10_spell_audit_remnants_handoff.md) | 2026-08-16 |
...
| 🟣 検証待ち | 自律 Pulse の器統合 + tell スペル (会話 Track の特殊機構剥がし) | 消し込みは終了済み (まはー判断による打ち切り。キャッシュ延命の論点は門 §2-1 = モデル格再設計が引き取った)。次 = 実機検証 (統合検証手順に相乗り)。 | まはー (実機検証) | [issue](../issues/slot_light_pulse_runs_on_conversation_vehicle.md) / [intent](../intent/autonomous_pulse_vehicle.md) | 2026-08-09 |
...
| 🟢 実装中 | v0.3.0 の門 (リリース範囲と実施順序) | 想起まわりの設計が確定した形 — 糸の台帳・捕獲の砂金取り統合・会話終了判断の退役 (v3 §13、v0.3/v0.4 割り振り込み)。次 = v3 §9 の残る未決 (Beat 語の二義・移行設計ほか) と連続性グラフを詰め、机上完動の判定をまはーと行って v0.3 実装 (形の層) へ。 | まはー (設計議論の続き) | [v030_release_gate.md](v030_release_gate.md) / [autonomous_behavior_v3.md](../intent/autonomous_behavior_v3.md) | 2026-08-16 |
| 🟠 実装中 | Track 撤廃 (最後の住人たちの引っ越し) | 順序① の実装とレビュー 3 巡ぶんの消し込みが揃い、回帰は緑。残る指摘は刺激の永続 ID が無いと直せないものだけで、右の別案件が引き取っている。次 = フルスイートを通して、まはーの実機検証へ渡す。 | まはー (実機検証) | [handoff](../handoff/2026-08-14_track_retirement_step1_review_handoff.md) / [intent](../intent/track_retirement.md) §7 | 2026-08-14 |
...
| 🟠 実装中 | 過去ログ (log.json) 取り込みの自動化と検算 | 三本柱 (アップグレード自動実行 dev5 / スキップ判定は現物の健全性だけ / 毎起動の検算バナー) を実装、テスト緑。Codex 1 巡は No-ship (high 3 + medium 2、裁定はハンドオフに記入済み)。次 = Opus セッションでハンドオフを入口に消し込み → 再レビュー → まはーの実機検証 (消し込み前の backend 再起動は非推奨)。 | 私 (Opus で消し込み) → まはー (実機検証) | [handoff](../handoff/2026-08-16_legacy_log_import_prevention_handoff.md) / [intent](../intent/building_memory_unified.md) | 2026-08-16 |
...
| 🟣 検証待ち | Elyth Remote MCP 移行 (Local MCP は 8/31 終了) | §I 再設計の実装と、レビュー 2 巡ぶんの消し込み (関所を接続直前へ / 両 Pulse root で取得 / timeout を fail-closed / 畳む理由に応じた片付け範囲) まで完了。レビュー 3 巡で指摘の的が「私の契約」→「片付けの範囲」→「遷移中の競合」と外周へ移り、最後は既存の接続ライフサイクル案件に接続した。次 = まはーの実機検証 (手順はハンドオフ §10 に操作順で用意済み)。4 巡目を回すかはまはーの裁定待ち。 | 私 (3 巡目) → まはー (実機検証) | [handoff](../handoff/2026-08-10_elyth_remote_mcp_handoff.md) / [intent](../intent/mcp_addon_integration.md) §I | 2026-08-10 |
