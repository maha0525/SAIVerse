...
|---|---|---|---|---|---|
| 🔵 設計中 | 知覚が多いと「残す量」と「整理を始める量」を両方満たせない状況への対応と、既定値の見直し | 議論で範囲が知覚の供給側 (想起の門・スペル通知・部屋の様子の差分化) まで広がり、方向は issue の議論節に整理済み。次 = 要約段の採否・二水位の細部・既定値の数字をまはーが裁定したら 0.3.8 で実装する。 | まはー (裁定) | [issue](../issues/watermarks_unsatisfiable_when_perception_is_large.md) / [metabolism.md](../concepts/metabolism.md) | 2026-09-04 |
