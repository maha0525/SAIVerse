...
| 🟢 実装中 | v0.3.0 の門 (リリース範囲と実施順序) | Wave 1 第三夜まで完了。arasuji_levels との統合が裁定され、正式語彙 (Episode chunk / connection) と三概念の定義が確定した。次 = 枠投下の補完文面 2 件のレビューと episode.md v1.0 改稿 (arasuji_levels 改訂込み) で Wave 1 を締め、Wave 2 (モデル格と保温) を開く。 | まはー (枠文面) / 私 (v1.0 改稿) | [v030_release_gate.md](v030_release_gate.md) / [track_retirement.md](../intent/track_retirement.md) | 2026-08-11 |
| 🟠 実装中 | Track 撤廃 (最後の住人たちの引っ越し) | 裁定 B②③ (閾値ポーラ・Handler tick 拡張点) は機構ごと撤去済みで、alert の生きている発火元はユーザー発話の一本のみ。①(判断点への直結) はまはー裁定により据え置き。次 = 撤去順序の先頭「判断点・meta_layer の Track 状態依存の付け替え」の範囲を詰めてまはーへ提示。 | 私 (次の工事の範囲詰め) → まはー (裁定) | [track_retirement.md](../intent/track_retirement.md) | 2026-08-11 |
