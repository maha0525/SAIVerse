...
| 🟠 実装中 | スルースの被覆の穴 — 会話を止めない採取と後から拾える記録 | intent 確定 (未決 4 点は同日まはー裁可済み、経緯は intent の追加の決定節)。次 = 第一段 (飛ばし判定・範囲の記録・429 誤分類除去と連続失敗停止・読み戻し規則) の実装と設計書同時改訂 → 検証。 | メティス (実装) | [intent](../intent/sluice_coverage_gaps.md) | 2026-09-08 |
