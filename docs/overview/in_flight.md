...
| 🟣 検証待ち | 編纂が同名ページを増やす輪の根治 | 実装・レビュー5巡の消し込み・テスト完了 (コミット未実施)。次 = 差分の確認とコミット可否。 | まはー (差分確認・コミット可否) | [issue](../issues/curation_duplicate_pages_loop.md) / [handoff](../handoff/2026-08-05_curation_loop_and_fragment_shift.md) | 2026-08-05 |
| 🟠 実装中 | Memopedia の本文を Fragment へ寄せる | 判定三段・下見/実行/取り消し・デバッグタブ UI まで実装、Codex レビュー八巡を消し込み済み。後始末の機構は撤去し「抜くときに空行も一緒に抜く」形へ。次 = レビュー再開と収束確認 → 実機。編纂再開には別途 (a)(b) が要る (intent §7)。 | 私 (レビュー再開) | [memopedia_body_to_fragment.md](../intent/memopedia_body_to_fragment.md) / [handoff](../handoff/2026-08-05_curation_loop_and_fragment_shift.md) | 2026-08-06 |
