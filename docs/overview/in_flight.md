...
| 🟡 実装待ち | 会話の終わり方 (出口) と応答のやり直し | 設計確定 — 出口 7 通り / 片付けは未読なら差し戻し・既読なら残す固定 / 操作はやり直しと送信の 2 つ (retryable は取り下げ)。やり直しは門の内へ引き上げ済み。束 1〜4 すべて実装済み (中断の印 / 無言の一箇所検査 / 続き・再送・取り消しの口 / バブルのボタン 3 種 / 出口 7)。フルスイート緑、フロントは型検査とビルドが通る。残 = 空 placeholder の掃除 (最ホットパスの構造変更のため切り出し)。次 = まはーの実機検証。 | まはー (実機検証) | [issue](../issues/user_utterance_path_failure_inventory.md) / [門 §2-17](v030_release_gate.md) | 2026-08-25 |
