...
|---|---|---|---|---|---|
| 🟠 実装中 | v0.4 自律運転の再開 (止め具の撤去と運転の配線) | 止め具の撤去 (定数削除・判定の復元・手動モード死に枝の掃除・凍結 doc 8 本の解除) を develop-v0.4 で実装し、関連テスト緑。次 = 隔離環境で判断点・見張り・コマ予約が発火するスモーク → v2 実装と v3 のティック設計の突き合わせ。 | 私 (スモーク) | [autonomous_behavior_v3.md §11](../intent/autonomous_behavior_v3.md) | 2026-09-25 |
| 🟣 検証待ち | 凍結解除した自律運転まわりの実機検証 (life / 時間割 / v2 配線 / 判断点 issue 2 件 / コマの器 2 件) | 凍結していた 6 件は develop-v0.4 の封印解除で検証可能になった (凍結の経緯は各 doc)。次 = 上の運転スモークと v3 突き合わせの決着後、各件を個別の行に開いて検証を再開する。 | 私 (運転スモーク待ち) | [life.md](../intent/life.md) / [timetable_redesign.md](../intent/timetable_redesign.md) ほか各 doc | 2026-09-25 |
