...
| 🔵 設計中 | 退場の境目とスペル群の原子性 | 懸念の並び (唱えだけ残り結果が消える) は構造的に生じないと確認済みで、逆向き (結果だけ窓に残る) を防ぐ保証が未検分。次 = 整列計画の境目の決まり方を読み、不変条件「スペルの群は退場の境目で泣き別れしない」の成立を検分する。守られていなければ整列側に群の原子性を入れる。 | 私 (検分) | [issue](../issues/eviction_boundary_spell_group_atomicity.md) | 2026-08-25 |
| 🔵 設計中 | ユーザー発話から応答完了までの停止点の棚卸し | 経路 7 段の停止点と「本人に何が見えるか」を対にした洗い出しを起票済み。型は 4 つ (内側の握り潰しが外側の網を無効化 / 契約の違う形を流す / 応答が生まれないことを表す出口が無い / 文言が事実と違う)。次 = 洗い出しのまはーレビューで、直す順番と 3 番目の出口の要否を決める。 | まはー (レビュー) | [issue](../issues/user_utterance_path_failure_inventory.md) | 2026-08-25 |
...
| 🟣 検証待ち | Elyth Remote MCP 移行 (Local MCP は 8/31 終了) | 手順 1〜7 まで通り、鍵の削除・入れ直しが実機で確認できた。途中で出た 3 つの欠陥 (鍵を消す口が無い / 不明スペルで全列挙 / 再接続の空振りが画面に出ない) は修正済み。次 = バックエンドを再起動し、再接続ボタンの通知表示を見てから手順 8〜11 を通す。Codex 4 巡目を回すかはまはーの裁定待ち。 | まはー (実機検証) | [handoff](../handoff/2026-08-10_elyth_remote_mcp_handoff.md) / [intent](../intent/mcp_addon_integration.md) §I | 2026-08-10 |
