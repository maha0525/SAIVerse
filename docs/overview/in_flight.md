...
| 🔵 設計中 | 知覚バッファ (Perception Buffer) | v0.1 draft 起草済。核=「主観時間は Pulse でのみ進む／知覚は Pulse 消費時のみ／未消費の間だけ型別 reduce」。差分通知・メタ記憶訂正・入室想起の 3 経路を一本化する横断基盤。次: draft レビュー → 未決A〜E(バッファ粒度/auto_ingest/再起動耐性/検知方式/プレビューUI)を詰める → Phase 1(器+メタ記憶訂正載せ替え) | まはー(レビュー) → 私 | [perception_buffer.md](../intent/perception_buffer.md) | 2026-07-09 |
