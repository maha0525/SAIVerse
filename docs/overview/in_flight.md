...
| 🟡 実装待ち | 会話の終わり方 (出口) と応答のやり直し | 設計確定 — 出口 7 通り / 片付けは未読なら差し戻し・既読なら残す固定 / 操作はやり直しと送信の 2 つ (retryable は取り下げ)。やり直しは門の内へ引き上げ済み。実装前の確認 3 点も決着 — やり直しは既読でも文脈に載り、連打は排他で二重応答にならない。次 = 中断された partial を残すかの裁定 (やり直しは「捨てて作り直す」なので残さないが私の見立て) を受けて実装に入る。 | まはー (裁定) → 私 (実装) | [issue](../issues/user_utterance_path_failure_inventory.md) / [門 §2-17](v030_release_gate.md) | 2026-08-25 |
