...
| 🟠 実装中 | 読み戻しがあらすじの途中まで遡ろうとして一歩も動かない | 設計は確定 (場所を問わずいちばん新しいあらすじから丸ごと開き、会話文が目標量に達したら終了。予算・各停止条件・上限の外し検算は撤去)。実装はサブエージェントに委譲中で、次 = 検収してテストとレビューを回す。 | 私 (実装と検収) | [issue](../issues/refill_reads_by_budget_instead_of_arasuji_unit.md) / [arasuji_levels.md](../intent/arasuji_levels.md) §15 | 2026-09-05 |
