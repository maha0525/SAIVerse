...
| 🟣 検証待ち | v0.3.0 の門 (リリース範囲と実施順序) | 形の層は全束コミット済みで、修正 4 本・Codex 横断走査の消し込み・手帳の読み口 (メモリタブの「手帳」節 + 読み取り API 2 本) に加え、本人が開く／書くスペル 2 本まで入り、フルスイートは緑。次 = まはーの実機検証 (起動前に `import_all_playbooks.py` が必須。見る点は全束完了ハンドオフのチェックリスト + 掃討ハンドオフ §3-2、加えて手帳タブの表示とスペル 2 本)。Codex の追加巡は回さない。 | まはー (実機検証) | [v030_release_gate.md](v030_release_gate.md) / [裁定ハンドオフ](../handoff/2026-08-22_v3_shape_layer_rulings_handoff.md) | 2026-08-22 |
