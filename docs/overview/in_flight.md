...
| 🟣 検証待ち | 知覚の堆積対策 7 件 (再会の門 / スペルのスキーマ同梱 / 部屋の差分+移管 / 合計の二水位上限 / 省略の印 / 保存時検査 / head との全文重複の根治) — v0.3.9 主案件 | 再設計はコミット済み。実機初験で再会の門の穴 2 つ (ユーザーの形の素通し・同席の実形との不一致) が見つかり修正済み。次 = 実機検証の続き (再会の抑止が効くか・部屋が戻るか) と数字 3 点の裁定。 | まはー (実機検証と数字の裁定) | [issue](../issues/persona_recall_perception_unbounded.md) / [head 重複 issue](../issues/room_state_duplicates_head_inventory.md) | 2026-09-06 |
| 🟠 実装中 | 部屋の様子のパッケージ (文字列解析の退役 + 一元化 + 実機検証の直し) — v0.3.9 出荷停止の原因 | 実機検証で見つかった 8 件を全部直し、主要な旅は本番で通過 (8 往復が経路一行に畳まれ、Close→Open も裁定どおりの形)。次 = 指示と様子を組成の末尾へ寄せる裁定の確定 (出来事が無い入室でも同じ規則にするか) と、その実装。 | まはー (並びの裁定) | [引き継ぎ](../handoff/2026-09-06_room_state_live_verification.md) / [intent](../intent/room_state_packages.md) | 2026-09-06 |
