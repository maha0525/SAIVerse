...
| 🟣 検証待ち | v0.3.0 の門 (リリース範囲と実施順序) | 数値欄 22 欄は危ない並び 5 型を隔離実験で短文・長文あわせて 90 回叩き、数字ループの再現なし。次 = まはーがスルースの実機検証 (Memory 窓 → Chronicle → 生成。送信量の横棒が残す量を超えていること) と、数値欄を現状維持とするかの裁定。飛ばされた範囲の採取し直しは別途手順を決める。 | まはー (実機検証 / 数値欄の裁定 / 採取し直しの手順) | [v030_release_gate.md](v030_release_gate.md) / [裁定ハンドオフ](../handoff/2026-08-22_v3_shape_layer_rulings_handoff.md) | 2026-08-24 |
