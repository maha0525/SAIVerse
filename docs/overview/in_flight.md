...
|---|---|---|---|---|---|
| 🟣 検証待ち | 組み込みモデルの棚卸し (提供終了した定義の削除・後継の追加・起動時警告の対称化) | 削除・追加・チュートリアルの付け替え・起動時警告は実装済みで、テストも通っている。次は Codex レビューを通し、まはーが Kimi K2.6 と NIM 設定の実用性を裁定したうえで、実機で起動時警告とチュートリアルの自動設定を確かめる。 | 私 (レビュー) → まはー (裁定・実機確認) | [release_history.md](release_history.md) の v0.3.12 (5) | 2026-09-11 |
