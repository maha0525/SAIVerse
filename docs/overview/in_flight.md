...
| 🟣 検証待ち | コンテキスト勘定が差し込み行を数えていない | 水位を測る全経路 (発火・§15 読み戻し・退場計画・表示に加え、手動整理の門・非常畳み・先回り畳み・印戻し・被覆補修の見積もり) を「保存行 + 送信直前に差し込まれる知覚」の合計へ揃え、ブロックの組成を一点化した。回帰緑。次 = 実機 (エリス) で表示の合計が実送信と一致し、整理が実際に発火するかの確認。 | まはー (実機検証) | [issue](../issues/context_accounting_excludes_injected_rows.md) | 2026-09-02 |
