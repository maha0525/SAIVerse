...
|---|---|---|---|---|---|
| 🔵 設計中 | 送る量の一系統化と会話以外の内容の節約 (v0.3.12 本丸) | intent 初稿が文書レビュー (言葉づかい・整合性) 済みで、まはーのレビュー待ち。次はレビューで未裁定 6 点 (既定値・部屋の痕跡と画像・下ろす印・既存設定の移行・機構行の上限・複数モデルのタイミング) を裁定して実装に入る。 | まはー (初稿レビュー) | [presented_context_reduction.md](../intent/presented_context_reduction.md) | 2026-09-09 |
