...
| 🔵 設計中 | 部屋の様子の差分の再設計 (文字列解析 → 包み単位) — v0.3.9 出荷停止の原因 | 知覚に現れる物の性質の洗い出しは issue に記録済み (Open/Close の意味・思い出の時間依存・消え方の多義など)。次 = 設計の問い 6 点をまはーと詰め、一元化 issue と一緒に設計するかも決める。 | まはー (設計議論) | [issue](../issues/room_state_diff_built_on_string_parsing.md) / [一元化 issue](../issues/room_state_single_source_of_truth.md) | 2026-09-06 |
