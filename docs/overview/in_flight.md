...
|---|---|---|---|---|---|
| 🟣 検証待ち | 組み込みモデルの棚卸し (提供終了した定義の削除・後継の追加・モデル設定の警告) | 削除・追加・警告・NIM の構造化出力の修正とチュートリアルの付け替えまで実装済み。次は再レビューとテスト全体を通してから、まはーが実機で警告とチュートリアルの自動設定を確かめる。NIM の標準モデルを Muse Glimmer 30B に寄せるかは、まはーの裁定待ち。 | 私 (再レビュー) → まはー (裁定・実機確認) | [release_history.md](release_history.md) の v0.3.12 (5) | 2026-09-11 |
