...
| 🟠 実装中 | 依存関係の管理 (lock ファイルと上げる/止めるの裁定簿) | lock の導入と 7 経路の切り替えは実装済み (契約テストつき、素の venv でもスイート緑)。次 = SDK を一族ごとに上げる: Web の土台 → google-genai → anthropic → openai の順で、各一族を別コミットにし、実 API の一回確認は課金前にまはーへ承認を取る。 | 私(SDK 一族の更新) | [dependency_management.md](../intent/dependency_management.md) | 2026-09-02 |
