...
| 🟣 検証待ち | 読み戻しがあらすじの途中まで遡ろうとして一歩も動かない | 実機の読み戻しは会話 41,922 字 ≧ 目標 40,000 と測って不要判定に落ち、あらすじを開く動作はまだ一度も走らない。次 = 送信合計が上限を超えて編纂が走り、会話が目標を割った回の Pulse で、あらすじが丸ごと開くのを確認する。残す量を一時的に上げれば即座に試せる。 | まはー (実機検証) | [issue](../issues/refill_reads_by_budget_instead_of_arasuji_unit.md) / [arasuji_levels.md](../intent/arasuji_levels.md) §15 | 2026-09-05 |
