...
|---|---|---|---|---|---|
| 🟣 検証待ち | 組み込みモデルの棚卸し (提供終了した定義の削除・後継の追加・モデル設定の警告) | 削除・追加・警告・NIM の構造化出力の修正とチュートリアルの付け替えまで実装済み。三巡目の指摘を受けて、標準モデルの即時反映を取り下げて「再起動まで反映されない」を警告で知らせる形に直している。次はその検収とテスト全体を通し、まはーが実機で警告とチュートリアルの自動設定を確かめる。 | 私 (検収) → まはー (実機確認) | [release_history.md](release_history.md) の v0.3.12 (5) | 2026-09-11 |
| 🟣 検証待ち | 部屋 ID に「/」が入った部屋の付け替え (過去ログが移らず毎起動で警告) | 実装とレビューを終え、フルスイートと隔離環境での起動確認 (付け替え→取り込み 23 件→警告なし→会話が元の順→部屋の設定の保存) も通った。次は v0.3.12 の束に入れて出し、N さんの環境で警告が消えて古い会話が見えるかを確かめる。 | まはー (v0.3.12 の束の確認とリリース) | [issue](../issues/building_id_contains_path_separator.md) | 2026-09-11 |
| 🟣 検証待ち | 送る量の一系統化と会話以外の内容の節約 (v0.3.12 本丸) | 設計 1〜4 とプリセットの画面まで実装済みで、レビューは収束しフルスイートも緑。次は統合ブランチ hotfix/v0.3.12 (GPT-Image 2.5 と同居) でまはーが実機確認する — 見どころは縮みの実測・プリセット画面の保存・「少ない」での整理の頻度の体感。 | まはー (実機検証) | [presented_context_reduction.md](../intent/presented_context_reduction.md) | 2026-09-10 |
...
| 🟣 検証待ち | 知覚が多いと「残す量」と「整理を始める量」を両方満たせない状況への対応と、既定値の見直し | 知覚の二水位上限と保存時検査の余裕は廃止され、この問いは送る量の一系統化 (上の行) が引き取った。次 = 一系統化の実機検証に相乗りして、知覚が大きい環境で会話が畳まれすぎないことを確認する。 | まはー (実機検証) | [issue](../issues/watermarks_unsatisfiable_when_perception_is_large.md) / [presented_context_reduction.md](../intent/presented_context_reduction.md) | 2026-09-05 |
...
| 🟣 検証待ち | GPT-Image 2.5 対応 (v0.3.12 候補) | 画像生成ツールに flare / sunburst の 2 モデルと xhigh / max 品質を実装し、feature ブランチにコミットしてある。次 = まはーがコミット内容を確認し、実機で一度生成して動作を確かめる。 | まはー (内容確認・実機生成) | [handoff の種リスト](../handoff/2026-09-09_v0311_shipped_and_v0312_seeds.md) | 2026-09-09 |
