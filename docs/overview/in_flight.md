...
| 🟣 検証待ち | 読み戻しがあらすじの途中まで遡ろうとして一歩も動かない | 再実装・レビュー消し込み (Codex 四巡、収束はまはー裁定済み)・フルスイート緑まで完了。次 = まはーの実機で、エリスの止まっていた窓に読み戻しが入ることを確認する。 | まはー (実機検証) | [issue](../issues/refill_reads_by_budget_instead_of_arasuji_unit.md) / [arasuji_levels.md](../intent/arasuji_levels.md) §15 | 2026-09-05 |
