...
|---|---|---|---|---|---|
| 🟣 検証待ち | 組み込みモデルの棚卸し (提供終了した定義の削除・後継の追加・モデル設定の警告) | 実装・レビューの消し込み・テスト全体まで済み (自律行動のテスト 1 件は並列実行のときだけ落ちる既存の不安定さで、単独では通る)。次はまはーが実機で、チュートリアルの自動設定 (OpenRouter 無料・NIM・Grok) と、定義の無いモデルを選んだときの警告が画面に出るかを確かめる。 | まはー (実機確認) | [release_history.md](release_history.md) の v0.3.12 (5) | 2026-09-11 |
