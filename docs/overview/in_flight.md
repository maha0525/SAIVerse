...
| 🟠 実装中 | 部屋の様子のパッケージ (文字列解析の退役 + 一元化 + 実機所見第一弾) — v0.3.9 出荷停止の原因 | 実機確認で所見 5 点 + テリスの欠落が出た。第一弾 (未消費の回収 / 配送順の一本化 / 遺物の掃除 = intent §11) の設計に GO 済み。次 = §11 の実装 → レビュー → 実機の旅のやり直し。 | メティス (実装) | [intent](../intent/room_state_packages.md) / [発端 issue](../issues/room_state_diff_built_on_string_parsing.md) | 2026-09-07 |
