...
|---|---|---|---|---|---|
| 🟠 実装中 | 部屋のアイテムの表示個数の上限と片付けの道具 | 設計は整合性検査とまはーの裁定で全論点確定。次は feature/room-item-cap で実装 (上限と差分の抑止 / めくるスペルとページの共通部品 / Building 設定 / アイテム作成の一般経路と Bag) を走らせ、隔離環境で出力を読む検収まで進める。 | 私 (実装) | [room_item_display_cap.md](../intent/room_item_display_cap.md) | 2026-09-11 |
