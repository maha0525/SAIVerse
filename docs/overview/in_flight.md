...
| 🟣 検証待ち | 会話の終わり方 (出口) と応答のやり直し | 手順①〜④は合格 (④は途中本文の残存・続きの生成・中断の通告・記憶への転記まで実機で確認済)。次 = ⑤ 通信断、⑤-b 再送が発言を二度載せないか、⑥ ライトとダークの見た目。 | まはー (実機検証) | [issue](../issues/user_utterance_path_failure_inventory.md) / [引き継ぎ](../handoff/2026-08-27_stop_path_and_lost_utterances.md) | 2026-08-27 |
...
| 🔵 設計中 | 中断のタイミング軸の棚卸し (思考中の停止ほか) | 洗い出しは完了 — 生成の一生を七区間に割り、区間 × 原因 (停止/例外) の全数表と穴 5 件を issue に整理した。次 = まはーが穴の扱いを裁定する (特に穴 1: 思考中の停止に再開手段が無い / 穴 2: tool-streaming の停止が握り潰される)。 | まはー (裁定) | [issue](../issues/beat_interruption_timing_inventory.md) | 2026-08-27 |
