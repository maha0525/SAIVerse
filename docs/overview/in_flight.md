...
|---|---|---|---|---|---|
| 🟣 検証待ち | ペルソナが話すモデルの決め方と、設定を変えたときの反映 (再起動不要・無いモデルで代わりに動かさない) | 実装・検収・レビュー (手元のモデルと Codex 二巡) まで済み。次 = まはーの実機で、設定を変えたその場で反映されることと、切り替えられなかった人や保存を断った知らせの文面を確かめる。 | まはー (実機検証) | [intent](../intent/persona_model_selection.md) | 2026-09-12 |
