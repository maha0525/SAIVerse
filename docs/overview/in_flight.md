...
|---|---|---|---|---|---|
| 🟠 実装中 | ペルソナが話すモデルの決め方と、設定を変えたときの反映 (再起動不要・無いモデルで代わりに動かさない) | 設計は全件決まり、実装を委譲して進めている。次は検収と隔離環境での確認 (画面の文面まで読む) を済ませ、レビューを回してからまはーの実機確認に渡す。 | 私 | [intent](../intent/persona_model_selection.md) | 2026-09-12 |
