...
| 🟣 検証待ち | 未確定の下書き行で発言が消える (根治) | 停止経路は実機合格。実機検証中に「中断の通告が heard_by 空で誰の記憶にも配られない」欠陥が出て修正済み。次 = 再起動後にもう一度停止を踏み、通告が次の Pulse でペルソナの記憶へ配られるかの確認。LLM エラー時の残存確認は普段使いの中で。 | まはー (実機検証) | [issue](../issues/orphaned_streaming_placeholder_cleanup.md) / [引き継ぎ](../handoff/2026-08-27_stop_path_and_lost_utterances.md) | 2026-08-27 |
