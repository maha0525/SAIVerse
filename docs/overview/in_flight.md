...
| 🟣 検証待ち | 会話の終わり方 (出口) と応答のやり直し | ⑤の原因 (Next 中継が切断を伝えない) を隔離再現で確定し、読み手の心拍監視 (15 秒の沈黙で復旧分岐へ) を実装済み。次 = 実機で生成中にバックエンドを止め、15 秒後に「通信が途中で切れました」等が出ることの確認。その後 ⑤-b・⑥。 | まはー (実機検証) | [issue](../issues/user_utterance_path_failure_inventory.md) / [引き継ぎ](../handoff/2026-08-27_stop_path_and_lost_utterances.md) | 2026-08-27 |
| 🟣 検証待ち | 未確定の下書き行で発言が消える (根治) | 停止経路は画面・記録・記憶・知覚まで実機合格 (通告の heard_by 欠陥も修正して配達を確認済)。残り = LLM エラーで Beat が落ちた回に言いかけた本文が残ることの確認。狙って踏めないので、普段使いでエラーに当たった回に記憶と記録を見る。 | まはー (普段使いでの確認) | [issue](../issues/orphaned_streaming_placeholder_cleanup.md) / [引き継ぎ](../handoff/2026-08-27_stop_path_and_lost_utterances.md) | 2026-08-27 |
...
| 🟣 検証待ち | 中断のタイミング軸の棚卸し (思考中の停止ほか) | 穴 1 の改訂 (返事ゼロの停止に「再送」) と終了処理の後始末 (Ctrl+C で走行中の生成を停止ボタンと同じ形で締める) まで実装済み。次 = 実機で「思考中に停止 → 再送が出る」と「生成中に Ctrl+C → 途中までの本文と中断の印が残る」の確認。 | まはー (実機検証) | [issue](../issues/beat_interruption_timing_inventory.md) | 2026-08-28 |
