...
| 🟡 実装待ち | 会話の終わり方 (出口) と応答のやり直し | 設計確定 — 出口 7 通り / 片付けは未読なら差し戻し・既読なら残す固定 / 操作はやり直しと送信の 2 つ (retryable は取り下げ)。やり直しは門の内へ引き上げ済み。束 1〜3 を実装済み (中断の印 / 無言の一箇所検査 / 続きと再送の口 / バブルのボタンと出口 7)。フルスイート緑、フロントは型検査とビルドが通る。次 = 束 4 (未読なら入力欄へ差し戻し) の実装と、まはーの実機確認 (ボタンの見え方は中断を作らないと確認できない)。 | 私 (束 4) → まはー (実機) | [issue](../issues/user_utterance_path_failure_inventory.md) / [門 §2-17](v030_release_gate.md) | 2026-08-25 |
