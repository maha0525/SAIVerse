...
|---|---|---|---|---|---|
| 🟣 検証待ち | 組み込みモデルの棚卸し (提供終了した定義の削除・後継の追加・モデル設定の警告) | 削除・追加・警告・NIM の構造化出力の修正は実装済みで、Codex の一巡目の指摘も直した。次はまはーがチュートリアルの OpenRouter 無料設定と NIM 設定、Kimi K2.6 の扱いを裁定し、それを反映してから再レビューとテスト全体を通す。 | まはー (裁定) → 私 (反映・再レビュー) | [release_history.md](release_history.md) の v0.3.12 (5) | 2026-09-11 |
