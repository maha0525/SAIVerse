...
|---|---|---|---|---|---|
| 🟣 検証待ち | 部屋や持ち物の変化の知らせが、二回届く・再起動後に消えることがある | 同時に調べた回の二重、読み込まれていないモデルの二重、再起動後の撮り直しによる喪失を直し、保存失敗時の一回の重複は受け入れとして明記した。次 = まはーが develop 向けの PR を確認してマージする。 | まはー (PR の確認) | [issue](../issues/head_diff_notification_duplicate_delivery.md) | 2026-09-25 |
| 🟣 検証待ち | アーリーアクセス版の配布 (安定版と分けた二本目の配布の線 + グローバル設定の切り替え) | 仕組みと画面の実装・検収 (隔離環境のブラウザで両チャンネル・両テーマを確認)・敵対レビュー 4 巡 approve まで完了。次 = develop への PR をまはーが確認してマージし、v0.3.x の安定版として配る。画面からの復元 (第二段の残り) は別途。 | まはー (PR の確認) | [intent](../intent/early_access_release.md) | 2026-09-25 |
