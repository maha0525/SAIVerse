...
| 🔴 調査中 | 会話の終わり方 (出口) と応答のやり直し | ①〜④は合格。⑤は不合格 — サーバー停止時にブラウザが切断に気づかず、無言のスピナーのまま固まる (出口 7 の文言まで報せが届かない)。次 = 正常終了の顔の切断と Next 中継の伝播の二仮説をコードと再現で確定し、修正する。⑤-b・⑥は⑤の後。 | メティス (調査・修正) | [issue](../issues/user_utterance_path_failure_inventory.md) / [引き継ぎ](../handoff/2026-08-27_stop_path_and_lost_utterances.md) | 2026-08-27 |
...
| 🟣 検証待ち | 中断のタイミング軸の棚卸し (思考中の停止ほか) | 穴 3 (記憶の補填) と穴 1 (浮いた吹き出しの畳み) は実装済み・テスト緑。穴 2 は凍結記録済み。次 = 実機で「思考中に停止 → 吹き出しがその場で消える」の確認。穴 3 は狙って踏めないので普段使いの中で。 | まはー (実機検証) | [issue](../issues/beat_interruption_timing_inventory.md) | 2026-08-27 |
