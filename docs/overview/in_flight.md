...
|---|---|---|---|---|---|
| 🟣 検証待ち | 移動の瞬間に積む知覚通知の是正 (v0.3.10 主案件) | 実機検証で見つかった追加の欠陥 (もう居ない相手の再会想起が届く) も直し方 6 として実装済み — 想起の発火点を移動の瞬間から Pulse の頭の同席チェックへ移した。次 = まはーの実機検証 (部屋の往復でスペル対と「がいます」が出ないこと、去った相手の想起が届かないこと)。 | まはー (収束判定・実機検証) | [issue](../issues/perception_state_pushed_at_event_time.md) | 2026-09-07 |
