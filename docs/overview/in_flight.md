...
| 🟣 検証待ち | 会話の終わり方 (出口) と応答のやり直し | 手順①〜③は合格。④で三層の欠陥が出て修正済み (印が画面へ届かない / 本文が記憶に残らない / 停止のタイミングで発言が消える)。次 = バックエンド再起動のうえで④の再確認、続けて⑤・⑤-b・⑥。 | まはー (実機検証) | [issue](../issues/user_utterance_path_failure_inventory.md) / [引き継ぎ](../handoff/2026-08-27_stop_path_and_lost_utterances.md) | 2026-08-27 |
| 🟡 実装待ち | 未確定の下書き行で発言が消える (根治) | ユーザー停止の経路は塞いだが、Beat が例外で丸ごと落ちる場合が残る。3 ヶ月で 32 件の発言が失われていた (既存分はそのまま残す裁定)。次 = placeholder の発番から確定までを try/finally で囲む工事 — 約 300 行の熱い経路なので Fable セッションで。 | Claude (実装) | [issue](../issues/orphaned_streaming_placeholder_cleanup.md) / [引き継ぎ](../handoff/2026-08-27_stop_path_and_lost_utterances.md) | 2026-08-27 |
