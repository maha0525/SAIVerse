...
| 🟣 検証待ち | 会話の終わり方 (出口) と応答のやり直し | ⑤の心拍監視は実機合格 (生成中にバックエンドを止めて 15 秒後に「通信が途中で切れました」が表示)。再送の応答復帰も実機で確認済み。次 = ⑤-b の残確認 (再送した回の記録に発言が二重に載っていないか) と ⑥ (再送・続きの生成ボタンのライト/ダーク表示)。 | まはー (実機検証) | [issue](../issues/user_utterance_path_failure_inventory.md) / [引き継ぎ](../handoff/2026-08-27_stop_path_and_lost_utterances.md) | 2026-08-29 |
