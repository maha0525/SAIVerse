...
| 🟢 実装中 | v0.3.0 の門 (リリース範囲と実施順序) | 机上完動の判定が出て設計フェーズが完了した (まはー「骨格は充分」)。次 = v0.3 実装 (形の層: メッセージ刻印 / 手帳とスルース拡張 / タグ / thread_edges / タスク帳の器 / Track 撤廃 / 自律 UI 隠し) の実施順序を組んで着手する。 | 私 (実装) | [v030_release_gate.md](v030_release_gate.md) / [autonomous_behavior_v3.md](../intent/autonomous_behavior_v3.md) | 2026-08-19 |
