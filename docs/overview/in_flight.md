...
| 🟠 実装中 | コア記憶の訂正導線 + ごみ箱 (短期対応) | **DB層✅ / API層✅ / メモリタブUI✅**(削除は実機確認済)。**仮想センサーは [知覚バッファ](../intent/perception_buffer.md) の一利用者として恒久対応済み**(通知過多→応急停止→Phase 1a で push→Pulse消費集約に載せ替え、実機検証待ち)。残: 未確認バッジの置き場所を確定(ペルソナ固有アフォーダンス、住民/神モードUI と交差) | まはー(バッジ置き場) → 私 | [memory_architecture_v2.md](../intent/memory_architecture_v2.md) §5.1 | 2026-07-09 |
...
| 🟠 実装中 | 知覚バッファ (Perception Buffer) | 核=「主観時間は Pulse でのみ進む／知覚は Pulse 消費時のみ／未消費の間だけ型別 reduce／知覚イベントは起動力属性を持ち決定論で反応可否を決める」。差分通知・メタ記憶訂正・入室想起・**会話取り込み**を一本化する横断基盤。**Phase 1a 実装済み・実機検証待ち**(永続バッファの器 `sai_memory/perception_buffer.py`+Pulse消費 flush を run_meta_user に+メタ記憶訂正を push へ載せ替え、pytest 34件)。検証: コア記憶を edit/delete/restore→memory.db の perception_buffer に積まれる→会話(Pulse)で1メッセージにまとまり SAIMemory へ。残 Phase 1b: 起動力ディスパッチャ+「絶対反応する」フラグを§3.1全契機に付与[漏れ=沈黙リグレッション]。Phase 2: 状態差分/会話取り込みの経路統合+3直挿入撤去。方針確定済(粒度=per-persona・永続化・検知/消費分離〈§4.5〉・軽量LLM門番(SN)は将来のみ・恒久二重禁止) | まはー(実機検証) → 私(Phase 1b) | [perception_buffer.md](../intent/perception_buffer.md) | 2026-07-09 |
