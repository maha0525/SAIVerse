...
|---|---|---|---|---|---|
| 🟣 検証待ち | llama-server idle 停止の busy 判定修正 (処理中サーバー射殺の根治) | 実機合格 — 長時間生成が idle タイマーを跨いで完走し、完走後の idle 停止も正常発火 (経緯は intent へ収録)。次 = ②intent「受け入れた限界」8項のまはー裁定 ③止血の idle_timeout:0 を戻すかの判断。 | まはー (限界8項の裁定) | [llama_server_auto_launch.md](../intent/llama_server_auto_launch.md) | 2026-08-11 |
...
| 🟣 検証待ち | 編纂の対称化 = 読み戻し (arasuji_levels §15) | プレビュー面は実機緑 (厚い生ログ・二重表示なし)。機構の本番未発火の根因は未被覆領域 (§16 被覆補修で根治中)。次 = テスタロッサとの一言会話で実機初発火 (計画が立っている状態を確認済み)。aifi は §16 補修で refill_applied=true まで回復済みだが、二例目として見るのは豆粒後始末 (隣人吸収 issue) の再編纂後。 | まはー (テスタロッサと一言) | [issue](../issues/metabolism_refill_when_below_target.md) / [intent](../intent/arasuji_levels.md) §15 | 2026-07-30 |
| 🟡 実装待ち | 極小 run の隣人吸収 (豆粒編纂の捏造根治) | 設計確定 — 裁定 8 点を issue に収録済み (吸収先=後ろ / 材料 0.5U / 時系列に階層をたたみ込む連鎖再生成 / 再編纂経路のみ / 失敗は帯にステータス表示 / Fragment 付け替え / embeddings 道連れ / 印は冪等再実行)。次 = 実装 (Lv2+ 再生成関数の新設を含む) → 検証 → aifi の豆粒後始末 (削除→補修再実行)。 | 私 (実装) | [issue](../issues/arasuji_tiny_run_absorption.md) | 2026-08-31 |
...
| 🟣 検証待ち | v0.3.0 の門 (リリース範囲と実施順序) | アップグレード検証は段 3 (update.bat 経路) まで全段合格し、掃討チェックリストの消化が進行中。次 = 掃討の緑をまはーの報告で消し込み、リリース判定点の裁定をまはーと行う。リリース時の必須作業 = dev6→0.3.0 の空エッジ追加と、リリースノートへの「v0.2.x からは update.bat を 2 回」の明記。 | まはー (判定) | [v030_release_gate.md](v030_release_gate.md) / [アップグレード検証](../handoff/2026-08-29_v0229_upgrade_test.md) | 2026-08-25 |
