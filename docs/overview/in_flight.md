...
|---|---|---|---|---|---|
| 🔵 設計中 | v0.4 自律運転の再開 (止め具の撤去 → v3 の運転の実装計画) | 止め具の撤去はコミット済みで、隔離環境のスモークで起床判断の発火と台帳記録まで確認した。v2 実装と v3 設計の対応表も完成 (handoff)。次 = 対応表を土台に v0.4 実装計画の intent を起草し、まはーの決めが要る点を絞って設計議論に出す。 | 私 (実装計画の起草) | [対応表](../handoff/2026-09-25_v2_v3_reconciliation_survey.md) / [v3 intent](../intent/autonomous_behavior_v3.md) | 2026-09-25 |
| 🟣 検証待ち | 凍結解除した自律運転まわりの実機検証 (life / 時間割 / v2 配線 / 判断点 issue 2 件 / コマの器 2 件) | 凍結していた 6 件は封印解除で検証可能になったが、対応表によると検証対象の一部 (時間割・コマ・作業セッション系) は v3 で退役予定。次 = 実装計画の起草時に「検証する価値が残る件」を選別してから個別の行に開く。 | 私 (実装計画での選別) | [life.md](../intent/life.md) / [timetable_redesign.md](../intent/timetable_redesign.md) ほか各 doc | 2026-09-25 |
| 🟣 検証待ち | ペルソナに自分の持ち物と外見が届いていない (部屋の様子の再設計からの退行) | 持ち物と外見を画像つきの独立したメッセージとして文脈に戻し、増減の通知も戻して develop 向けの PR を出した。次 = マージ後にまはーがアイフィで、持ち物と外見が文脈に載ることと、持ち物の増減が通知で届くことを確かめる。 | まはー (PR の確認と実機確認) | [issue](../issues/inventory_and_appearance_dropped_from_context.md) | 2026-09-25 |
...
| 🟠 実装中 | 実行台帳 (execution ledger) | 段階移行 Phase 0〜5 が全段実装済み。スルース (Metabolism) 経路の実機検証は本番 (v0.3 系) 側で継続。判断点・時間割・配送の段の検証は、develop-v0.4 の運転スモークと一緒に行う (封印解除済み)。 | まはー(実機検証) | [execution_ledger.md](../intent/execution_ledger.md) | 2026-07-21 |
