...
| 🟣 検証待ち | 自動想起の Jev 選別層 (実験) | 実装・検収・実 API 通し・レビュー収束まで済み、実験ブランチにコミット済み。次 = まはーが実験ブランチで有効化して実機評価する (浮かぶ記憶の質・ノイズの減り・体感遅延。手順は intent の効果測定の節)。 | まはー (実機評価) | [auto_recall_jev_rerank.md](../intent/auto_recall_jev_rerank.md) | 2026-09-17 |
