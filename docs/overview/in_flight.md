...
| 🟠 実装中 | 被覆補修 (arasuji_levels §16) — 忘却の穴の根治 | 設計まはー承認済み (不変条件 = 編纂対象は窓の中か被覆済みのどちらか)。見回り (定期・U 単位・止め線 = 温かい anchor) + 窓誕生時の印 + 削除でカーソル巻き戻し。次 = 実装 (サブエージェント走行中) → 単体・隔離検証 → 本番一人目 aifi。 | 私 (実装) | [intent](../intent/arasuji_levels.md) §16 / [issue](../issues/window_refill_rarely_plans_for_starved_personas.md) | 2026-08-31 |
