...
| 🟣 検証待ち | 過去ログ (log.json) 取り込みの自動化と検算 | テスタロッサの部屋の取り込み漏れの再発防止として、取り込みのアップグレード自動実行 (dev5)・スキップ判定を現物の健全性だけに・毎起動の検算バナーを実装、テスト緑。次 = 実機検証 — backend 再起動で dev5 が走り、テスタロッサの部屋の全履歴が UI に戻るか、ナビの部屋の未取込警告バナーが出るかを見る。 | まはー (再起動 + 確認) | [intent](../intent/building_memory_unified.md) 「過去ログ取り込みの自動化と検算」節 | 2026-08-16 |
