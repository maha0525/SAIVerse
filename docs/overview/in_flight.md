...
| 🔵 設計中 | 想起用タグ + Track の役割縮小 | 書き手の席 (機構のあらすじコール相乗り = B2 維持) は確定、読み側は前提のコマ・episode が v3 で消え「暮らしの糸の再開」の照準で再設計中。次 = 本 intent 単独では動かず、v3 §12 の暮らしの設計の決着を受けて §9 を改稿する。 | まはー (v3 設計議論) | [recall_tags_and_track_reduction.md](../intent/persona_cognition/recall_tags_and_track_reduction.md) | 2026-08-15 |
...
| 🟢 実装中 | v0.3.0 の門 (リリース範囲と実施順序) | 設計フェーズの順序が判明した形 — タグのスキーマは暮らしの設計の下流で、照準は「暮らしの糸の再開」(v3 §12、タグの席と捕獲三席まで暫定合意)。次 = 小説の固定点を三日分データ粒度で流す検算からまはーと設計を続け、机上完動と線引きの緊張の解き方を見極めて v0.3 実装へ。 | まはー (設計議論の続き) | [v030_release_gate.md](v030_release_gate.md) / [autonomous_behavior_v3.md](../intent/autonomous_behavior_v3.md) | 2026-08-15 |
