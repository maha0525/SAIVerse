...
| 🟣 検証待ち | 通話モード (Gemini Live でペルソナと音声通話) | backend・frontend とも実装済みで、レビュー指摘の修正まで収束。次 = まはーの実機体感 (ペルソナメニューの「通話する」から)。実マイク→実音声の全経路と Live API の実挙動はこの体感が本検証。 | まはー (実機体感) | [voice_call.md](../intent/voice_call.md) | 2026-09-16 |
