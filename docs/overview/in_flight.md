...
|---|---|---|---|---|---|
| 🟣 検証待ち | 移動の瞬間に積む知覚通知の是正 (v0.3.10 主案件) | 直し方 8 まで実装済み — 配達中に来た配達依頼の引き継ぎと、プレビューへの「Pulse の頭で検知される分」の読み取り専用の合成を足した。次 = まはーの実機検証 (部屋の往復でスペル対と「がいます」が出ないこと、去った相手の想起が届かないこと、別ペルソナ宛ての通知が遅れないこと、退室がプレビューに映ること)。 | まはー (収束判定・実機検証) | [issue](../issues/perception_state_pushed_at_event_time.md) | 2026-09-07 |
| 🟣 検証待ち | 知覚の届く順が起きた順とずれる | 配達の遅れの根 (別ペルソナ宛ての配達依頼が誰にも引き継がれず回復処理待ちになること) を塞いだ。次 = まはーの実機で遅れが消えたことを確認し、残る並びのずれ (想起が移動の経路一行より上に出る) の対処方向を裁定する。 | まはー (実機確認・方向の裁定) | [issue](../issues/perception_arrival_order_differs_from_event_order.md) | 2026-09-07 |
...
| 🟣 検証待ち | v0.2 形式の記憶 DB で会話が毎回見送りになる (床の確立失敗) | develop へマージ済みで、v0.3.10 の発行待ち。次 = v0.3.10 を発行して報告者に配布し、最初の一言で見送りが出ないことを確認してもらう。 | まはー (v0.3.10 発行) → 外部 (報告者確認) | [issue](../issues/window_floor_unmet_on_v02_memory_db.md) | 2026-09-07 |
