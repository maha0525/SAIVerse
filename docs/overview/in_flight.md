...
| 🟣 検証待ち | Elyth Remote MCP 移行 (Local MCP は 8/31 終了) | 移行本体と §I 再設計 (discovery 廃止 → Pulse 頭接続 + Beat 頭一覧取得 + 知覚バッファ配送) の実装と、Codex 指摘 6 件の消し込み (placeholder 検査を connect 直前の単一関所へ / 作業セッションも Pulse 頭で取得 / timeout を fail-closed) まで完了。次 = 再レビュー 1 巡 → まはーの実機検証 (§6 + intent §I 検証の旅)。 | 私 (再レビュー) → まはー (実機検証) | [handoff](../handoff/2026-08-10_elyth_remote_mcp_handoff.md) / [intent](../intent/mcp_addon_integration.md) §I | 2026-08-10 |
