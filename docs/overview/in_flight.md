...
| 🟣 検証待ち | 中断のタイミング軸の棚卸し (思考中の停止ほか) | 穴 1 は改訂裁定 (返事ゼロの停止に「再送」を出す) まで実装済み。次 = 実機で「思考中に停止 → 吹き出しが消えて発言に再送が出る → 押すと応答が来る」の確認。穴 3 は狙って踏めないので普段使いの中で。 | まはー (実機検証) | [issue](../issues/beat_interruption_timing_inventory.md) | 2026-08-28 |
