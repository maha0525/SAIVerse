...
| 🟣 検証待ち | 知覚の堆積対策 7 件 (再会の門 / スペルのスキーマ同梱 / 部屋の差分+移管 / 合計の二水位上限 / 省略の印 / 保存時検査 / head との全文重複の根治) — v0.3.9 主案件 | 7 件目 (帰還時に head のアイテム一覧と知覚が全文二重になる欠陥の根治) も実機ツアーの発見から追加実装し、レビュー消し込みまで済み。次 = まはーの実機で、更新後に知覚の合計が下の水位まで自動で下りること・帰還時の重複が一行になることと、数字 3 点 (知覚の既定・余裕・オプトアウトの穴、issue に記録) の裁定。 | まはー (実機検証と数字の裁定) | [issue](../issues/persona_recall_perception_unbounded.md) / [head 重複 issue](../issues/room_state_duplicates_head_inventory.md) | 2026-09-05 |
