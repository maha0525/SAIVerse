...
|---|---|---|---|---|---|
| 🟣 検証待ち | Pulse の Beat が 1 件に結合され部屋の帰属と境界が失われる | 実装・検収・レビュー 3 巡の消し込みまで完了し、feature/room-item-cap にコミット済み。次はまはーの実機で、同席者ありの移動スペル入り Pulse を再現し、元の部屋に Beat 1 とリンク付き移動イベントだけが残ることを確認する。 | まはー (実機検証) | [issue](../issues/pulse_beats_merge_into_single_record.md) | 2026-09-13 |
| 🟣 検証待ち | 部屋のアイテムの表示個数の上限と片付けの道具 | 実装・隔離環境での出力読みの検収・レビュー三巡の消し込みまで済み。次はまはーの実機で、アイフィさんの部屋の縮みの実測と、めくるスペル・入れ物・作成ボタン・Building 設定を一通り確かめる。 | まはー (実機検証) | [room_item_display_cap.md](../intent/room_item_display_cap.md) | 2026-09-12 |
