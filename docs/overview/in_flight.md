...
| 🔵 設計中 | 自律 Pulse の器統合 + tell スペル (会話 Track の特殊機構剥がし) | 主要裁定は出た (器はセッション運転へ統合・暮らしコマは予算1締めなしプロファイル / tell は宛先明示の 1 Beat Playbook 起動で言葉は会話 aspect=標準モデルが書く / 席違いフォールバックは機械検査)。次 = issue を元に intent を起こして設計詳細 (プロファイルの持ち方・記録タグ・剥がしの段取り) を確定し、実装へ。 | 私 (intent 起こし) → まはー (レビュー) | [issue](../issues/slot_light_pulse_runs_on_conversation_vehicle.md) | 2026-08-08 |
