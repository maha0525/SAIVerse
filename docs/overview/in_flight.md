...
| 🟠 実装中 | 知覚消費点の Beat 頭化 + フィード入室配送 | 実装済み・レビュー 1 巡 (ローカル+Codex) 完了、指摘と裁定はハンドオフに記録済み (修正必須 2 件: flush の冪等化 / 配送の persona 単位ロック)。次 = Opus セッションが消し込み → 収束後フル 1 回 → コミット → まはーへ実機再開の合図。 | Opus (消し込み) | [handoff](../handoff/2026-08-08_beat_head_perception_handoff.md) / [perception_buffer.md](../intent/perception_buffer.md) §4.2 | 2026-08-08 |
