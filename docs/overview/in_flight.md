...
| 🟣 検証待ち | Elyth Remote MCP 移行 (Local MCP は 8/31 終了) | 移行本体と §I 再設計 (discovery 廃止 → Pulse 頭接続 + Beat 頭一覧取得 + 知覚バッファ配送) の実装・テスト・Codex 1 巡まで完了。次 = ハンドオフ §8 の high 5 件の消し込み (筆頭 = reconnect の placeholder 検査迂回、次 = work_session の取得迂回) → 再レビュー → まはーの実機検証 (§6 + intent §I 検証の旅)。 | 次セッション (消し込み) → まはー (実機検証) | [handoff](../handoff/2026-08-10_elyth_remote_mcp_handoff.md) / [intent](../intent/mcp_addon_integration.md) §I | 2026-08-10 |
