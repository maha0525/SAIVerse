...
| 🟣 検証待ち | 部屋の様子のパッケージ (文字列解析の退役 + 一元化 + 実機所見第一弾) — v0.3.9 出荷停止の原因 | §11 (回収・配送順・遺物掃除) の実装とレビュー三巡が収束し、コミット済み。次 = バックエンド再起動して実機の旅 (intent §10) + 所見 ①②④ の解消確認。テリスの欠落は別 issue で調査待ち。 | まはー (実機検証) | [intent](../intent/room_state_packages.md) / [発端 issue](../issues/room_state_diff_built_on_string_parsing.md) | 2026-09-07 |
