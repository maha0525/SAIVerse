...
| 🟢 実装中 | v0.3.0 の門 (リリース範囲と実施順序) | episode.md はレビューの往復で v1.3 に達し、最終形 (エピソードは行動の記録一種のみ・会話はメッセージへの記録からの導出・チャンクは arasuji_levels 準拠) が確定した。次 = まはーが v1.3 を通読レビューして Wave 1 を締め、Wave 2 (モデル格と保温、生ログ正史を含む) の設計議論を開く。 | まはー (v1.3 レビュー → Wave 2) | [v030_release_gate.md](v030_release_gate.md) / [episode.md](../intent/episode.md) | 2026-08-11 |
