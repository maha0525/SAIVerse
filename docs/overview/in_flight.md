...
| 🟣 検証待ち | 生成の段階の信号 (門 §2-18/19 + retry 門番) | 実装・検収・Codex 1 巡と消し込み (7 件) まで完了、フルスイート緑でコミット済み。次 = まはーの実機検証 — 通信を切って戻る回で「発言は届いています」の案内と再送導線が出るか、続きの生成のボタンが保存の実態と一致するか、既に応答が付いた発言の再送が正直に断られるかを見る。 | まはー (実機検証) | [§2-18](../issues/unknown_send_outcome_has_no_recovery_path.md) / [§2-19](../issues/stream_completion_is_not_proof_of_persistence.md) / [J](../issues/retry_api_has_no_server_side_eligibility_check.md) | 2026-08-29 |
| 🟣 検証待ち | 編纂材料の一本化 + U 判定の材料字数化 (門 §2-9) | 材料規則と U 判定 (圧縮後で測る) の実装・検収・Codex 各 1 巡の消し込みまで完了、フルスイート緑。次 = まはーの実機検証 — スペル入りの会話が畳まれた後のあらすじに痕跡が残ること、一次あらすじが材料 1 万字級で立つこと、確認窓の「あと約 N 文字」の文言が実態と合うことを見る。過去の通知行が編纂対象へ再入場する一回きりの移行ノイズが出たら観察して報告する。 | まはー (実機検証) | [arasuji_levels.md](../intent/arasuji_levels.md) §7-1/§7-8 / [v030_release_gate.md](v030_release_gate.md) §2-9 | 2026-08-29 |
