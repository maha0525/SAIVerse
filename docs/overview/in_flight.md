...
| 🟣 検証待ち | 記憶DBの用意が失敗すると接続が残りロック待ちになる | 用意の失敗で接続が閉じられず書き込みロックが残る欠陥を、同型 3 箇所とも塞いで回帰緑。引っ越しが倒れる理由自体は環境依存で未特定。次 = 診断ツール v1.2 を報告者に渡し、複製での引っ越し試運転の traceback で原因を確定する。 | まはー (報告者への配布) → 私 (原因の特定) | [issue](../issues/memory_db_connection_leak_on_init_failure.md) | 2026-09-02 |
...
| 🟣 検証待ち | 依存関係の管理 (lock ファイルと上げる/止めるの裁定簿) | lock 導入・7 経路の切り替え・SDK 四族 (Web / google-genai / anthropic / openai) と残りの下限化まで実装済みで、lock だけの venv でフルスイート緑。次 = ブランチ全体のレビュー (ローカル → Codex) のあと、まはーの本番 venv 入れ替えと再起動、実 API の一回確認 (課金前に承認を取る)。 | 私(レビュー) → まはー(venv 入れ替え・再起動・課金承認) | [dependency_management.md](../intent/dependency_management.md) | 2026-09-02 |
