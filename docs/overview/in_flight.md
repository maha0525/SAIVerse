...
| 🟠 実装中 | Track 撤廃 (最後の住人たちの引っ越し) | 撤去順序①の範囲詰め (全数調査) は完了 — 判断側依存は v1 メタ判断に閉じており、実体は「v1 の残り 3 入口の処理と退役」(intent §7)。次 = §7.3 の裁定 5 点 (①の読み替え / alert 直結の範囲 / social・life_purpose の受け皿 / スペル語彙の時期 / ④群の工程移動) をまはーが裁定。 | まはー (§7.3 裁定 5 点) | [track_retirement.md](../intent/track_retirement.md) §7 | 2026-08-13 |
