...
|---|---|---|---|---|---|
| 🟠 実装中 | アーリーアクセス版の配布 (安定版と分けた二本目の配布の線 + グローバル設定の切り替え) | 設計確定 (裁定 6 点は intent §6)。第一段 (切り替え・通知・発行の自動化・版の比較・鎖の検算) の実装を feature/early-access-channel で進行中。次 = 実装 → 検収 → レビュー → develop への PR。 | 私 (実装・検収) | [intent](../intent/early_access_release.md) | 2026-09-25 |
