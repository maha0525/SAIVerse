...
| 🟣 検証待ち | v0.3.1 ホットフィックス (リリース直後の実機指摘) | hotfix/v0.3.1 にアラーム UI 整理・スペル非表示・コア記憶のモバイル修理とタブ非表示を実装済み (Codex 三巡消し込み済み、全スイート緑)。次 = まはーの実機確認 → schedule_management の Playbook 取り込み → Bag アイテム作成経路を積むか判断 → develop へ PR。 | まはー (実機確認) | ブランチ hotfix/v0.3.1 | 2026-09-01 |
