...
| 🟣 検証待ち | 編纂材料の一本化 (門 §2-9 + 知覚付記の撤去) | 実装・検収・Codex 1 巡の消し込みまで完了、フルスイート緑でコミット済み。次 = まはーの実機検証 — スペル入りの会話が畳まれた後のあらすじに痕跡 (一行または全文) が残ること、記憶の整理が普通に通ることを見る。過去の通知行が編纂対象へ再入場する一回きりの移行ノイズが出たら観察して報告する。 | まはー (実機検証) | [arasuji_levels.md](../intent/arasuji_levels.md) §7-1/§7-8 / [v030_release_gate.md](v030_release_gate.md) §2-9 | 2026-08-29 |
