...
| 🟠 実装中 | v0.3.1 ホットフィックス (リリース直後の実機指摘) | ENABLE_MEMORY_WEAVE_CONTEXT 撤去・エラー文言・システムプロンプト棚卸し・Chronicle 帯予算の設定 UI・診断レポートの帯実寸まで実装済み。次 = まはーが実機で確認し、帯が膨らんでいるユーザーに診断レポートを再提出してもらって原因を確定 → Playbook 取り込みと Bag アイテム作成経路を判断 → develop へ PR。 | 私 (実装) | ブランチ hotfix/v0.3.1 | 2026-09-01 |
