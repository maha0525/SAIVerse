...
| 🟠 実装中 | 自律 Pulse の器統合 + tell スペル (会話 Track の特殊機構剥がし) | 消し込み済み・フルスイート緑。ただし器の統合で暮らしコマが軽量モデルになり、標準モデルのキャッシュを温めなくなっている (life.md §5.2 の前提が崩れた)。次 = この論点のまはー裁定 — 結論しだいで実装が変わりうるため実機検証はその後。 | まはー (キャッシュ延命の裁定) | [issue](../issues/slot_light_pulse_runs_on_conversation_vehicle.md) / [intent](../intent/autonomous_pulse_vehicle.md) | 2026-08-09 |
...
| 🔵 設計中 | v0.3.0 の門 (リリース範囲と実施順序) | 朝の裁定二便を反映済み (AUTONOMOUS 化は再裁定対象へ、オンボーディングは門の外)。次 = 残る裁定 3 点 (定時スケジュール×習慣テンプレートの統合 / user 発言帰属の方針 / episode.md 実装範囲) をまはーが裁く → 設計議論の一本目 (エピソードの単位) を開く。 | まはー (裁定) | [v030_release_gate.md](v030_release_gate.md) | 2026-08-09 |
