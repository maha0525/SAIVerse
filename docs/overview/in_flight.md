...
| 🟣 検証待ち | v0.3.0 の門 (リリース範囲と実施順序) | 数値欄は現状維持で裁定済み。スルースの実機初走行は採取まで成功したが、退場の関所が提示に載らない行を数えて永久保留になる欠陥が見つかり、関所の照合を提示に載る行へ限定する修正を実装中。次 = 修正の検収とレビューの後、まはーが「生成」を再実行して退場まで通ることを確認する。 | 私 (修正の検収) → まはー (実機再検証) | [v030_release_gate.md](v030_release_gate.md) / [関所 issue](../issues/metabolism_deferral_mislabeled_as_window_claim.md) | 2026-08-24 |
