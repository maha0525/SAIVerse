...
| 🟣 検証待ち | Elyth Remote MCP 移行 (Local MCP は 8/31 終了) | 手順 9 まで通過 (8 は確かめる相手のコマ発火が動いていないので v0.4 送り)。検証中に出た欠陥 4 件は修正済みで、直近はアドオン無効化が画面へ反映されない件。次 = バックエンドを再起動し、Elyth を OFF にして一覧から行がその場で消えるか、直後に話しかけて異常が出ないかを見る。Codex 4 巡目の要否はまはーの裁定待ち。 | まはー (実機検証) | [handoff](../handoff/2026-08-10_elyth_remote_mcp_handoff.md) / [intent](../intent/mcp_addon_integration.md) §I | 2026-08-10 |
