...
| 🟡 実装待ち | 会話の終わり方 (出口) と応答のやり直し | 設計確定 — 出口 7 通り / 片付けは未読なら差し戻し・既読なら残す固定 / 操作はやり直しと送信の 2 つ (retryable は取り下げ)。やり直しは門の内へ引き上げ済み。束 1 (中断の印) と束 2 (無言をなくす) を実装済み・フルスイート緑。無言は「結果を作る場所で一度だけ検査する」形にし、握り潰し 2 箇所も解除した。残 = 空 placeholder の掃除 (最ホットパスの構造変更になるため切り出し)。次 = 束 3 (バブルのボタン・ダイアログ・続き生成と再送の口) を実装する。 | 私 (実装) | [issue](../issues/user_utterance_path_failure_inventory.md) / [門 §2-17](v030_release_gate.md) | 2026-08-25 |
