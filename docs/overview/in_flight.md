...
| 🟠 実装中 | Track 撤廃 (最後の住人たちの引っ越し) | 順序① (v1 メタ判断の退役 + alert 直結化 + 「いまの活動」の出来事化) は実装と、レビュー指摘 5 件の消し込みまで揃っている。次 = 再レビューを回して収束を観測し、フルスイートを通してから、まはーの実機検証へ渡す。 | 私 (再レビュー) → まはー (実機検証) | [handoff](../handoff/2026-08-14_track_retirement_step1_review_handoff.md) / [intent](../intent/track_retirement.md) §7 | 2026-08-14 |
