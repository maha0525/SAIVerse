...
|---|---|---|---|---|---|
| 🟣 検証待ち | 部屋や持ち物の変化の知らせが、二回届く・再起動後に消えることがある | 同時に調べた回の二重、読み込まれていないモデルの二重、再起動後の撮り直しによる喪失を直した変更が develop に入っている。次 = まはーの実機で、入退室や持ち物の変化の知らせが一回だけ届くことと、再起動をはさんでも消えないことを確かめる。 | まはー (実機確認) | [issue](../issues/head_diff_notification_duplicate_delivery.md) | 2026-09-26 |
| 🟣 検証待ち | アーリーアクセス版の配布 (安定版と分けた二本目の配布の線 + グローバル設定の切り替え) | 第一段 (仕組みと画面) は develop に入っている。次 = まはーの実機で、グローバル設定からアーリーアクセスへ切り替えて更新が二本目の線から来ることを確かめ、次の版に載せる。画面からの復元 (第二段の残り) は別途。 | まはー (実機確認) | [intent](../intent/early_access_release.md) | 2026-09-25 |
| 🟣 検証待ち | スペルのあとで返事が止まったとき、最後の発言に「続きの生成」を出す (Gemini のブロックが理由なしで出る報告から派生) | 実装・レビュー消し込み・フルスイート緑・隔離環境のブラウザでの場面確認 (スペル後の失敗 → ボタンと案内 → 二言目) まで完了し、develop にコミット済み (未 push)。次 = まはーの実機確認とリリース判断。発端の 3 人のユーザーさんへの返信はリリース後。 | まはー (実機確認・リリース判断) | [intent](../intent/reply_stop_exit.md) / [handoff](../handoff/2026-09-25_spell_stop_continue_handoff.md) | 2026-09-25 |
| 🟣 検証待ち | ペルソナに自分の持ち物と外見が届いていない (部屋の様子の再設計からの退行) | 持ち物と外見を画像つきの独立したメッセージとして文脈に戻し、増減の通知も戻した変更が develop に入っている。次 = まはーがアイフィで、持ち物と外見が文脈に載ることと、持ち物の増減が通知で届くことを確かめる。 | まはー (実機確認) | [issue](../issues/inventory_and_appearance_dropped_from_context.md) | 2026-09-25 |
| 🟣 検証待ち | 右パネルの現在地が、部屋を移動しても 10 秒後に前の部屋へ戻る (パネルからの操作も前の部屋に効く) | 隔離環境で再現と解消の両方を確認済み。まはーの環境では元々再現しないため、実機確認は報告者に任せる。次 = リリース後に報告者へ更新を案内し、戻らないことと ＋ がいま見ている部屋を指すことの確認をもらう。 | 外部 (リリース後、報告者の確認) | [issue](../issues/right_sidebar_polling_holds_stale_building.md) | 2026-09-22 |
| 🟣 検証待ち | 移動ログが続くと、そのあいだの時刻が次の箱に覆われて読めない (報告者の環境では再現するが開発機では再現しない) | 行間を負の余白から正の余白へ書き換えて重なりが起こりえない形にし、間隔の不変は数値で確認済み。開発側では再現しないため確認は報告者に任せる。次 = リリース後に報告者へ更新を案内し、修正版の確認とあわせてブラウザとバージョンを訊く。 | 外部 (リリース後、報告者の確認と環境の聞き取り) | [issue](../issues/system_notice_time_hidden_by_next_notice.md) | 2026-09-22 |
| 🟣 検証待ち | NIM のモデルで構造化出力を使うと画像がモデルに届かず、画像の上限を定義に書くとクライアントが作れない | 実機での確認は NIM 側の不調 (5 分待たせて 504 が続く) で整理が完走せず棚上げ中。次は NIM が安定している時間帯に画像つきの会話で整理を回し、スルースの呼び出しに画像が入ることをログで確かめる。 | まはー (仕切り直し) / 外部 (NIM の復調) | [issue](../issues/nim_structured_output_drops_images.md) | 2026-09-22 |
| 🟣 検証待ち | 上限の小さいモデルに切り替えると記憶の整理から抜け出せない (スルースを飛ばす判定が、実際に送る量を見ていない) | 修正は develop に入っている。次はまはーがうるさんへ修正版を案内し、4o に切り替えて会話が成立するかを確かめてもらう。 | まはー (更新案内) → 外部 (うるさんの確認) | [issue](../issues/sluice_skip_ignores_model_context.md) | 2026-09-18 |
| 🟣 検証待ち | スペル不使用モード — 無効ペルソナのシステムプロンプトからスペル前提の説明を全て外し、切り替えは保存で即反映 | 実装・検収・レビュー収束済み (敵対 4 巡で approve)。次はまはーの実機検証: ペルソナを不使用モードに切り替え → 保存の直後にコンテキストプレビューからスペル・Playbook・アイテムの説明が消えていることと、トークン数の実測差を見る。 | まはー (実機検証) | [spell_disabled_mode.md](../intent/spell_disabled_mode.md) | 2026-09-14 |
| 🟣 検証待ち | 部屋 ID に「/」が入った部屋の付け替え (過去ログが移らず毎起動で警告) | v0.3.12 として発行済み。次は N さんに更新を案内し、N さんの環境で警告が消えて古い会話が見えることの確認をもらう。 | まはー (更新案内) → 外部 (N さんの確認) | [issue](../issues/building_id_contains_path_separator.md) | 2026-09-11 |
| 🔵 設計中 | 読み戻し実装セッション全体の監査と残存不具合 | 監査報告と合成データの再現例の確認待ち。次は印戻しの下限保護、head 読取失敗の扱い、モデル切替の表示整合の修正方針を確定する。 | まはー (監査結果と修正方針) | [監査報告](../issues/audit_20260730_review_guards.md) | 2026-09-06 |
...
| 🟠 実装中 | Godot / ARDY 仮想身体デモ | 身体・表情・一人称撮像に加え、ARDY を別 PC で生成する経路と身体スペルの Building 指定まで隔離 E2E 済み。次 = まはーが設定画面で生成先 URL と Building を選んで実機確認し、NEBULA の生成サーバーを常駐させるかを決める。残 = player locomotion、瞬き・視線、実 HMD、/emote、Idle scheduler。 | まはー(生成先・Building の実機確認、生成サーバー常駐の可否) / 私(残りの実装) | [virtual_embodiment_godot.md](../intent/virtual_embodiment_godot.md) | 2026-09-23 |
| 🟠 実装中 | コードレビュー運用・残存finding修正 | 実機検証 3 点すべて合格 (自律 Pulse でダイアログが出ず auto-mode skip / 定時 Playbook 起動 / SPELL_ENABLED=false のペルソナが realtime spell 部屋で喋っても実行行ゼロ — ON の対照は過去ログで確認)。次 = A3 の着手裁定のみ。 | まはー(A3 裁定) | [完了計画書](audit_remediation_plan.md) / [W10 走行メモ](../handoff/2026-08-16_w10_spell_audit_remnants_handoff.md) | 2026-08-17 |
...
| 🟠 実装待ち | runtime_llm.py 巨大 node 分割 | 実機スモーク完了 (①〜③実走緑、④sync はまはー裁定で実機見送り — 単体回帰を歯止めに)。次 = Phase 2 (4 経路の分離) の着手タイミングをまはーと決める (リリース前は凍結が自然)。 | まはー(GO) → 私(Phase 2) | [runtime_llm_node_split_design.md](../issues/runtime_llm_node_split_design.md) | 2026-07-22 |
...
| 🟣 検証待ち | 知覚消費点の Beat 頭化 + フィード入室配送 | 到着時読解は実機確認済み、配送の重複なしも機械検査で確認済み (全ペルソナの buffer→バッチ対応が 1:1)。残 = 作業セッション頭の知覚消費の実機確認だけで、これは作業コマの初走行 (v0.4 の運転再開) への相乗り待ち。 | まはー (実機検証の続き) | [issue](../issues/feed_arrival_pulse_cannot_see_articles.md) / [handoff](../handoff/2026-08-08_beat_head_perception_handoff.md) | 2026-08-08 |
...
| 🟣 検証待ち | v0.3.0 の門 (リリース範囲と実施順序) | **リリース発行済み** (タグ v0.3.0 + GitHub Release、ZIP は Actions が自動添付)。残るは Release の ZIP を使った Git 自動導入のクリーン Windows 実機テストのみ。 | まはー (クリーン実機テスト) | [v030_release_gate.md](v030_release_gate.md) / [Release v0.3.0](https://github.com/maha0525/SAIVerse/releases/tag/v0.3.0) | 2026-09-01 |
...
| 🟣 検証待ち | 未確定の下書き行で発言が消える (根治) | 停止経路は実機合格済みで、残りは「確定後の隙間で Beat が死んだ回の記憶補填」の実地発火だけ。補填の成功は backend.log に「memory backfill on beat death succeeded」の印が出る。次 = 普段使いで発火を待ち、メティスがログの印を掃いて発火と救済を報告する。 | 実地の発生待ち (ログ監視はメティス) | [issue](../issues/orphaned_streaming_placeholder_cleanup.md) / [引き継ぎ](../handoff/2026-08-27_stop_path_and_lost_utterances.md) | 2026-08-29 |
...
| 🟣 検証待ち | v0.2 形式の記憶 DB で会話が毎回見送りになる (床の確立失敗) | 報告者は v0.3.10 適用済みで、初回の記憶の整理が走る段階まで進んだ。次 = 整理が完走して会話が返ることの回復確認をもらう。 | 外部 (報告者確認) | [issue](../issues/window_floor_unmet_on_v02_memory_db.md) | 2026-09-07 |
| 🟣 検証待ち | Chronicle の被覆の穴 — 詰まらない統合と確定を装わない記録 | 第一段は v0.3.11 として発行済みで、まはーの実機確認も済んでいる。次 = 発端の報告者に更新後の再測定 (送信量が下がるか) をもらう。第二段 (仮のあらすじと事後承認) は未起草。 | 外部 (報告者の再測定) | [intent](../intent/chronicle_coverage_gaps.md) | 2026-09-09 |
| 🟣 検証待ち | スルースの被覆の穴 — 会話を止めない採取と後から拾える記録 | 第一段は v0.3.11 として発行済みで、隔離環境の実サーバーで冷間の飛ばしと記録の実走も確認済み。次 = 稟乃さんに更新案内を送り (まはー)、会話が戻ることの確認をもらう。その後に第二段 (期間選択 UI) の着手裁定。 | まはー (更新案内) → 外部 (回復確認) | [intent](../intent/sluice_coverage_gaps.md) | 2026-09-09 |
| 🟣 検証待ち | Windows の psutil 無し環境で UI セルフアップデートが必ず失敗する (v0.3.11 主案件) | v0.3.11 として発行済み。次 = 次の版への更新のとき、画面からの更新が一発で通ることを実機で確認して閉じる (ZIP の Git 自動導入の行と同じ型)。 | まはー (次リリース時の実機確認) | [issue](../issues/self_update_unsafe_without_psutil.md) | 2026-09-09 |
| 🟣 検証待ち | 生成が途中で終了した回の後始末 (ⅰマーク二重・中断通告・Gemini の続きの生成) | 実装・レビュー収束済みだが、サーバー切断は狙って起こせないため実地の発生待ち。次 = 普段使いで生成が途中で切れた回に、発言の後ろに通告が入ることと続きの生成が Gemini でも通ることを見る。通告の書き込みの印はメティスがログを掃いて報告する。 | 実地の発生待ち (ログ監視はメティス) | [issue](../issues/server_cut_stream_writes_no_interruption_notice.md) | 2026-09-22 |
| 🟣 検証待ち | UI ローカライズおよびペルソナ・City 言語設定 | 画面文言の外出し、ペルソナ/City言語設定、City言語継承、直書き回収まで実装済み。次 = まはーの実機で言語切り替えとペルソナ設定の保存、表示の整合を確認する。 | まはー (実機検証) | [intent](../intent/localization.md) | 2026-09-14 |
| 🟣 検証待ち | 通話モード (Gemini Live でペルソナと音声通話) | 実験的機能 (開発者モード限定の入口) として develop に入っている。次はまはーの実機で開発者モードから通話を一度通し、育成テーマ (会話の浅さ・文字起こし精度) の当たりを付ける。 | まはー (実機確認) | [voice_call.md](../intent/voice_call.md) | 2026-09-18 |
| 🟣 検証待ち | プロバイダタブの API キー入力 (キー設定ボタン) | 実装・隔離環境での画面と保存の確認まで完了し、PR を作って確認待ちにする。次 = まはーが PR の内容を確認してマージし、実機のプロバイダタブでキー設定ボタンと保存の反映を見る。 | まはー (PR 確認 → 実機確認) | [intent §12](../intent/model_provider_management.md) | 2026-09-27 |
| 🟣 検証待ち | 反射判断 — 型付き質問で確率を返す判断層と答える側の差し替え | 第 1 段・第 2 段とも develop へ入り、v0.3.14 として発行済み (既定は OFF)。フォールバックの可視化 (チャットの注記・設定画面の集計・待ち時間のグローバル設定) も同じ版に乗っている。次 = まはーが実機で軽量モデルの割り当て・ペルソナ個別の上書き・注記の出方を評価する。 | まはー (実機評価) | [intent](../intent/reflex_judgment.md) | 2026-09-21 |
