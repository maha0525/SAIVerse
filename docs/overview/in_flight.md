...
| 🟡 実装待ち | 会話の終わり方 (出口) と応答のやり直し | 設計確定 — 出口 7 通り / 片付けは未読なら差し戻し・既読なら残す固定 / 操作はやり直しと送信の 2 つ (retryable は取り下げ)。やり直しは門の内へ引き上げ済み。確認 3 点は決着し、途中終了は「続き」を取る方針に差し替わった (削除の根拠はユーザーの意思だけ、操作は続き/やり直し/送信の 3 つ)。続きの機構は 504 経路に実在するので発火条件を広げる工事になる。次 = 部分文に「言い切っていない」印を持たせるかの裁定を受けて実装。 | まはー (裁定) → 私 (実装) | [issue](../issues/user_utterance_path_failure_inventory.md) / [門 §2-17](v030_release_gate.md) | 2026-08-25 |
