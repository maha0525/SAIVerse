...
| 🟣 検証待ち | 編纂の対称化 = 読み戻し (arasuji_levels §15) | プレビュー面は実機緑 (厚い生ログ・二重表示なし)。機構の本番未発火の根因は未被覆領域 (§16 被覆補修で根治中)。次 = テスタロッサとの一言会話で実機初発火 (計画が立っている状態を確認済み) → aifi は §16 治癒後に二例目。 | まはー (テスタロッサと一言) | [issue](../issues/metabolism_refill_when_below_target.md) / [intent](../intent/arasuji_levels.md) §15 | 2026-07-30 |
| 🟣 検証待ち | 被覆補修 (arasuji_levels §16) — 忘却の穴の根治 | 実装完了・Codex 一巡消し込み済み (採用 4 / 却下 1 却下理由は intent §16-2)・テスト 25+298 緑・コミット済み。次 = 実機検証: aifi の Chronicle タブに帯が出る → 見積もり確認 → 実行 → context-status の refill_applied が false→true になれば §15 読み戻しの前提回復も同時に確認できる。 | まはー (実機) | [intent](../intent/arasuji_levels.md) §16 / [issue](../issues/window_refill_rarely_plans_for_starved_personas.md) | 2026-08-31 |
