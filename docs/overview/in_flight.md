...
| 🟠 実装中 | v0.3.1 ホットフィックス (リリース直後の実機指摘) | 手動整理の背景ジョブ一本化・名称統一・ジョブ再接続まで実装済み (全スイート緑)。次 = ユーザー報告の根治 (ENABLE_MEMORY_WEAVE_CONTEXT 撤去とエラー文言修正) とシステムプロンプト棚卸しの検算 → まはー実機確認・Playbook 取り込み・Bag アイテム作成経路の判断 → develop へ PR。 | 私 (実装) | ブランチ hotfix/v0.3.1 | 2026-09-01 |
