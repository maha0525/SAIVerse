...
| 🟠 実装中 | コードレビュー運用・残存finding修正 | 実機検証 3 点のうち 2 点合格 (自律 Pulse でダイアログが出ず auto-mode skip / 定時 Playbook 起動 — 2026-08-31)。次 = 残る 1 点「SPELL_ENABLED=false で realtime spell が止まる」の実機確認と、A3 の着手裁定。 | まはー(実機検証 + A3 裁定) | [完了計画書](audit_remediation_plan.md) / [W10 走行メモ](../handoff/2026-08-16_w10_spell_audit_remnants_handoff.md) | 2026-08-17 |
...
| 🟣 検証待ち | 知覚消費点の Beat 頭化 + フィード入室配送 | 到着時読解は実機確認済み、配送の重複なしも機械検査で確認済み (2026-08-31、全ペルソナの buffer→バッチ対応が 1:1)。残 = 作業セッション頭の知覚消費の実機確認だけで、これは作業コマの初走行 (v0.4 の運転再開) への相乗り待ち。 | まはー (実機検証の続き) | [issue](../issues/feed_arrival_pulse_cannot_see_articles.md) / [handoff](../handoff/2026-08-08_beat_head_perception_handoff.md) | 2026-08-08 |
