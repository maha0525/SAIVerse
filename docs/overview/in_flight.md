...
| 🟠 実装中 | 知覚バッファ (Perception Buffer) | 核=「主観時間は Pulse でのみ進む／知覚は Pulse 消費時のみ／未消費の間だけ型別 reduce／知覚イベントは起動力属性を持ち決定論で反応可否を決める」。差分通知・メタ記憶訂正・入室想起・**会話取り込み**を一本化する横断基盤。**Phase 1a 実機検証済み**(quon_city_a: コア記憶3件復元→バッファに溜まる→会話 Pulse で1メッセージに畳まれ SAIMemory へ→バッファ空、まで全経路確認。pytest 34件)。次: **Phase 1b(起動力ディスパッチャ+「絶対反応する」フラグを§3.1全契機に付与[漏れ=沈黙リグレッション]) → Phase 2(状態差分/会話取り込みの経路統合+3直挿入撤去) をまとめて実装 → その後まとめて実機検証**(まはー方針)。方針確定済(粒度=per-persona・永続化・検知/消費分離〈§4.5〉・軽量LLM門番(SN)は将来のみ・恒久二重禁止) | 私(1b→2 実装) → まはー(実機検証) | [perception_buffer.md](../intent/perception_buffer.md) | 2026-07-09 |
