...
| 🟣 検証待ち | 時間割の抜本改修 (習慣×偶然) | 実装 (T1〜T4+T2b+経験の台帳) と Codex レビュー八巡の消し込みが worktree ブランチ feature/timetable-redesign に全コミット済み (フルスイート緑・残指摘 2 件は issue 化)。次 = メイン系へ rebase 統合 → playbook 2 本の再取込 → 実機検証 (手順は RSS・時間割の夜間ハンドオフ §3、intent §13 から辿れる)。 | 私 (rebase 統合) / まはー (実機検証) | [timetable_redesign.md](../intent/timetable_redesign.md) | 2026-08-04 |
...
| 🟣 検証待ち | 編纂が同名ページを増やす輪の根治 | 根治と「統合時に吸収側の Fragment を survivor へ付け替える」修正までコミット済み。次 = (a)(b) を済ませて編纂を再開したうえでの実機検証。 | まはー (実機検証、(a)(b) 完了後) | [issue](../issues/curation_duplicate_pages_loop.md) / [handoff](../handoff/2026-08-05_curation_loop_and_fragment_shift.md) | 2026-08-06 |
| 🟣 検証待ち | Memopedia の本文を Fragment へ寄せる | 実装とレビュー対応まで完了 (既存 Fragment との照合・想起可視性の穴も塞ぎ済み)。次 = ロック漏れ工事の完了後、UI 実画面の確認と本番アイフィへの適用 (まはーと一緒に、操作ごとの承認つき)。 | 私 (ロック漏れ工事) → まはー (実機) | [memopedia_body_to_fragment.md](../intent/memopedia_body_to_fragment.md) / [handoff](../handoff/2026-08-05_curation_loop_and_fragment_shift.md) | 2026-08-06 |
| 🟡 実装待ち | Memopedia 書き込みのロック漏れ (抽出器ほか) | 一部の生成箇所が `Memopedia(conn)` を db_lock 無しで作り、同じ接続なのに排他が成立しない。抽出器は追記失敗を警告ログで握り潰すため記憶が黙って落ちる。次 = 漏れている生成箇所へ adapter のロックを渡し、抽出バッチの書き込み失敗を失敗として扱う。 | 私 (実装) | [issue](../issues/memopedia_writers_bypass_adapter_lock.md) | 2026-08-05 |
