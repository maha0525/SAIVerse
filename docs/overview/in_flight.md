...
|---|---|---|---|---|---|
| 🟣 検証待ち | 移動の瞬間に積む知覚通知の是正 (v0.3.10 主案件) | 実装済み。レビューはローカル + Codex 三巡で消し込み、三巡目の想起の配送保証 2 件は既存設計への指摘として却下 (理由は issue の経緯節)。次 = まはーの収束判定と実機検証 (部屋の往復でスペル対と「がいます」が出ないこと)。 | まはー (収束判定・実機検証) | [issue](../issues/perception_state_pushed_at_event_time.md) | 2026-09-07 |
