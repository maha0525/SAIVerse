...
| 🟣 検証待ち | 知覚の堆積対策 7 件 (再会の門 / スペルのスキーマ同梱 / 部屋の差分+移管 / 合計の二水位上限 / 省略の印 / 保存時検査 / head との全文重複の根治) — v0.3.9 主案件 | リリース前確認で差分の組成に出荷停止級の欠陥が見つかり、発行は差分の再設計 (下の行) の決着待ち。差分以外の 6 件は生きている。次 = 再設計の完了後に束を組み直して再検証。 | 私 (再設計) | [issue](../issues/persona_recall_perception_unbounded.md) / [head 重複 issue](../issues/room_state_duplicates_head_inventory.md) | 2026-09-06 |
| 🔵 設計中 | 部屋の様子の差分の再設計 (文字列解析 → パッケージ単位) — v0.3.9 出荷停止の原因 | 設計の問いは全て裁定済み (Close は一行 / 思い出機能は退役して上位互換を想起用タグ intent に約束 / 消え方は描き分けない / 一元化と一緒に設計 / メディアはパッケージの属性 / 本文の変化は diff 表示が理想)。次 = 一元化と合わせた設計を起こし、レビュー資料と一緒に出す。 | 私 (設計起草) | [issue](../issues/room_state_diff_built_on_string_parsing.md) / [一元化 issue](../issues/room_state_single_source_of_truth.md) | 2026-09-06 |
