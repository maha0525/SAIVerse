...
| 🟣 検証待ち | Elyth Remote MCP 移行 (Local MCP は 8/31 終了) | 実装と 3 巡ぶんの消し込みは終わり、まはーの実機検証待ち。検証手順にある「鍵を消す」操作が UI に無かったので、アドオン設定の secret 欄に削除ボタンを足した (全ペルソナ共通側は API にも削除経路が無かった)。次 = まはーがハンドオフ §10 の手順 1〜11 を通す。4 巡目を回すかも同時に裁定。 | まはー (実機検証) | [handoff](../handoff/2026-08-10_elyth_remote_mcp_handoff.md) / [intent](../intent/mcp_addon_integration.md) §I | 2026-08-10 |
