...
| 🟠 実装中 | v0.3.0 の門 (リリース範囲と実施順序) | 実機検証でリリースを止める欠陥が二つ出た: ①冷えた起点の前進 (機構 1) がスルースを通さずに退場させる ②スルースの構造化出力が整数欄で数字ループに入り一度も成功していない。次 = ①は不変条件「起点はパンマーカーを越えない」で直す、②は隔離環境で生の応答を一回取って原因を見てから直す (課金あり、まはー承認待ち)。両方直してから実機検証の 5 番 (スルース) へ戻る。 | まはー (②の実験承認) / 私 (①の修正) | [v030_release_gate.md](v030_release_gate.md) / [裁定ハンドオフ](../handoff/2026-08-22_v3_shape_layer_rulings_handoff.md) | 2026-08-22 |
