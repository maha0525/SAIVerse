...
|---|---|---|---|---|---|
| 🟣 検証待ち | 右パネルの現在地が、部屋を移動しても 10 秒後に前の部屋へ戻る (パネルからの操作も前の部屋に効く) | 修正はこのブランチにあり、隔離環境では再現と解消の両方を見ている。次はまはーの実機で、パネルを開いたまま部屋を移り、10 秒待っても戻らないことと、アイテムの ＋ がいま見ている部屋を指すことを確かめる。そのうえで報告者へ更新を案内する。 | まはー (実機確認) → 外部 (報告者の確認) | [issue](../issues/right_sidebar_polling_holds_stale_building.md) | 2026-09-21 |
| 🟣 検証待ち | 移動ログが続くと、そのあいだの時刻が次の箱に覆われて読めない (報告者の環境では再現するが開発機では再現しない) | 行間を負の余白から正の余白へ書き換えて、重なりが起こりえない形にした。見た目の間隔は実測で従来と一致。次はまはーの実機で間隔が変わっていないことを見て、報告者には修正版の確認とあわせてブラウザとバージョンを訊く。 | まはー (実機確認) → 外部 (報告者の確認と環境の聞き取り) | [issue](../issues/system_notice_time_hidden_by_next_notice.md) | 2026-09-21 |
| 🟣 検証待ち | NIM のモデルで構造化出力を使うと画像がモデルに届かず、画像の上限を定義に書くとクライアントが作れない | 修正は develop に入っている。次はまはーの実機で、NIM のモデルを使って画像つきの記憶の取り込みを一度回し、画像がモデルに届くことと、画像の埋め込み上限が効くことを確かめる。 | まはー (実機確認) | [issue](../issues/nim_structured_output_drops_images.md) | 2026-09-18 |
...
| 🟣 検証待ち | 生成が途中で終了した回の後始末 (ⅰマーク二重・中断通告・Gemini の続きの生成) | 通告の追加・絵文字の除去・印の残留の栓まで実装し、代行レビュー二巡で収束・回帰緑。次 = まはーの実機で、生成が途中で切れた発言の後ろに通告が入り、続きの生成が Gemini でも通ることを確認する。 | まはー (実機検証) | [issue](../issues/server_cut_stream_writes_no_interruption_notice.md) | 2026-09-14 |
...
| 🟣 検証待ち | 反射判断 — 型付き質問で確率を返す判断層と答える側の差し替え | 第 1 段は実装・レビュー収束済みで、develop 向けの [PR #309](https://github.com/maha0525/SAIVerse/pull/309) がまはーの確認待ち。次 = まはーが PR を確認・マージし、実機でモデルロールの割り当てとペルソナのトグルから想起を評価する (手順は docs/handoff/ の引き継ぎ)。その後に第 2 段。 | まはー (PR 確認 → 実機検証) | [intent](../intent/reflex_judgment.md) | 2026-09-20 |
| 🟣 検証待ち | 自動想起の Jev 選別層 (実験) | 拾い上げの拡張とクエリの種の修正は実機で通っている。ON/OFF は反射判断の第 1 段でペルソナ設定のトグルへ移った。次 = まはーがそのトグルを入れて、発言に沿った想起になったか・ノイズ・体感遅延を実機で評価する。 | まはー (実機再評価) | [auto_recall_jev_rerank.md](../intent/auto_recall_jev_rerank.md) | 2026-09-19 |
