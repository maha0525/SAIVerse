...
| 🟠 実装中 | 極小 run の隣人吸収 (豆粒編纂の捏造根治) | 吸収本体 + span すくい取り + 帳簿追随は実装・検収済み。裁定の改訂 (見送り廃止 → anchor 引き戻し + 窓あふれ時はジョブ内で即時畳み) を issue に収録済み。次 = 引き戻しラウンドの実装検収 → ローカルレビュー → Codex → コミット → aifi の豆粒後始末 (削除→補修再実行)。 | 私 (実装検収) | [issue](../issues/arasuji_tiny_run_absorption.md) | 2026-08-31 |
