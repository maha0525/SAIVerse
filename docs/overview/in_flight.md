...
| 🟡 実装待ち | 反射判断 — 型付き質問で確率を返す判断層と答える側の差し替え | 設計確定 (裁定全件済み。スイッチはペルソナごとの「自動想起を強化する」、同梱 3 つ、実装は 2 段)。次 = compact 後のセッションで第 1 段 (共通層 + jev 互換 provider type + 役割 + ペルソナのスイッチ + 想起の載せ替え) を実装する。入口は [引き継ぎ](../handoff/2026-09-19_jev_reflex_judgment_handoff.md)。 | 私 (第 1 段の実装) | [intent](../intent/reflex_judgment.md) | 2026-09-19 |
| 🟣 検証待ち | スペル入りの複数 Beat の Pulse で思考が UI に出ない | 周ごとに思考を回収して、その Beat の建物の記録と記憶の両方へ添える形で実装済み。経路間の非対称も揃え、回帰も足した。次 = まはーの実機で、スペルを使ったターンの各吹き出しに思考の折りたたみが戻ることを確認する。 | まはー (実機検証) | [issue](../issues/spell_pulse_beats_missing_reasoning.md) | 2026-09-19 |
| 🟣 検証待ち | 自動想起の Jev 選別層 (実験) | 拾い上げの拡張は実機で判定が通ることを確認済み。加えて、部屋の様子がユーザー発言からクエリの種を奪うバグを修正した (これだけは OFF にも効く)。次 = まはーがサーバーを再起動し、発言に沿った想起になったか・ノイズ・体感遅延を実機で評価する。 | まはー (実機再評価) | [auto_recall_jev_rerank.md](../intent/auto_recall_jev_rerank.md) | 2026-09-19 |
