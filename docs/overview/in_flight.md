...
| 🟢 実装中 | v0.3.0 の門 (リリース範囲と実施順序) | v3 §9 の未決は全 9 件が決着 (§9-9 は観察面の再定義込み — ライフビューは暮らしの窓へ、できごとは本人の言葉優先の導出で存続)。次 = 連続性グラフ (門 §2-14) のデータモデルを詰め、机上完動の判定をまはーと行って v0.3 実装 (形の層) へ。 | まはー (設計議論の続き) | [v030_release_gate.md](v030_release_gate.md) / [autonomous_behavior_v3.md](../intent/autonomous_behavior_v3.md) | 2026-08-19 |
