...
| 🔵 設計中 | 反射判断 — 型付き質問で確率を返す判断層と答える側の差し替え | まはーの裁定 5 件と互換勢 4 つの調査を反映し、モデル設定ファイル + 役割の形へ改訂済み (専用の切り替え設定は不要になった)。次 = 残る裁定 3 件 (英字表記と env 名・同梱する設定ファイルの範囲・実装の段割り) をまはーに確認して設計確定。 | まはー (残り裁定 3 件) | [intent](../intent/reflex_judgment.md) | 2026-09-19 |
