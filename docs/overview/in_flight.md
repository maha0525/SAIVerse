...
| 🟣 検証待ち | 編纂の対称化 = 読み戻し (arasuji_levels §15) | プレビュー面は実機緑 (厚い生ログ・二重表示なし)。ただし読み戻し機構は本番未発火 (発火ログ `[metabolism] window refill` が全セッションでゼロ)。次 = 発生待ちへ回すか強制発火 (大窓モデルへ切替えて一言) かのまはー裁定。 | まはー (裁定) | [issue](../issues/metabolism_refill_when_below_target.md) / [intent](../intent/arasuji_levels.md) §15 | 2026-07-30 |
