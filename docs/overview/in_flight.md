...
| 🟠 実装中 | Codex サブスク認証の自立 (デバイスコードログイン) | 全層実装済み・テスト緑だが Codex 1 巡目が No-ship (競合系 high 3 + medium 1、全件受諾)。次 = Opus セッションがハンドオフを入口に消し込み → まはーの実機ログイン検証へ。 | 私 (Opus 消し込み) → まはー (実機ログイン) | [intent](../intent/codex_subscription_auth.md) / [handoff](../handoff/2026-08-16_codex_subscription_auth_handoff.md) | 2026-08-16 |
