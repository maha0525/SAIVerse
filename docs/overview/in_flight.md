...
| 🟢 実装中 | v0.3.0 の門 (リリース範囲と実施順序) | 自律行動 v3 の骨格裁定とリリース線引き (v0.3=形 / v0.4=運転、設計は全域を先に完成) が確定し、intent と門の計画 v2 に収録した。次 = 設計の残り (想起用タグのスキーマを筆頭に v3 §9 と連続性グラフ) をまはーと詰め、机上完動に達したら v0.3 実装 (形の層) へ。 | まはー (設計議論の続き) | [v030_release_gate.md](v030_release_gate.md) / [autonomous_behavior_v3.md](../intent/autonomous_behavior_v3.md) | 2026-08-12 |
...
| 🟣 検証待ち | City の識別子と表示名の分離 | 識別子 (CITY_SLUG) と表示名 (CITYNAME) の分離、マップ見出しの表示名化と編集ボタン、移行まで実装済み。実 DB のコピーで移行を予行し、部屋とペルソナ ID が動かないことを確認済み。次 = まはーの実機検証 (見出しの表示名 / 編集の反映 / 再起動後も部屋と建物ログが元のまま / 新規ペルソナ ID の形)。 | まはー (実機検証) | [city_identity.md](../intent/city_identity.md) | 2026-08-14 |
