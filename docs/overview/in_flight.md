...
| 🟣 検証待ち | 極小 run の隣人吸収 (豆粒編纂の捏造根治) | 実装・レビュー収束・コミット済み (経緯は issue)。次 = aifi の後始末: バックエンド停止 → 補修前バックアップ (KEEP_pre_repair、退避済み) への復元 (実施時にまはー GO) → 再起動 → 修正済み補修の実行 → 合体あらすじの品質と world DB の古い印の自然回収をメティスが検算。 | まはー (復元 GO と実機) | [issue](../issues/arasuji_tiny_run_absorption.md) | 2026-08-31 |
