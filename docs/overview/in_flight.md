...
| 🟣 検証待ち | Elyth Remote MCP 移行 (Local MCP は 8/31 終了) | §I 再設計の実装と、レビュー 2 巡ぶんの消し込み (関所を接続直前へ / 両 Pulse root で取得 / timeout を fail-closed / 畳む理由に応じた片付け範囲) まで完了。次 = 3 巡目のレビューで裏返しの有無を見る (2 巡目は 1 巡目の修正が作った裏返しだった) → まはーの実機検証 (手順はハンドオフ §10 に操作順で用意済み)。 | 私 (3 巡目) → まはー (実機検証) | [handoff](../handoff/2026-08-10_elyth_remote_mcp_handoff.md) / [intent](../intent/mcp_addon_integration.md) §I | 2026-08-10 |
