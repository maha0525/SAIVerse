...
|---|---|---|---|---|---|
| 🟣 検証待ち | NIM のモデルで構造化出力を使うと画像がモデルに届かず、画像の上限を定義に書くとクライアントが作れない | 修正とテストを終え、develop 向けの PR #306 を出した。次はまはーの PR 確認で、マージ後に NIM のモデルで画像つきの記憶の取り込みが画像を送ることを確かめる。 | まはー (PR 確認) | [issue](../issues/nim_structured_output_drops_images.md) | 2026-09-17 |
| 🟣 検証待ち | 上限の小さいモデルに切り替えると記憶の整理から抜け出せない (スルースを飛ばす判定が、実際に送る量を見ていない) | 実装・レビュー・隔離環境の通し確認まで済み、develop 向けの PR #304 を出した。次はまはーの PR 確認で、マージ後にうるさんへ修正版を案内し、4o に切り替えて会話が成立するかを確かめてもらう。 | まはー (PR 確認) → 外部 (うるさんの確認) | [issue](../issues/sluice_skip_ignores_model_context.md) | 2026-09-17 |
...
| 🟣 検証待ち | 通話モード (Gemini Live でペルソナと音声通話) | 実機体感まで済み、実験的機能 (開発者モード限定の入口) として PR 提出済み。次 = まはーの PR 内容確認とマージ判断。マージ後の育成テーマ = 会話の浅さと文字起こし精度 (保存済み音声の文脈付き再転写)。 | まはー (PR 確認) | [voice_call.md](../intent/voice_call.md) | 2026-09-16 |
| 🟣 検証待ち | 自動想起の Jev 選別層 (実験) | 候補の拾い上げが狭く、浮かぶべき過去の会話が Jev の手前に届いていない。拾い上げの拡張 (発話の自席占領の除外・message 枠の拡大・日本語キーワード抽出) を実装した。次 = まはーが実験ブランチで有効化し、過去の会話が浮かぶようになったかを実機で再評価する。 | まはー (実機再評価) | [auto_recall_jev_rerank.md](../intent/auto_recall_jev_rerank.md) | 2026-09-18 |
