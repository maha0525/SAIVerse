...
|---|---|---|---|---|---|
| 🟠 実装中 | 部屋や持ち物の変化の知らせが、ペルソナに二回届くことがある (同時に調べた回・B の保存だけ失敗して再起動した回) | 同時に調べた回の二重登録を直し、再起動をまたぐ重複は受け入れとして intent に明記した。次 = ローカルレビューと Codex のレビューを指摘ゼロまで回し、develop 向けの PR を出す。 | 私 (レビューと PR) | [issue](../issues/head_diff_notification_duplicate_delivery.md) | 2026-09-25 |
