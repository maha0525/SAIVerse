...
| 🟣 検証待ち | 知覚の堆積対策 7 件 (再会の門 / スペルのスキーマ同梱 / 部屋の差分+移管 / 合計の二水位上限 / 省略の印 / 保存時検査 / head との全文重複の根治) — v0.3.9 主案件 | 出荷停止の原因だった差分の再設計は実装まで済んだ (下の行)。次 = 再設計をコミットしたうえで束を組み直し、実機検証と数字 3 点 (知覚の既定・保存時検査の余裕・オプトアウトの穴) の裁定へ進む。 | まはー (実機検証と数字の裁定) | [issue](../issues/persona_recall_perception_unbounded.md) / [head 重複 issue](../issues/room_state_duplicates_head_inventory.md) | 2026-09-06 |
| 🟣 検証待ち | 部屋の様子のパッケージ (文字列解析の退役 + 一元化) — v0.3.9 出荷停止の原因 | 実装・レビュー収束 (敵対レビュー 14 巡、全消し込み red 証明つき)・フルスイート緑・コミット済み。次 = intent §10 の実機の旅 (往復一行 / Close 一行 / 絵が見える / 整理後に絵が戻る / 滞在中の追加が届く / 起動直後に部屋が見える)。 | まはー (実機検証と PR #282 確認) | [intent](../intent/room_state_packages.md) / [発端 issue](../issues/room_state_diff_built_on_string_parsing.md) | 2026-09-07 |
