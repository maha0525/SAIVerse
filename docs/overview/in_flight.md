...
| 🔵 設計中 | 読み戻しがあらすじの途中まで遡ろうとして一歩も動かない | 原因と方針は確定 (予算で履歴を切ってから段を探す順序が誤り。段を単位に開く形へ入れ替える)。次 = Fable 5 の監督下で実装する。最終防衛ラインも同じ予算方式で段を分割しうるので、その整合を実装前に決める。 | まはー (Fable 5 での実装) | [issue](../issues/refill_reads_by_budget_instead_of_arasuji_unit.md) / [arasuji_levels.md](../intent/arasuji_levels.md) §15 | 2026-09-04 |
