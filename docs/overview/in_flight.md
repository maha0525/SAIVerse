...
|---|---|---|---|---|---|
| 🟣 検証待ち | 知覚の堆積対策 6 件 (再会の門 / スペルのスキーマ同梱 / 部屋の差分+移管 / 合計の二水位上限 / 省略の印 / 保存時検査) — v0.3.9 主案件 | 6 件全部とレビュー 4 巡の消し込みまで実装済み・フルスイート緑。次 = まはーの実機で、更新後に知覚の合計が下の水位まで自動で下りることと、数字 3 点 (知覚の既定・余裕・オプトアウトの穴、issue に記録) の裁定。 | まはー (実機検証と数字の裁定) | [issue](../issues/persona_recall_perception_unbounded.md) / [watermarks issue](../issues/watermarks_unsatisfiable_when_perception_is_large.md) | 2026-09-05 |
