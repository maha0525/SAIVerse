...
| 🟠 実装中 | スルースの被覆の穴 — 会話を止めない採取と後から拾える記録 | 第一段の A (飛ばしと記録)・C (安全装置・読み戻し規則) と設計書同時改訂まで実装済み、フルスイート合格。次 = 隔離環境での稟乃さんの形の再現検証 → 前置き文面のまはー検収 → レビュー。 | メティス (実装) | [intent](../intent/sluice_coverage_gaps.md) | 2026-09-08 |
