...
| 🟠 実装中 | Track 撤廃 (最後の住人たちの引っ越し) | 順序① (v1 メタ判断の退役 + alert 直結化 + 「いまの活動」の出来事化) は実装完了・回帰緑。Codex レビュー 1 巡で指摘 6 件 — 全件裏取り済み・裁定つきハンドオフに記入済み (F1 はまはー裁定で却下=会話優先の仕様どおり)。次 = Opus セッションで残り 5 件の消し込み (F2→F5→F6→F3→F4 推奨) → 実機検証。 | Opus (消し込み) | [handoff](../handoff/2026-08-14_track_retirement_step1_review_handoff.md) / [intent](../intent/track_retirement.md) §7 | 2026-08-14 |
