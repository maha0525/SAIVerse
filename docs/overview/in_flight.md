...
| 🟣 検証待ち | Elyth Remote MCP 移行 (Local MCP は 8/31 終了) | 移行本体と §I 再設計 (discovery 廃止 → Pulse 頭接続 + Beat 頭一覧取得 + 知覚バッファ配送) の実装・テストまで完了。次 = Codex レビュー指摘の裁定と消し込み (次セッション、ハンドオフ §7 参照) → まはーの実機検証 (ハンドオフ §6)。 | 次セッション (消し込み) → まはー (実機検証) | [handoff](../handoff/2026-08-10_elyth_remote_mcp_handoff.md) / [intent](../intent/mcp_addon_integration.md) §I | 2026-08-10 |
