...
| 🔵 設計中 | 構造化判断の口 — 裏側 (Jev / 互換の宛先 / 軽量モデル) の差し替え | Jev で開けた「状況と質問を渡すと確率が返る」形を、裏側差し替え可能 + 想起以外の構造化判断でも使える形へ一般化する intent を起草済み。次 = まはーの設計レビュー (裁定ポイント 4 つ: 名称・費用の記帳・キー無し許容・質問の型の範囲)。 | まはー (設計レビュー) | [intent](../intent/structured_judgment_backends.md) | 2026-09-19 |
