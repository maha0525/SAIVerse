...
| 🟣 検証待ち | 通話モード (Gemini Live でペルソナと音声通話) | 実機体感の第一巡を反映済み (一巡切断の修正・声の指定の検証・発話単位の音声保存)。次 = バックエンド再起動してまはーの実機体感の続き。音声保存の実機確認もここに含む。 | まはー (実機体感) | [voice_call.md](../intent/voice_call.md) | 2026-09-16 |
