...
| 🟣 検証待ち | 会話の終わり方 (出口) と応答のやり直し | 手順①〜③は実機で踏み、出た 4 件を修正済み (案内の寿命 + 応答者ゼロの札)。次 = バックエンド再起動とフロント再読込のうえで、①〜③の再確認と手順④〜⑥の実機検証。 | まはー (実機検証) | [issue](../issues/user_utterance_path_failure_inventory.md) / [引き継ぎ](../handoff/2026-08-26_conversation_exits_review_and_live_fixes.md) | 2026-08-26 |
