...
| 🟣 検証待ち | Memopedia 書き込みのロック漏れ (抽出器ほか) | 漏れていた生成箇所への adapter ロックの配線と、抽出失敗の三層の握り潰し撤去 (ERROR ログ + 実行台帳 + 画面通知で表へ出す) まで実装済み。次 = 実機で Metabolism を一度回し、抽出が通ることと失敗時の通知を確認。 | まはー (実機確認) | [issue](../issues/memopedia_writers_bypass_adapter_lock.md) | 2026-08-06 |
