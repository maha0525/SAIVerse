...
| 🟣 検証待ち | 自律 Pulse の器統合 + tell スペル (会話 Track の特殊機構剥がし) | 消し込みは 2026-08-08 に終了 (まはー判断: キャッシュの件で変わる箇所をこれ以上磨く意味が薄い)。キャッシュ延命の論点は門 §2-1 (モデル格再設計) が引き取った。次 = 実機検証 (統合検証手順に相乗り)。 | まはー (実機検証) | [issue](../issues/slot_light_pulse_runs_on_conversation_vehicle.md) / [intent](../intent/autonomous_pulse_vehicle.md) | 2026-08-09 |
