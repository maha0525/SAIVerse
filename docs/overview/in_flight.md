...
| 🟡 実装待ち | 会話の終わり方 (出口) と応答のやり直し | 設計確定 — 出口 7 通り / 片付けは未読なら差し戻し・既読なら残す固定 / 操作はやり直しと送信の 2 つ (retryable は取り下げ)。やり直しは門の内へ引き上げ済み。設計は全て確定し、束 1 (中断の記録の一本化 + 自動継続の撤去) は実装済み・フルスイート緑。印は `_interrupted` で、建物履歴とペルソナの記憶の両方に載る。次 = 束 2 (無言をなくす — 握り潰しの解除 / 生 HTML / 出口イベントの新設 / 空 placeholder の掃除) を実装する。 | 私 (実装) | [issue](../issues/user_utterance_path_failure_inventory.md) / [門 §2-17](v030_release_gate.md) | 2026-08-25 |
