...
| 🟠 実装中 | コア記憶の訂正導線 + ごみ箱 (短期対応) | **DB層✅ / API層✅ / メモリタブUI✅**(削除は実機確認済)。**仮想センサーは 2026-07-09 に SAIMemory 挿入を応急停止**(削除/訂正の一括操作で通知が会話文脈を埋める問題、実機発覚。経路は残し実挿入だけ early return)。残①: 恒久対応は **[知覚バッファ](../intent/perception_buffer.md) の一利用者**として載せ替える(単発の即時挿入を廃し Pulse 消費で集約)。残②: 未確認バッジの置き場所を確定(ペルソナ固有アフォーダンス、住民/神モードUI と交差) | まはー(バッジ置き場) → 私 | [memory_architecture_v2.md](../intent/memory_architecture_v2.md) §5.1 | 2026-07-09 |
...
| 🔵 設計中 | 知覚バッファ (Perception Buffer) | v0.1 draft。核=「主観時間は Pulse でのみ進む／知覚は Pulse 消費時のみ／未消費の間だけ型別 reduce／知覚イベントは起動力属性を持ち決定論で反応可否を決める」。差分通知・メタ記憶訂正・入室想起・**会話取り込み**を一本化する横断基盤。主要設計判断は全て方針確定(消費者=全Pulse・会話も経路統合・粒度=per-persona・プレビューは ContextPreviewModal・**バッファは永続化**・**検知は消費と分離しプレビューも検知タイミング**〈§4.5〉・単一永続バッファへ一本化(3直挿入撤去)・軽量LLM門番(SN)は将来余地のみ・大原則=知覚経路を恒常二重にしない)。次: draft 通しレビュー → Phase 1 GO(永続バッファの器+検知/消費分離+起動力ディスパッチャ+「絶対反応する」フラグを現状§3.1全契機に付与[漏れ=沈黙リグレッション]+メタ記憶訂正載せ替え) | まはー(レビュー) → 私 | [perception_buffer.md](../intent/perception_buffer.md) | 2026-07-09 |
