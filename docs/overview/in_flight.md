...
| 🟢 実装中 | v0.3.0 の門 (リリース範囲と実施順序) | Wave 0 実装 7 件が完了しレビュー 1 巡 (ローカル+Codex) も済み。次 = Codex 指摘 5 件の消し込みを Opus セッションが実施 (必須 2・裁定待ち 2・受容 1、ハンドオフ参照)。並行してまはーが設計議論一本目 (エピソードの単位) を開ける。 | Opus (消し込み) / まはー (設計議論) | [v030_release_gate.md](v030_release_gate.md) / [ハンドオフ](../handoff/2026-08-09_wave0_review_handoff.md) | 2026-08-09 |
