...
| 🟠 実装中 | v0.3.1 ホットフィックス (リリース直後の実機指摘) | 実機指摘バッチに加えて Memopedia 索引デフォルト ON と手動整理の head 再構築一本化まで実装済み (全スイート緑)。次 = 手動整理ボタンの背景ジョブ化と名称統一 (「溜まった会話をあらすじにまとめる」) → まはー実機確認・Playbook 取り込み・Bag アイテム作成経路の判断 → develop へ PR。 | 私 (実装) | ブランチ hotfix/v0.3.1 | 2026-09-01 |
