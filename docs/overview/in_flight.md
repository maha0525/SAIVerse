...
|---|---|---|---|---|---|
| 🟠 実装中 | 部屋 ID に「/」が入った部屋の付け替え (過去ログが移らず毎起動で警告) | 付け替えの実装とレビュー指摘の訂正をすべてコミットし、隔離環境で N さんの形の部屋が付け替え→取り込み→警告なしになるのを確かめた。Codex の指摘がデータベースだけを戻したときの縁まで外側へ移ったので、いまはレビューを終えてよいかのまはーの判断待ち。終えたらフルスイートと隔離環境の確認をやり直して報告する。 | まはー (レビューを終えるか) | [issue](../issues/building_id_contains_path_separator.md) | 2026-09-11 |
...
| 🟣 検証待ち | 冷えたウィンドウの見張りの素通しゲート | 状態が変わらないペルソナへ同じ判定・失敗を繰り返さないゲートを実装済み・対象テスト緑。次 = hotfix/v0.3.12 の実機で、再起動後にモデル不達のペルソナへの再試行が一回で止まりログが静かになることを確認する。 | まはー (実機検証) | [arasuji_levels.md](../intent/arasuji_levels.md) §14-4 | 2026-09-10 |
