...
| 🟣 検証待ち | 未確定の下書き行で発言が消える (根治) | Beat の出口の except 節へ後始末を差し、例外で落ちる経路も塞いだ。Codex 2 巡 + ローカルレビューの消し込みまで済み、収束はまはー承認。通告の文面も決着。次 = 実機で LLM エラー時に言いかけた本文が残ることの確認。 | まはー (実機検証) | [issue](../issues/orphaned_streaming_placeholder_cleanup.md) / [引き継ぎ](../handoff/2026-08-27_stop_path_and_lost_utterances.md) | 2026-08-27 |
