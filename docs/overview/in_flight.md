...
| 🟣 検証待ち | v0.3.0 の門 (リリース範囲と実施順序) | 実機検証で出た欠陥 2 件は両方修正済み。機械検査が数値欄を 22 欄と数え直し、「4 Playbook」では足りずスペルの引数も同じ道に乗ると判明。次 = まはーが再起動してスルースの実機検証 (Memory 窓 → Chronicle → 生成。送信量の横棒が残す量を超えていること)、私が 22 欄のうち危ない並びを絞って隔離実験の回数を提示する。飛ばされた範囲の採取し直しは別途手順を決める。 | まはー (実機検証 / 採取し直しの手順) / 私 (数値欄の実験の絞り込み) | [v030_release_gate.md](v030_release_gate.md) / [裁定ハンドオフ](../handoff/2026-08-22_v3_shape_layer_rulings_handoff.md) | 2026-08-24 |
