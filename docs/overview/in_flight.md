...
| 🟣 検証待ち | Chronicle タブが最新エントリを表示しない (500件切り詰め) | 一覧 API を「新しい側から取得して表示順へ反転」に変更済み。隠すのは古い側の L1 だけ・切り詰めは件数を応答で明示・フロントにバナー追加。次 = 実機でエリスの最新エントリが見えることの確認。 | まはー (実機検証) | [issue](../issues/arasuji_modal_500_limit_truncation.md) | 2026-07-30 |
...
| 🟣 検証待ち | 判断点の席の競合制御とイベントの取りこぼし | 裁定 B (保持+再試行窓) で実装完了・レビュー収束・フルスイート緑。範囲外は event_delivery_reachability_gaps へ切り出し済み。次 = コミット → 実機検証。 | まはー (コミット判断) | [issue](../issues/judgment_seat_contention_and_event_loss.md) | 2026-07-31 |
