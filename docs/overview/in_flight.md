...
|---|---|---|---|---|---|
| 🟠 実装中 | 送る量の一系統化と会話以外の内容の節約 (v0.3.12 本丸) | 縮める規則 (設計 1・2) と、しきい値の一系統化・知覚の二水位の廃止・プリセットの定義と正典の書き直し (設計 3) まで実装と契約テストが済み、まはーの検収待ち。次は全体設定とモデル編集の画面をプリセットの形に改修する (いまの画面は廃止した知覚の欄と余裕の事前検査を持つため、既定の 20,000 / 60,000 が保存できない)。 | まはー (検収) → 私 (画面の改修) | [presented_context_reduction.md](../intent/presented_context_reduction.md) | 2026-09-09 |
