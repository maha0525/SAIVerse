...
|---|---|---|---|---|---|
| 🟠 実装中 | 部屋 ID に「/」が入った部屋の付け替え (過去ログが移らず毎起動で警告) | 設計はすべて GO 済みで、実装の第一陣が走っている。次は第一陣の検収のあと、整合性の検査で見つかった漏れとまはーの判断 2 件 (どちらも b) を訂正の回として実装し、隔離環境で N さんの形の部屋を通して出力を読む。 | 私 (訂正の回と検証) | [issue](../issues/building_id_contains_path_separator.md) | 2026-09-11 |
