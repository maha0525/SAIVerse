...
|---|---|---|---|---|---|
| 🔵 設計中 | 部屋や持ち物の変化の知らせが、ペルソナに二回届くことがある (同時に調べた回・B の保存だけ失敗して再起動した回) | 三つのケースとも隔離したテストで再現し、直し方の提案を issue に書いた。次 = まはーが三つのケースそれぞれの扱い (直す / 受け入れて明記) を決め、決まったものから実装する。 | まはー (直し方の判断) | [issue](../issues/head_diff_notification_duplicate_delivery.md) | 2026-09-25 |
