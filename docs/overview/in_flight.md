...
| 🟠 実装中 | 生成の段階の信号 (門 §2-18/19 + retry 門番) | 設計は三件とも issue に確定済み — 保存完了の三値信号と保存後イベント、送信結果の三値問い合わせ口、retry の「後続のペルソナ発言があるか」門番 (判定関数は共有)。実装をサブエージェントへ委譲する。次 = 検収、Codex 1 巡、コミット。 | 私 (検収) | [§2-18](../issues/unknown_send_outcome_has_no_recovery_path.md) / [§2-19](../issues/stream_completion_is_not_proof_of_persistence.md) / [J](../issues/retry_api_has_no_server_side_eligibility_check.md) | 2026-08-29 |
