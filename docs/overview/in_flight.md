...
| 🟣 検証待ち | v0.3.0 の門 (リリース範囲と実施順序) | 束 1〜6 が全て入り、v3 §11 の「形の層」は実装完了 — 6c も Codex 1 巡 4 件の消し込みとフルスイート緑を経てコミット済み。次 = まはーの実機検証 (会話の遷移の一行・起動時 migration の走行・隠した UI の不在・仲裁ゲートの新挙動) と、Opus+Codex+ローカルの掃討フェーズ (Fable 外で別途)。 | まはー (実機検証) | [v030_release_gate.md](v030_release_gate.md) / [autonomous_behavior_v3.md](../intent/autonomous_behavior_v3.md) | 2026-08-22 |
