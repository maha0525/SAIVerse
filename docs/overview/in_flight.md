...
| 🟠 実装中 | 知覚消費点の Beat 頭化 + フィード入室配送 | 実装済み (消費点移設 / 入室配送 / 看板描画 / テンプレ整合)・対象+隣接テスト緑。次 = ローカル+Codex レビュー 1 巡の結果をハンドオフへ記録し、消し込みは Opus セッションが担当 → 済んだら時間割・フィードの実機検証を再開。 | 私 (レビュー記録) → Opus (消し込み) | [perception_buffer.md](../intent/perception_buffer.md) §4.2 / [issue](../issues/feed_arrival_pulse_cannot_see_articles.md) | 2026-08-08 |
