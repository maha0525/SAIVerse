...
| 🟣 検証待ち | v0.3.1 ホットフィックス (リリース直後の実機指摘) | ユーザー報告二件の根治 (整理の門の撤去・帯予算 UI・診断拡充・死んだ参照の sweep・ワールドエディタのページ分け) とプロンプト掃除まで実装済み、VERSION=0.3.1 (全スイート緑)。次 = まはーの実機確認 (ページ分け・掃除後の補修再実行・帯ユーザーの診断再提出) と Playbook 取り込み・Bag アイテム作成経路の判断 → develop へ PR → main タグでリリース。 | まはー (実機確認) | ブランチ hotfix/v0.3.1 | 2026-09-01 |
