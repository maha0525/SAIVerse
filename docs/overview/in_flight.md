...
| 🟠 実装中 | Track 撤廃 (最後の住人たちの引っ越し) | 順序①の範囲は §7.4 で確定 (裁定 5 点決着 — v1 メタ判断の退役 + alert 直結化。social/生きる目的は受け皿なし、スペル語彙と選択肢系は④へ)。次 = 順序①の実装 (v1 退役・熟慮経路の on_event 流用・「いまの活動」の出来事化・テストとドキュメント追従)。 | 私 (順序①の実装) | [track_retirement.md](../intent/track_retirement.md) §7 | 2026-08-14 |
