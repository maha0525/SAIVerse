...
|---|---|---|---|---|---|
| 🟣 検証待ち | ペルソナが話すモデルの決め方と、設定を変えたときの反映 (再起動不要・無いモデルで代わりに動かさない) | 実装・検収・レビュー (手元のモデルと Codex 二巡) まで済み、develop への PR ([#294](https://github.com/maha0525/SAIVerse/pull/294)) を出した。次 = まはーの実機で、設定を変えたその場で反映されることと、切り替えられなかった人や保存を断った知らせの文面を確かめ、PR の内容を見てマージする。 | まはー (実機検証と PR) | [intent](../intent/persona_model_selection.md) | 2026-09-12 |
