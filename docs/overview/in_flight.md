...
| 🟡 実装待ち | 知覚消費点の Beat 頭化 + フィード入室配送 | 設計裁定済み — 消費点を Pulse の頭から Beat の頭へ精密化し、スペルが世界を変えた帰結を同ラウンドで知覚させる。次 = SEA runtime の消費点移設 → フィード側二点 (入室配送の相乗り / 設置物欄の描画) → 時間割・フィードの実機検証を再開。 | 私 (実装) | [perception_buffer.md](../intent/perception_buffer.md) §4.2 / [issue](../issues/feed_arrival_pulse_cannot_see_articles.md) | 2026-08-08 |
