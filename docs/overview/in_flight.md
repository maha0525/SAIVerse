...
|---|---|---|---|---|---|
| 🟡 実装待ち | 送る量の一系統化と会話以外の内容の節約 (v0.3.12 本丸) | 設計はプリセットの数字まで全部確定した。次は実装 — 縮める規則・しきい値の一系統化と廃止・プリセットの画面・正典の書き直しを一つの束で進める。 | まはー (実装開始の GO) | [presented_context_reduction.md](../intent/presented_context_reduction.md) | 2026-09-09 |
...
| 🟣 検証待ち | 知覚が多いと「残す量」と「整理を始める量」を両方満たせない状況への対応と、既定値の見直し | 水位側は v0.3.8 として配布済み、残っていた上限系 (知覚の二水位上限 + 省略の印 + 保存時検査) も v0.3.9 主案件の行と同じ束で実装済み。次 = v0.3.9 の実機検証に相乗りして、知覚が大きい環境で上限が効くことを確認する。 | まはー (実機検証) | [issue](../issues/watermarks_unsatisfiable_when_perception_is_large.md) / [metabolism.md](../concepts/metabolism.md) | 2026-09-05 |
...
| 🟣 検証待ち | Unity Gateway の削除 (認証なしで全ネットワークインターフェースに開いていた) | 削除と、呼ぶツールが見つからない手順の扱いの修正は、レビューの収束・テスト全体・隔離環境での起動確認まで済んだ。次 = まはーが develop 向けの PR を確認してマージする (v0.3.12 には入れない)。 | まはー (PR 確認) | [issue](../issues/unity_gateway_removal.md) | 2026-09-11 |
