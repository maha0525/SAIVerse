...
|---|---|---|---|---|---|
| 🔵 設計中 | ペルソナが話すモデルの決め方と、設定を変えたときの反映 (再起動不要・無いモデルで代わりに動かさない) | intent を書き、文書の整合性の検査の結果を反映した。まはーに決めてもらう 5 件 (返事の途中の扱い、モデルとプロバイダの設定変更を含めるか、ほか) の返事を受けてから実装に入る。 | まはー (決定 5 件) | [intent](../intent/persona_model_selection.md) | 2026-09-12 |
