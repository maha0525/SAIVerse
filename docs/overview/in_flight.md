...
| 🟢 実装中 | v0.3.0 の門 (リリース範囲と実施順序) | 束 6 前半 (6a 安い退役 / 6b 判断点の Track 語彙除去 / 6b2 会話経路の Track なし化) は Codex 1 巡 7 件の消し込みまで済んでコミット。次 = 最終便 6c (エピソード書き込み退役 + UI 隠し + 機械写し migration + テーブル退役) を委譲する。 | 私 | [v030_release_gate.md](v030_release_gate.md) / [autonomous_behavior_v3.md](../intent/autonomous_behavior_v3.md) | 2026-08-22 |
| 🟠 実装中 | Track 撤廃 (最後の住人たちの引っ越し) | 会話経路の Track なし化まで実装済み (住人 2・3 の書き手・6・8・12 が退去し、会話の器は saiverse/user_conversation.py へ)。回帰はフルスイート緑。次 = 仲裁の発火条件が「会話が開いていないか」へ揃った影響を、まはーが実機で確認する。 | まはー (実機検証) | [intent](../intent/track_retirement.md) §8 | 2026-08-21 |
