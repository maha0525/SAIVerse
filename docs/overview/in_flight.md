...
| 🟠 実装中 | 被覆補修 (arasuji_levels §16) — 忘却の穴の根治 | 発火は UI 手動・ペルソナごと、事前確認は既存の件数+金額見積もり、モデルは memory weave (まはー裁定)。既存の生成ジョブ・見積もり・削除 API を土台に、作るのは可視化・止め線・窓誕生時の印の三点。次 = 実装 → 単体・隔離検証 → 本番一人目 aifi。 | 私 (実装) | [intent](../intent/arasuji_levels.md) §16 / [issue](../issues/window_refill_rarely_plans_for_starved_personas.md) | 2026-08-31 |
