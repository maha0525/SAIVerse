...
|---|---|---|---|---|---|
| 🟣 検証待ち | ペルソナが話すモデルの決め方と、設定を変えたときの反映 (再起動不要・無いモデルで代わりに動かさない) | develop へマージ済みで、v0.3.13 (release/v0.3.13) に載る。次 = v0.3.13 の動作確認の中で、設定を変えたその場で反映されることと、切り替えられなかった人や保存を断った知らせの文面を確かめる。 | まはー (実機検証) | [intent](../intent/persona_model_selection.md) | 2026-09-12 |
...
| 🟣 検証待ち | Unity Gateway の削除 (認証なしで全ネットワークインターフェースに開いていた) | develop へマージ済みで、v0.3.13 (release/v0.3.13) に載る。次 = v0.3.13 の動作確認の中で、起動後に 8765 番が開かないことを確かめる。 | まはー (実機検証) | [issue](../issues/unity_gateway_removal.md) | 2026-09-11 |
