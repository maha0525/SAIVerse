...
|---|---|---|---|---|---|
| P0 | 記憶・人格境界 | **指摘あり（一次監査完了）** | 2026-07-16 | 合計 **P1×18 / P2×2**（Atlas編纂P1×3含む）。P1×8 / P2×1は修正・回帰固定済み（snapshot restore staging/rollback化は第二陣）、P1×2はまはー裁定で現状仕様として保留。**まはー裁定(2026-07-16): RemoteProxy転送/visitor heard_byのP1×2はmulti-city凍結スコープへ、native import P1×2/P2×1は「同一persona復元と別persona移植の分離」で修正**。残りはMetabolism×3(→Beat/ExecutionContext工事)、native import、Building転記cursor | [一次監査](../handoff/2026-07-12_memory_persona_boundary_audit.md) / [Atlasレビュー](../handoff/2026-07-12_concept_consolidation_code_review.md) |
...
| P1 | Persona / City / Building 分離 | **指摘あり・version境界修正済み・multi-city凍結** | 2026-07-16 | City移動profileへSAIVerse versionを必須化(完全一致要求)。**まはー裁定(2026-07-16): multi-city機能は凍結** — dispatch確定処理未実装/来訪profile署名/RemoteProxy転送のP1×2は修正でなく入口封鎖で対応(実装タスク: inter-city API/pollingの明示無効化)。残りの単一City内finding(移動原子性/occupancy一意性/chat境界/Region/City変更)は修正追跡を継続 | [一次監査](../handoff/2026-07-15_persona_city_building_separation_audit.md) |
