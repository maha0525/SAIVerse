...
2. **会話**: 従来どおり話せる。会話の開始・終了が機構名義の遷移の一行としてログに刻まれる (assistant 名義でない)。沈黙タイムアウトで会話が閉じる。
3. **仲裁の新挙動 (⚠️ 挙動変更)**: 「別の活動中に話しかけたときの応じる判断」が設計どおり発火する。旧実装は残留 running の偶然でほぼ不発だった。v0.3 は自律 OFF なので実害はほぼ出ないはずだが、様子は見る。
...
2. **会話開設 (遷移の一行) の書き込み失敗時に応答を止めない判断の追認** — 6c で「正典がメモリ内状態へ移ったため記録失敗は応答を止めない」に変更した (6b2 消し込み時の逆の裁定を、正典の移動を理由に再反転)。実機で問題が出なければ追認で閉じる。
