...
## 完了 (2026-08-16、まはーの実機検証)

backend の再起動で、見るポイント 1〜4 すべて確認できた。

1. テスタロッサの部屋の履歴 360 件が UI に戻った (2.5 ヶ月ぶり)
2. ナビの部屋の 94 件も自動で取り込まれ、既存 72 件より前に並んだ
   (旧設計では「手動対応が必要」としてバナーを出し続ける予定だった箇所)
3. 起動ログに取り込みの行が出た
4. テスタロッサの次の Pulse で過去ログの一気読みは起きなかった

### レビューの巡回 (3 巡で打ち切り)

| 巡 | 指摘 | 実バグとして消し込み |
|---|---|---|
| 1 | 6 | 4 (cursor 追い越し / 検算の沈黙 / commit 境界 / 痕跡判定) |
| 2 | 3 | 2 (空室の水位漏れ / 巻き戻した部屋のメタデータ) |
| 3 | 5 | 2 (混在実行のメタデータ補完 / 対応表の合流位置) |

打ち切りの根拠は「もう出ないだろう」という予測ではなく、3 巡分の観測:
指摘の的が本流 (ユーザーのメッセージが消える・検算が黙る) から、前提の障害が先に
起きている状態 (旧実装が残した部分取り込み / cursor 行だけが消える / 水位を測れない)
へ移った。後ろ 2 つは意識して選んだ引き換えで intent に記載済み。

### 切り出した issue

- [cursor 取り込みの失敗が握り潰される](../issues/legacy_cursor_import_failure_is_silent.md)
- [message_id を持たない行の照合](../issues/legacy_log_no_id_rows_cannot_be_reconciled.md)
- [状態を変える API に送り元の確認が無い](../issues/api_state_changing_routes_have_no_origin_check.md)

### ローカル LLM レビューは 3 回とも本体が走らなかった

1 回目はコンテキスト超過 (108441 > 102400 トークン)、以降は先客との相席で応答が空。
返ってきたのはすべてサブエージェントの代読で、同じ差分に Codex が実バグ 8 件を
出したことを踏まえると、一次スクリーニングとしては機能していない。
経緯と次回の手順は memory `project_local_llm_review` に追記済み。
