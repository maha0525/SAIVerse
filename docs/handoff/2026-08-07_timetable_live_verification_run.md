...
- [ ] **フィード配送**: フィード施設の Building にペルソナを置き、取得サイクル後に話しかけると「[フィード] 『◯◯』の新着記事」の知覚が入る。記事ビューアの内容と一致 (透明性)。**注記 (2026-08-08)**: 配送はサイクル任せ — 出かけるコマの到着 Pulse に記事が無いのは既知 (バグではない)。visual context の見出し表示は未実装疑いで**検証不能** → [issue](../issues/feed_arrival_pulse_cannot_see_articles.md)
