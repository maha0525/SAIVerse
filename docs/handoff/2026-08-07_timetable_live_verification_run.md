...
- [ ] **`[feed]` の取得ログ**が出る (フィード施設がまだ無い初回は「スキップ」で正常)
...
- [ ] コマ種別が新 7 種 (調べる/絵を描く/日記を書く/随筆を書く/出かける/自室で過ごす/自由時間)。旧種別の過去データ表示が壊れていない
- [ ] **習慣テンプレート** (ペルソナメニュー、ライフ設定の隣): コマ行を作成・保存できる
- [ ] **ライフ設定モーダル**: 起床・就寝・予算・モード上書きが 1 画面。予算欄に最低値ガイドが出る
- [ ] **v1 亡霊の消滅**: 自律行動マネージャーの間隔入力・LifeView 最下部の間隔フォーム・DebugPanel の「自律 Pulse を 1 回」ボタンが消えている
- [ ] ライフ帯が「**活動 N/M回**」表示 + 「**ふりかえり・判断: N回**」の別枠
- [ ] 右サイドバーに**話しかけやすさチップ** (活動中/休憩中)
- [ ] **経験タブ** (メモリーモーダル): エアの実データで索引・統計バッジ・薄い棚の減光・ページの動的合成が見える

**head (会話コンテキストのプレビューで)**:

- [ ] 「**行ける場所**」の節 (現在地の直後あたり) と「**進行中のことと、やりたいこと**」の節 (生きる目的の直後) がある。Track が二度出ていない
- [ ] (相乗り) head の末尾側に直近の記憶が戻っている / 送信前プレビューに厚い生ログが見え、開き直しと一致する / 二重表示なし

**フィード系**:

- [ ] グローバル設定 → フィードタブ: 施設作成 (Building+プリセット選択) → 購読と健康状態が一覧に出る
- [ ] 「今すぐ取得」→ 記事ビューアに当日の実在記事 (Zenn/GIGAZINE 等と照合)
- [ ] 素の URL (例 `publickey1.jp`) を貼って自動発見が働く

**その他の相乗り**:

- [ ] Chronicle タブ: エリスの全 513 件が並び、切り詰めバナーが消えている
- [ ] チャットオプション「データ送信量の管理」が読み取り専用の状態表示になっていて、水位バーとプレビューが整合
- [ ] 会話を一度開始して「1件」ダイアログが出ない
...
- [ ] **フィード配送**: フィード施設の Building にペルソナを置き、取得サイクル後に話しかけると「[フィード] 『◯◯』の新着記事」の知覚が入る。記事ビューアの内容と一致 (透明性)。visual context にも購読タイトル・最新見出し
