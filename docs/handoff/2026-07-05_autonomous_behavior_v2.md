...
残り: まはーレビュー（設計判断3件は全て決定済み、下記）→ §4-7 の活性化配線。

### 実 LLM 実行3回の改善サイクル（新聞+生データのセットレビュー方式）

- 2回目の実行で発覚→修正: 2f129f6 施設移動が実際には移動していない / c1acae3 スペル args parse 失敗の無言握り潰し（差し戻し+複数行 JSON 救済） / 85a3d45 ペルソナ可視時刻が実時刻（仮想クロック無視）
- 3回目の実行で発覚→修正: d08e86d 時間割組み替えの不当却下（消化済み区間との境界で昇順違反）+ 却下の無通知 + completed タスクの再 done 経路2つ / 44d27ea DES 空回り（実時刻由来の定期予約を ScenarioPlayer が cancel_all）
...
1. ~~計画時の ref 選択ミス~~ → **解決済み (f8a2f2f)**。真因はモデルでなく提示の不整合: やりたいこと候補一覧が欲求を生の task:N で表示し、enum は desire:N だったため、構造化出力の制約デコードが task:2 を書こうとして enum 内の task:1 に滑落していた（まはーの「enum に task:1 しか無かった説」が的中）。提示 ref を to_desire_ref で enum と統一し、表示と enum の整合を回帰テストで固定
...
3. ~~裁定独白の出典作話~~ → **解決済み (9d8213b、まはー決定: プロンプト明示)**。post_session / day_close の状況テキストに「実際に参照・取得した情報源だけを出典として書く」を明示
