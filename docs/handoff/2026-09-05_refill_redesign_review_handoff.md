...
## 裁定済み (2026-09-05 このセッション内で解決)

- **最終防衛ライン (床) を作業セッションにも張るか** → **張らない**で確定 (まはー「作業セッション対応のために労力を割かなくていい」)。
- **作業セッションの生死** → まはー「作業コマも生活コマも無くなった」— 退役済みの疑いが濃い (呼び出し元と landscape の記述はコード・docs の撤去漏れの可能性)。念入りな調査を [work_session_liveness_audit.md](../issues/work_session_liveness_audit.md) に起票済み (実機検証より後・7/30 棚卸しと続けてやると効率が良い)。配線済みの読み戻しは調査の結論まで残す。
