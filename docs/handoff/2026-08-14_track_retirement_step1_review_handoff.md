...
- **私の裁定**: **旧経路と同挙動＝順序①での退行ではない**（旧 alert 仲裁も non-running 分岐でしか発動しなかった）。ただし裁定 B ①の不変条件「開いている出来事 ≠ 会話のときのユーザー発話は判断を経由」には未達。**busy 判定を status 分岐の前へ出すと「呼びかけへの即応答」が減る方向の挙動変更**になるため、直すか（＝作業中は残留 running でも仲裁を通す）はまはーの製品判断を仰ぐこと。機械的に直してよい種類ではない。
...
1. F2 → F5 → F6 → F3 → F4 の順を推奨（小さい確実な修正から。F3/F4 は設計を伴う）。F1 は着手前にまはーへ挙動変更の裁定を仰ぐ。
