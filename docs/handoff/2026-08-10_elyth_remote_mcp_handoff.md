...
**残り**: 手順 10 (画面から行がその場で消えるか) の再確認と、手順 11。手順 11 は「畳んでいる最中の二重起動」を見るもので、窓が短く手では狙えない — 隔離テストが押さえているので、実機では「無効化直後に話しかけても変なことにならない」だけ見れば足りる (§10 の 11 に書き直した)。バックエンドの再起動が要る。
