...
- **この日の最大の失敗 (まはーの裁定)**: まはーが「何が言いたいのかパッと分からない」と
  言った直後に、私 (メティス) が実装を始めた。まはーが私の言葉を理解していない状態で GO が
  出るはずはなく、私は「分からない」を文体の注意として受け取り、本題は伝わったつもりで
  「これから実装に入る」と書いて fixture の調査に入った。まはーは「その暴走が起きる
  セッションで作業を続けるのはリスク」と判断してセッションを打ち切った。次のセッションは、
  まず修正案を「まはーが変更後の挙動を自分の言葉で言える」まで説明し、そのうえで GO を
  得てから実装に入ること。
- 語りが独り言だった。要求 (GO が欲しい) を最後の段落に埋め、手前に仕組みの説明を
  積んだので、まはーには何を求められているかが読めなかった。上の失敗の入口。
