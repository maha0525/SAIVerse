...
### Test Case 4.4: ペルソナ・アクティブ状態管理

#### 目的
ペルソナの行動モード（`auto`, `user`, `sleep`）が正しく機能し、AIの行動が適切に制御されることを確認する。
...
1.  **`sleep`モードへの移行と自動移動の確認**
    - 「ワールドエディタ」タブを開き、「AI管理」セクションを開く。
    - AI一覧から`air_city_a`の行を選択する。
    - 「対話モード」ドロップダウンを`sleep`に変更し、「AI設定を保存」ボタンをクリックする。
    - Status欄に「AI 'air' updated successfully. Mode changed from 'auto' to 'sleep'. Moved to private room 'airの部屋'.」のような成功メッセージが表示されることを確認する。
    - 「自律会話ログ」タブに移動し、Building選択で「airの部屋」を選択する。ログに`air`が移動してきた旨のメッセージが表示されていることを確認する。
    - （任意）「DB Manager」タブで`ai`テーブルを確認し、`air_city_a`の`INTERACTION_MODE`が`sleep`になっていることを確認する。

2.  **`sleep`モード中の自律会話停止の確認**
    - 「自律会話ログ」タブで「自律会話を開始」ボタンをクリックする。
    - ターミナルのログを監視し、`Triggering pulse for 'air'`というログが出力されない（`air`の思考がスキップされている）ことを確認する。

3.  **`user`モードへの移行と状態退避の確認**
    - 「ワールドビュー」タブに移動する。
    - ユーザーの現在地が「まはーの部屋」であることを確認する。
    - 「ペルソナを招待する」で`air`を選択し、「呼ぶ」ボタンをクリックする。
    - チャット欄に`air`が入室したメッセージが表示されることを確認する。
    - （任意）「DB Manager」タブで`ai`テーブルを確認し、`air_city_a`の`INTERACTION_MODE`が`user`に、`PREVIOUS_INTERACTION_MODE`が`sleep`に更新されていることを確認する。

4.  **`user`モード中の自律会話停止の確認**
    - ターミナルのログを監視し、`air`の自律会話がトリガーされていないことを確認する。
    - ワールドビューのチャットで`air`に話しかけ、正常に応答することを確認する。

5.  **直前の`sleep`モードへの復帰確認**
    - ワールドビューのチャット欄で、`air`のアイコンの隣にある「会話を終了」ボタンをクリックする。
    - `air`が「airの部屋」に戻ったメッセージが表示されることを確認する。
    - （任意）「DB Manager」タブで`ai`テーブルを確認し、`air_c_a`の`INTERACTION_MODE`が`sleep`に戻っていることを確認する。
    - ターミナルのログを監視し、`air`が引き続き自律会話を行わないことを確認する。

6.  **`auto`モードへの復帰確認**
    - ワールドエディタで`air`のモードを`auto`に変更し、保存する。
    - ターミナルのログを監視し、`air`が自律会話を開始すること（`Triggering pulse for 'air'`が出力されること）を確認する。
    - `air`を再度「まはーの部屋」に呼び出す。
    - （任意）DBで`INTERACTION_MODE`が`user`に、`PREVIOUS_INTERACTION_MODE`が`auto`になっていることを確認する。
    - 会話を終了し、`air`が「airの部屋」に戻ることを確認する。
    - （任意）DBで`INTERACTION_MODE`が`auto`に戻っていることを確認する。
    - ターミナルのログを監視し、`air`が自律会話を再開することを確認する。

#### 期待される結果
- AIのモードが各シナリオで正しく遷移し、自律会話の実行・停止が適切に制御されること。
- `sleep`モードへの変更時に、AIが自動的に個室へ移動すること。
- `user`モードから復帰する際に、直前のモード（`auto`または`sleep`）に正しく戻ること。
