...
# ---------------------------------------------------------------------------
# desire (旧「やりたいこと候補プール」note_type, autonomous_desire.md §5)
#
# P3c-0 (desire 正規化) で候補の実体は親なし+stage='candidate' の目的ノードへ
# 移行し、この note_type を新規作成する経路 (旧 ensure_desire_note) は撤去済み。
# 残るのは「ユーザーが note_create で作れない」という拒否側の検証のみ
# (sea/head_pipeline/sections/open_notes.py の読み取り専用消費は別途 P3c① で
# 退役予定)。
# ---------------------------------------------------------------------------
