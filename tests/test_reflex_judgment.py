...
- 答える側はモデルの役割「反射判断」への割り当てで決まる (env
  ...
  jev 互換でない protocol は「使えなかった」 (第 2 段まで LLM 変換は無い)。
...
def test_non_jev_protocol_is_unavailable_and_says_why(monkeypatch, caplog):
    """通常の LLM プロトコルのモデルが割り当てられていたら「使えなかった」。

    第 1 段には質問をプロンプトへ変換する層が無い。黙って効かないままにせず、
    WARNING に理由を出す。
    """
    caplog.set_level("WARNING", logger="saiverse.reflex_judgment")
    _set_configs(monkeypatch, {MODEL_KEY: _model_config(protocol="gemini_native", provider="gemini")})
