...
#:
#: まはー裁定 (2026-08-24、issue の「22 欄の隔離実験」の節): **現状維持**。
#: 危ない並び 5 型を隔離実験で 90 回叩いて数字ループの再現がなかったため、
#: これらの欄は型を変えずにこの一覧の見張りの下に置く。例外は経路 B の 2 欄
#: (起床判断は v0.4 の配線前、夜間編纂は編纂の再開前に直す)。本番で数字
#: ループが実際に観測されたら、issue の実験手順で裏を取ってから型を変え、
