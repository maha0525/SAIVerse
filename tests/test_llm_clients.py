...
class TestLLMClients(unittest.TestCase):
    ...
    @patch('llm_clients.openai.OpenAI')
    @patch('llm_clients.openai._validate_required_schema_keys', new=None)
    def test_openai_client_generate_with_schema_falls_back_when_validator_unavailable(self, mock_openai):
        mock_client_instance = MagicMock()
        mock_openai.return_value = mock_client_instance
