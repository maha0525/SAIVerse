...
class PulseDispatcher:
    ...
    def dispatch_user_utterance(
        ...
        **フォールバックは「まだ何もしていない失敗」に限る** (2026-08-21 Codex
        ...
        いたが、新規会話では会話状態と main_line 起動の**後**に沈黙タイマーの
        ...
        - どちらでもない: 送出する (黙って返すより見えて失敗する方が正しい)。

        ⚠ 束 6c (2026-08-22) で「会話の出来事を開けなかった」分類は対象消滅した
        (autonomous_behavior_v3.md §7)。「いま会話中か」の正典はメモリ内状態だけが
        持ち、会話の始まり / 終わりはどこにも記録しない (2026-08-23 裁定) ので、
        会話開設に DB 書き込みは絡まない。三つ目の分岐は残す — 受け口が将来また
