...
#:
#: 2026-09-09 に target 4万 → 2万・high 12万 → 6万 (まはー裁定、docs/intent/
#: presented_context_reduction.md 「まはーが決めること」1)。旧既定はユーザーの
#: 反応から**既定としては大きすぎた**ので、その組はプリセット「多い」へ降格した。
#: 2万字は日本語の会話で数十往復ほど。差 4万は、畳みの発火を「たまに・まとめて」に
#: 保てる幅 (幅が狭いほど整理の LLM 呼び出しとキャッシュの読み直しが増える)。
#:
#: 廃止したキーは黙って無視される (エラーにしない) — 旧 DB / 旧モデル JSON との
#: データ互換:
#:
#: - ``metabolism_low_chars`` (旧三水位の低水位、2026-09-04 廃止): 現役の役割が
#:   「anchor 未確立時の初期読み込み量」だけに縮んでおり、発話直前の最終防衛ライン
#:   (arasuji_levels.md §15-5) が窓を残す量まで埋め直すため効く場面が無かった。
#: - ``perception_target_chars`` / ``perception_high_chars`` (知覚の提示上限の
#:   二水位、2026-09-05 導入 → 2026-09-09 廃止): 「会話を畳んでも減らせない大物」
#:   だった操作通知と現在地でない部屋の様子が、Metabolism の瞬間の縮み
#:   (sai_memory/presented_reduction.py) で減るようになったため、専用のしきい値を
#:   持つ理由が消えた。しきい値は「残す量 / 上限」の一系統だけになった。
BUILTIN_METABOLISM_TARGET_CHARS = 20_000
BUILTIN_METABOLISM_HIGH_CHARS = 60_000
...
#: 全体設定の画面が出す水位のプリセット (2026-09-09 まはー裁定、数字まで確定 —
#: docs/intent/presented_context_reduction.md 「まはーが決めること」1)。
#:
#: 「デフォルト」の段は組み込み既定そのものを参照するので、ここと
#: :data:`BUILTIN_METABOLISM_DEFAULTS` は食い違えない (契約テスト
#: tests/test_metabolism_global_defaults.py が固定する)。
#: どの段も 残す量 : 上限 = 1 : 3。幅 (上限 − 残す量) が整理の頻度を決めるので、
#: 小さい段ほど整理の LLM 呼び出しとキャッシュの読み直しが増えうる — 画面は
#: この注意書きを添えて出す。
#:
#: 定義はこの一枚だけに置き、全体設定の GET (``/api/config/metabolism-defaults``)
#: が応答へ載せて画面が読む。画面側に数字を書き写すと、既定を動かしたときに
#: 「プリセットのデフォルトだけ古い」という食い違いが生まれる。
METABOLISM_PRESETS: tuple[dict[str, Any], ...] = (
    {
        "id": "large", "label": "多い",
        "metabolism_target_chars": 40_000, "metabolism_high_chars": 120_000,
    },
    {
    ...
    {
        "id": "small", "label": "少ない",
        "metabolism_target_chars": 10_000, "metabolism_high_chars": 30_000,
    },
)

#: 全体設定の水位既定 (2026-09-03)。三層の真ん中:
#: 組み込み既定 < **全体設定** < モデル定義。キー無し / None = 未設定
#: (組み込み既定に落ちる)。
#:
#: 真実の置き場は DB (user_settings.METABOLISM_*_CHARS)。このモジュールは
...
#: (``resolve_metabolism_watermarks``) ので、差し替えの途中で「target は新・
#: high は旧」の混ざった組を観測しない (Codex 指摘 2026-09-03)。外から読むときは
#: ``get_global_watermark_defaults()`` を使う (この変数名は private)。
#:
...
def set_global_watermark_defaults(values: Mapping[str, int | None]) -> None:
    ...
    廃止済みのキー (``metabolism_low_chars`` / ``perception_target_chars`` /
    ``perception_high_chars``) が混ざっていても黙って無視する (旧 DB / 旧
    クライアントとのデータ互換)。

    新しい不変写像を作って一回の代入で差し替える — 読み手が持っている古い一枚は
    変化しない。二つの水位を一枚に収めているので、両方を変える保存でも
    「片方だけ新しい組」を読む瞬間が生まれない。
    """
    global _GLOBAL_WATERMARK_DEFAULTS
    _GLOBAL_WATERMARK_DEFAULTS = MappingProxyType({
        key: _positive_int_or_none(values.get(key))
...
def effective_watermark_defaults_from(
    ...
    """全体既定の一枚を「キー無しのモデルが従う実効既定」(二キー) に均す。
...
def resolve_metabolism_watermarks(model: str) -> tuple[int | None, int | None]:
    ...
    (chronicle_eviction.md §4)。モデル定義に残っている廃止済みのキー
    (``metabolism_low_chars`` / ``perception_*_chars``) は読まない = 黙って
    無視される。
...
def compose_watermark_map(
    config: Mapping[str, Any], global_defaults: Mapping[str, int | None],
) -> dict[str, int | None]:
    """二水位を**素のまま** (実行時の救済を掛けずに) キー付きで組む。保存時検査の入力。

    実行時の解決 (:func:`compose_metabolism_watermarks`) と同じ関数
    (:func:`compose_watermark`) を通す — 検査と実行が二本に分かれると、検証は
    通るのに実行時は壊れる組が保存できる。
    """
    effective = effective_watermark_defaults_from(global_defaults)
    return {
