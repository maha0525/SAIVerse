...
def collect_episode_digest_index(
    manager: Any,
    persona_id: str,
    memory_conn: Any,
) -> Dict[str, tuple]:
    """digest 確定済み episode の ``{episode_ref: (digest_message_id, 本文)}``。

    Chronicle の episode 整列 (W4、experience_structure.md 工程(2)) の恒等転写
    材料。world DB の Episode (closed + DIGEST_REF='message:{id}') を index に
    し、digest 本文は persona の memory.db (``memory_conn``) から引く。本文が
    引けない ref は index に入れない — その範囲は通常材料として圧縮される
    (digest が失われた土地を未編纂のまま放置しない)。

    呼び出し元: sea/session_lifecycle (Metabolism) と
    api/routes/people/arasuji.py (手動生成ジョブ) — 一点管理。
    失敗は空 dict に degrade する (episode index が無くても編纂は
    バッチ降格 §5 で成立する)。
    """
    if (
        not persona_id
    ...
    try:
        ...
        try:
            rows = (
                db.query(Episode.SHORT_ID, Episode.DIGEST_REF)
                .filter(
                    Episode.PERSONA_ID == persona_id,
                    Episode.STATUS == STATUS_CLOSED,
                    Episode.DIGEST_REF.isnot(None),
                )
                .all()
            )
        finally:
            db.close()
        for short_id, digest_ref in rows:
            if short_id is None or not digest_ref:
                continue
            ref_str = str(digest_ref)
            if not ref_str.startswith("message:"):
                continue
            mid = ref_str.split(":", 1)[1].strip()
            if mid:
                ref_to_mid[f"episode:{short_id}"] = mid
    except Exception:
        LOGGER.warning(
            "[episode] failed to query digest index (persona=%s)",
    ...
    out: Dict[str, tuple] = {}
    try:
        for ep_ref, mid in ref_to_mid.items():
            row = memory_conn.execute(
                "SELECT content FROM messages WHERE id = ?", (mid,)
            ).fetchone()
            if row and row[0] and str(row[0]).strip():
                out[ep_ref] = (mid, str(row[0]).strip())
    except Exception:
        LOGGER.warning(
            "[episode] failed to load digest bodies (persona=%s)",
            persona_id, exc_info=True,
        )
        return {}
    return out
