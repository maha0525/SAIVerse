"""purpose_step: 目的ノードの 1 ステップの状態を更新する (旧 task_update_step 後継、P2c-1)。

配列全置換 (分解) は ``purpose_decompose`` — 引数の系統が別なので 2 本に
分けた (P2c-1 実装判断)。挙動・引数は旧 task_update_step と 1:1。
"""
...
def purpose_step(
    ...
    """Update the status of a step within the purpose node addressed by ``node_ref``."""
    ...
    try:
        ...
        ref = updated_task.get("task_ref") or node_ref
...
def schema() -> ToolSchema:
    return ToolSchema(
        name="purpose_step",
        description=(
            "目的ノード（task:N）の中の1ステップの状態とメモを更新します。"
            "ステップへの分解（全置換）は purpose_decompose を使ってください。"
        ),
        parameters={
            "type": "object",
            "properties": {
                "node_ref": {
                    "type": "string",
                    "description": "対象の目的ノードの参照（例: task:5）",
                },
                "step_position": {
                    "type": "integer",
                    "minimum": 1,
                    "description": "ノード内でのステップの位置（1始まり）",
                },
                "status": {
                    "type": "string",
                    "enum": sorted(ALLOWED_STATUSES),
                    "description": "ステップの新しい状態",
                },
                "notes": {
                    "type": "string",
                    "description": "任意: このステップへのメモや所感",
                },
                "auto_advance": {
                    "type": "boolean",
                    "default": True,
                    "description": (
                        "true のとき、ステップの完了/スキップ時に次の未完ステップを"
                        "自動的にアクティブにします"
                    ),
                },
            },
            "required": ["node_ref", "step_position", "status"],
        },
        result_type="string",
        spell=True,
        spell_display_name="目的のステップを更新",
    )
