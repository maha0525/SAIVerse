...
class SpellListSection:
    ...
    def render(self, snapshot: SpellListSnapshot) -> Optional[RenderedSection]:
        ...
        lines: list[str] = [
            ...
            "失敗した場合と、通常の /spell と同じ発言に混ざっている場合は、通常通り結果が返ってきます。",
            "基本は /spell を使い、quick_spell は結果を確認する必要がないと確信できるときだけ使ってください。",
            "",
            "### 複数スペルの同時使用",
            "1回の発言に複数の /spell 行を書くと、書いた順にまとめて実行され、結果は一度に返ってきます。",
