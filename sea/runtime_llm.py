...
from sea.runtime_utils import _format, _is_llm_streaming_enabled, event_building_id
...
#: 「建物には入ったが、記憶にはまだ入っていない本文」を Beat の出口へ渡す state キー。
#:
#: 置くのはスペルループの退避書き込みだけ — 下書き行が無い (作れなかった / もう
#: 確定した) 周の本文を建物へ直接書いたうえ、その周の記憶の書き込みが失敗した回。
#: この形のまま Beat が死ぬと「建物にはあるのに本人が覚えていない」発言になるので、
#: 出口の補填 (`_backfill_memory_on_beat_death`) が発火条件と本文の両方にこれを使う。
BEAT_BODY_UNMEMORIZED_KEY = "_beat_body_unmemorized_text"
...
#: 保存した発言の形と、その書き足し口。止まりかけた生成は、自分が保存した
#: 発言の形 (途中で切れた / スペルの実行が終わる前 / スペルの結果を受け取った
#: 後) と「この後で話が止まった」を「最後に保存した発言」の記録に書き足す
#: だけで、印と通告は置かない — それは返事の実行の一番外側の後始末
#: (sea/reply_stop_exit.py) が一回だけ行う。設計: docs/intent/reply_stop_exit.md
from .runtime_emitters import (  # noqa: E402
    SAVED_FORM_COMPLETE,
    SAVED_FORM_CUT,
...
def _withdraw_placeholder_with_signal(
    runtime: Any,
    persona: Any,
    building_id: str,
    msg_id: str,
    *,
    ...
    """本文が一つも生まれなかった下書き行を取り下げ、画面の吹き出しも片付ける。

    ``_finalize_speak_with_signal`` の対。空文字で確定すると本文の無い記録が
    建物とペルソナのログに永続し、下書きの印が倒れて孤児掃除の網からも外れる
    (docs/issues/pulse_beats_merge_into_single_record.md の H-1)。

    画面側は、行が消えた回だけ取り消し (``streaming_discard``) を流す。行が
    残った回に流すと、記録は残っているのに吹き出しだけ消える食い違いになる。
    """
    withdrawn = runtime._withdraw_speak_placeholder(persona, building_id, msg_id)
    if withdrawn and event_callback:
        try:
            event_callback({
            ...
            LOGGER.warning(
                "[sea][pipeline] could not deliver the discard for the "
                "withdrawn draft row (msg=%s)", msg_id, exc_info=True,
            )
    return withdrawn


def _withdraw_if_nothing_was_said(
    runtime: Any,
    persona: Any,
    *,
    building_id: str,
    ...
    """一言も生まれなかった下書き行なら取り下げる。取り下げたら ``True``。

    「本文ゼロ」は二つ揃って初めて言える — 回収できた部分文が空で、かつその行
    から sub-speak が一度も飛んでいない (``sub_seq`` が 0)。片方でも欠けると
    画面か音声には既に言葉が流れているので、取り下げるとその言葉が消える。

    ``sub_seq`` は **その行の** 連番を渡すこと。Beat ごとに新しい下書き行を
    作る経路では 2 周目以降の連番が 0 から積み直されるので、Beat を跨いで
    持ち回った値を渡すと、一言も流れていない行が「流れた」と誤判定される。

    ``False`` になるのは二通り — 判定に当てはまらない回と、取り下げを断られた
    回 (誰かが既に記憶へ転記していた行は消せない)。どちらも呼び出し元は従来
    どおりの確定へ落とす。空文字での確定は本文の無い記録を永続させるが、確定
    しない方 (= 永久に残る孤児の下書き行) よりは掃除の当てがある。

    この判定を共有するのは、締めの Beat が本文を生まなかった回 (正常復帰) と
    Beat が例外で死んだ回の後始末が、同じ「空の記録が確定して永続する」形を
    持つため (docs/issues/pulse_beats_merge_into_single_record.md の H-1 / N-1)。
    """
    if (partial_text or "").strip():
        return False
    if int(sub_seq or 0) > 0:
        return False
    return _withdraw_placeholder_with_signal(
        runtime, persona, building_id, msg_id,
        pulse_id=pulse_id,
        node_id=node_id,
...
def _occupants_snapshot(runtime: Any, building_id: str) -> Optional[List[str]]:
    """いまその部屋に居る面々の写し。引けなければ ``None``。

    Beat の記録の「聞いた人」(heard_by) と「同席者」(with) は、その Beat が
    **始まった時点**の在室者で作る (契約 3)。記録を書くのが後になる経路
    (スペルループを使わない 2 経路) のために、開始時点でここで写しを取る。
    None は「写しが無い」— 受け手は書き込み時点の在室表へ倒す。
    """
    try:
        occupants = runtime.manager.occupants.get(building_id, [])
    except Exception:
        LOGGER.debug(
            "[sea][spell] could not snapshot the occupants of %s", building_id,
            exc_info=True,
        )
        return None
    return [str(oid) for oid in (occupants or []) if oid]
...
def _emit_say_and_capture(
    ...
    *,
    ...
    ``occupants_snapshot``: 「聞いた人」を導く在室者の写し (Beat 開始時点)。
    None なら書き込み時点の在室表 (従来挙動)。
    """
    bmsg = runtime._emit_say(
        persona, eff_bid, text, pulse_id=pulse_id,
        metadata=metadata if metadata else None,
        event_callback=event_callback,
        occupants_snapshot=occupants_snapshot,
    )
...
def _emit_beat_segments(
    ...
    *,
    ...
    """Beat ごとのセグメントを、それぞれの部屋へ 1 件ずつ記録する。

    ストリーミングを使わない 2 経路 (tool モード / 全文一括) の共通処理。
    スペルループが周ごとに割った本文を、**その周の生成が始まった部屋** へ順に
    ...
    「聞いた人」(heard_by) は各セグメントが持つ在室者の写し (= その Beat が
    始まった時点) から導く (契約 3)。この経路は全 Beat をループ完了後に書く
    ので、書くときに在室表を引くと Beat 1 の記録にまで「ループが終わった
    時点の在室者」が載る。

    戻り値は「1 件でも建物へ**書けた**か」— 呼び出しの回数ではなく、建物の記録
    ...
    for idx, segment in enumerate(segments):
        ...
        if idx == 0 and strip_prefix and seg_text.startswith(strip_prefix):
            seg_text = seg_text[len(strip_prefix):].lstrip("\n")
        if not seg_text.strip():
            continue
        prepared.append((seg_text, segment))
    if not prepared:
        # 1 件も書かない回でも、締めのメタデータの組み立ては 1 回走らせる。
        # ``_build_say_metadata`` は state から自動想起の本文 (`_auto_recall_text`)
        # を pop で取り出す — ここで呼ばないと消費されず、次のノードの発言に
        # 前の Beat の想起が付いて流れる。
        final_metadata_factory(None)
        return False
...
def _finalize_beat(runtime, beat: BeatExecution) -> None:
    ...
    if memorize_config and state.get("_beat_memorized"):
        # 止まった生成の保存 (`_save_cut_utterance`) がこの Beat の本文を
        ...
        if not playbook.name.startswith(("meta_", "sub_")):
            ...
            if event_callback:
                event_callback({
                    ...
                    "pulse_id": state.get("_pulse_id"),
                    # 発火時点の部屋。名乗らないと、別の部屋で進んでいる Beat の
                    # 活動記録が閲覧中の部屋の吹き出しに混ざる。
...
async def _consume_pipeline_stream(
    ...
    *,
    ...
    emit_building_id: Optional[str] = None,
    ...
    - ``emit_building_id``: この stream が属する下書き行の部屋。sub-speak の
      配送先と、画面イベントに載せる ``building_id`` に使う。省略すると現在地を
      1 回だけ引いて使う (従来挙動)。Beat ごとに下書き行を切り替えるスペル
      ループは、その周の行を作った部屋を明示で渡す。
...
def _record_interruption_notice(
    ...
    *,
    ...
    """中断があった事実を、建物の記録へ host 名義で一行置く。置けたら True。
    ...
    呼ぶのは返事が止まった回の後始末 (sea/reply_stop_exit.py) だけで、文面
    (``content``) もそちらが発言の形と原因から選ぶ。書き込みに失敗しても
    例外は投げない (通告は救済であって、返事の本体ではない) が、置けたか
    どうかは返す — 印と通告は必ず対 (不変条件 2) なので、置けなかった回は
    呼び出し元が印を取り下げる。
    """
    try:
        ...
        written = persona.history_manager.add_to_building_only(
            building_id,
            {"role": "host", "content": content},
            heard_by=heard_by,
        )
        # 置けたかの判定は、戻り値に DB 採番の message_id が付いているか
        # (builtin_data/tools/tell.py と同じ裁定)。add_to_building_only は
        # DB 挿入が再試行の後に失敗しても例外を投げず、渡した dict を id 無しで
        # 返す — 例外が出なかったことは、置けた証拠にならない。
        if isinstance(written, dict) and written.get("message_id"):
            return True
        LOGGER.warning(
            "[sea][pipeline] the interruption notice was not persisted to the "
            "building (msg=%s building=%s)", msg_id, building_id,
        )
        return False
    except Exception:
        LOGGER.warning(
...
def _save_cut_utterance(
    *,
    ...
    sub_seq: int,
    text: str,
    form: str = SAVED_FORM_CUT,
    memory_text: Optional[str] = None,
    node_id: Optional[str] = None,
) -> int:
    """止まった生成の、まだ保存していない言いかけを発言として保存する。返すのは
    進めた後の ``sub_seq``。

    **止まった回は、この関数を通らないと発言が消える。** 下書き行 (placeholder)
    は本文が空のまま作られ、確定して初めて中身が入る。確定しないと content が空の
    ままデータベースに残り、画面はそれを描かないので、**本人が喋った内容がどこにも
    無くなる** (2026-08-26 実機で発生)。

    ここでやるのは**保存だけ** — 下書き行を本文で確定し (同時に voice-tts の
    ストリームも閉じる)、言いかけた本文を本人の記憶へ書き、保存した発言の形と
    「この後で話が止まった」を「最後に保存した発言」の記録に書き足す。
    「言い切っていない」印と中断の通告は置かない — それは返事の実行の一番外側の
    後始末 (sea/reply_stop_exit.py) が、すべての書き込みの後で一回だけ置く
    (docs/intent/reply_stop_exit.md)。止まった場所ごとに印と通告を置くと、内側と
    外側の順序が狂い、取り下げ・付け替えの機構が要る形に戻る。

    ``form``: 保存する本文の形。途中で切れた本文 (既定) / スペルの実行が終わる
    前に止まった周の本文 / 言い切ってから止まった本文。

    ``memory_text``: 記憶へ書く本文。既定 (None) は ``text`` そのもの。空文字を
    渡すと書かない — スペルの周の本文は周の頭でもう記憶に書かれている
    (スペル結果の HTML を記憶へ入れないためにも、行の本文とは別に渡す)。
    記憶には、途中で切れた本文のときだけ「言い切っていない」印を付ける — 後から
    想起しても言い切ったものとは扱われないように。
    ...
    if memory_text is None:
        memory_text = text
    if body and form == SAVED_FORM_CUT:
        # 同じ Beat の後段 (スペル無効のペルソナは止められても例外なしで
        # `_finalize_beat` まで進む) が記憶へ書くときの印。記録の建物側の印は
        # 後始末が置く。
    ...
    if msg_id:
        sub_seq += 1
        try:
            _saved = _finalize_speak_with_signal(
            ...
            # 「最新の発言 id」と記録への書き足しは、保存できた回にしか進めない
            # — 保存されていない行を後続ツールや後始末に参照させない。
            if getattr(_saved, "status", None) == "saved":
            ...
            LOGGER.warning(
                "[sea][pipeline] saving the stopped utterance raised; "
                "placeholder may remain unconfirmed",
                exc_info=True,
            )
        LOGGER.info(
            "[sea][pipeline] Stopped mid-reply: saved the draft row "
            "msg=%s seq=%d len=%d form=%s",
            msg_id, sub_seq, len(text or ""), form,
        )
        if event_callback and body:
            # 流し込みの吹き出しを閉じる合図 (止まった Beat はこの先で例外を
            # 投げるか黙って閉じるので、通常の完了の合図は届かない)。
            # 「言い切っていない」の印は載せない — 画面のボタンは、返事の
            # 後始末が行に印を付けた後のエラー札 / 知らせ / 履歴の突き合わせで
            # 出る (印の無い行にボタンを出さない)。配達に失敗しても保存は崩さない。
            try:
                event_callback({
                    "type": "streaming_complete",
                    "persona_id": getattr(persona, "persona_id", None),
                    "node_id": node_id or "llm",
                    "pulse_id": state.get("_pulse_id"),
                    "building_id": building_id,
                })
            except Exception:
                LOGGER.warning(
                    "[sea][pipeline] could not close the streaming bubble "
                    "(msg=%s)", msg_id, exc_info=True,
                )

    if not body or not (memory_text or "").strip():
        return sub_seq
    ...
    try:
        _stored = runtime._store_memory(
            ...
            metadata=(
                ...
                if form == SAVED_FORM_CUT else None
            ),
            playbook_name=playbook.name,
            beat_state=state,
        )
        ...
        LOGGER.warning(
            "[sea][pipeline] could not store the stopped utterance to "
...
def _note_stream_cut(
    persona: Any,
    stream_error: Dict[str, Any],
    *,
    message_id: Optional[str],
    reply_last_message_id: Optional[str] = None,
    event_callback: Optional[Callable[[Dict[str, Any]], None]],
) -> None:
    """締めの生成がサーバーに切られたことを「最後に保存した発言」に書き足す。

    ``message_id`` は切られた本文を保存できた行。保存できなかった回と、切られた
    本文が一文字も無かった回は None — そのときは、この返事で最後に保存できた行
    (``reply_last_message_id`` = 呼び出し元の ``state["_last_message_id"]``) が
    「いま最後の発言」と一致するときだけ、そこへ「この後で話が止まった」を
    付ける (続きはその発言から起こす)。一致しない・無いときは付けない — 記録の
    最後の発言がこの返事より前のものだと、返事の後始末は時間窓で読み飛ばすので、
    書き足した知らせがどこにも出ずに消える。どこにも付けられなかった回は、
    画面への知らせだけをここで出す — 知らせは保存の成否と無関係に事実なので消さない。
    付けられた回の知らせは、返事の後始末が案内の材料と一緒に出す
    (sea/reply_stop_exit.py)。
    ...
    if message_id:
        noted = note_saved_utterance(
            persona_id, message_id=str(message_id),
            form=SAVED_FORM_CUT, stopped=True, detail=detail,
        )
    elif reply_last_message_id:
        # message_id を指定して書き足す — 記録の最後の発言がこの返事で保存した
        # 行でなければ (返事より前の発言・別経路の行) 書き足さずに False。
        noted = note_saved_utterance(
            persona_id, message_id=str(reply_last_message_id),
            stopped=True, detail=detail,
        )
    else:
        noted = False
    if noted or not event_callback:
        return
    try:
        event_callback({
            "type": "info",
            # 先頭にアイコンを書かない (画面側が info 種別に自前で描く)。
            "content": (
                "メッセージの生成が途中で終了しました。"
        ...
        LOGGER.warning(
            "[sea][llm] could not deliver the stream-cut notice", exc_info=True,
        )


@dataclass
class BeatSegment:
    """1 Beat (= LLM 1 回の生成) が生んだ、建物に 1 件として残る本文。

    スペルループは 1 周につき 1 回 LLM を呼ぶので、周がそのまま Beat になる。
    1 セグメントの中身は、その周の「スペル行の前の文 + スペルごとの
    ``<user_only>`` ブロック + スペル行の後の文」。スペルを含まない締めの発言も
    1 セグメントになる。

    ``building_id`` は **その Beat の生成が始まった時点でペルソナがいた部屋**。
    周の中のスペルで移動が起きても、その周の発言は生成が始まった部屋に残り、
    移動後の部屋には次の Beat が新しい 1 件として落ちる
    (docs/issues/pulse_beats_merge_into_single_record.md の契約 1・2)。

    ``emitted`` はストリーミング経路のための印 — その経路ではループの中で
    ...
    ``form`` は **この本文が行に残ったときの形** (sea/runtime_emitters.py の
    ``SAVED_FORM_*``)。スペルの周は「受け取った結果で終わる本文」、実行の途中で
    止まった周は「結果の来ていないスペルが残る本文」、締めの発言は「言い切った
    本文」(サーバーに切られた回は「途中で切れた本文」)。記録へ書いた側がこの値を
    「最後に保存した発言」に書き足し、返事が止まった回の後始末が中断の通告の
    文面を選ぶ (docs/intent/reply_stop_exit.md)。
    """
...
@dataclass
class SpellLoopResult:
    """:func:`_run_spell_loop` の戻り値。

    - ``segments``: Beat ごとに割れた本文 (時系列順)。最後の要素が締めの Beat で、
      ストリーミング経路ではこれだけが ``emitted=False`` で戻る (中間 Beat は
      ループの中で確定済み)。
    - ``final_continuation``: 最終ラウンド (= spell を含まない LLM 応答) の text
      のみ。呼び出し元は ``state["last"]`` (= 後段の memorize が SAIMemory へ
    ...
    - ``stop_error``: スペルの後の続きの生成 (LLM 呼び出しそのもの) が失敗して
      ...
      (スペルの失敗として従来どおり扱う)。
      ストリーミングを使わない経路では、返事を止める例外 (取り消し・関所の
      閉鎖・使うモデルが無い回) もここに入る — 周を 1 つ以上終えていれば、
...
def _spell_display_name(name: str, success: bool) -> str:
    ...
    if success:
        ...
        return (schema.spell_display_name if schema else "") or name
    return name


def _compose_stopped_round(
    text: str,
    executed: List[Tuple[str, Optional[Dict[str, Any]], bool]],
    *,
    stream_cut: bool = False,
) -> Optional[Dict[str, str]]:
    """まだ確定していない周が止まったときに、その周の本文を行へ残す形に組む。

    返すのは ``{"text": 行の本文, "memory_text": 記憶の本文, "form": 形}``
    (本文が空なら None)。行に残るものは、現実に起きたところまでと一致させる
    (docs/intent/reply_stop_exit.md 不変条件 5):
    ...
    - 形は、結果の来ていないスペルが一つでもあれば「実行が終わる前に止まった
      周」、全部の結果を受け取っていれば「結果を受け取った周」。

    ``executed`` は周の実行の途中経過 — 有効なスペルを位置順に並べた先頭から、
    受け取った ``(結果, メタ, 成功か)``。記憶の本文 (``memory_text``) は周の頭で
    記憶に書く本文と同じ形 (スペル行は正規形、結果の HTML を含まない)。
    """
    if not (text or "").strip():
        return None
    malformed: List[Tuple[str, str, Any]] = []
    all_parsed = _parse_spell_lines(text, quiet=True, malformed_out=malformed)
    if not all_parsed and not malformed:
        return {
            "text": text,
            "memory_text": text,
            "form": SAVED_FORM_CUT if stream_cut else SAVED_FORM_COMPLETE,
        }
    classified = [
...
async def _run_spell_loop(
    ...
    initial_stream_error: Optional[Dict[str, Any]] = None,
) -> SpellLoopResult:
    ...
    戻り値は :class:`SpellLoopResult` — **周ごとに 1 件** の記録に割れた本文
    (:class:`BeatSegment`) の列と、締めの発言。2026-09-12 以前は全周を 1 本の
    テキストに結合して 1 件だけ保存していたため、周の途中で部屋が変わっても
    保存先の部屋を 1 つしか選べなかった
    ...
    ``{"msg_id": str, "building_id": str, "sub_seq": int, "finalized": bool,
    "placeholder_round": int, "cancellation_token": ...}`` で、この関数が
    Beat の切れ目ごとに **下書き行を確定して次の部屋に新しい行を作り**、
    dict を in-place で書き換える。呼び出し元と Beat 死亡時の後始末は
    「いま生きている下書き行」をこの dict から読む。None の場合は従来通り
    ``generate()`` 単発呼び出しで全文一括受信し、記録は呼び出し元が行う。
    ...
    **下書き行を作った部屋** を渡す (確定時に引き直さない 2026-06-11 の不変条件を
    ...
    ``initial_stream_error``: ラウンド 1 の本文を運んだストリームを、サーバーが
    ...
    **止まった周の本文**: 返事を止める例外 (取り消し・関所の閉鎖・使うモデルが
    ...
    その周の本文を :func:`_compose_stopped_round` で組む。ストリーミング経路では
    共有の器 (``pipeline_streaming_state["salvage"]``) に置いて投げ、Beat の出口の
    ...
    if not spell_enabled or not text:
        return SpellLoopResult(
            ...
            final_stream_error=initial_stream_error,
        )
    ...
    # この Beat の記録先の部屋と、この Beat の本文を作ったコールの使用量。
    # どちらも「周を跨ぐと次の Beat のものに入れ替わる」値なので、周の頭で
    # 参照して周の終わりに更新する。
    current_building_id = initial_building_id or runtime._effective_building_id(
        persona, building_id,
    )
    # この Beat の「聞いた人」を作る在室者の写し (契約 3)。部屋と同じく周の頭で
    ...
    # いまの ``text`` を運んだストリームを、サーバーが途中で切った申告。
    # 周の本文を確定したら倒し、周の再ストリームのたびに入れ替える。
    text_stream_error: Optional[Dict[str, Any]] = initial_stream_error
    # いまの ``text`` を運んだストリームが、停止で途中から読まれなくなったか
    # (周の再ストリームの途中の停止)。止まった周の本文を「途中で切れた」と
    # 書く材料。ラウンド 1 の本文の途中停止は呼び出し元が先に確定するので
    # ここには来ない。
    text_cancelled_midway = False
    # この周の実行の途中経過 — 止まった周の本文を組む材料
    # (``_compose_stopped_round``)。周の頭で空に戻す。
    _round_progress: Dict[str, Any] = {
        "valid_spells": [],
        "executed": [],
        "results_memorized": False,
    }
    _node_memorize_cfg = getattr(node_def, "memorize", None) if node_def is not None else None
    _node_memorize_tags: List[str] = (
    ...
    def _stop_open_round(exc: BaseException) -> Optional[SpellLoopResult]:
        ...
        ストリーミング経路は共有の器に置いて None を返す (呼び出し元が投げ、
        Beat の出口の保存がその本文で下書き行を確定する)。そうでない経路は、
        周を 1 つ以上終えていればセグメントに積んだ戻り値を返し (呼び出し元が
        建物へ書いてから投げる)、そうでなければ None (従来どおり投げる — 周が
        一つも始まっていない本文は早期の吹き出しとして既に建物にある)。

        受け取り済みのスペル結果は記憶にも書く (普段は周の結果をまとめて
        書くが、全部そろう前に止まった回はそこに届かない)。
        """
        ...
        if executed and not _round_progress.get("results_memorized"):
            try:
                _partial_results = "\n".join(
                    ...
                    for spell, (result_text, _meta, ok) in zip(valid_spells_now, executed)
                )
                if _partial_results:
                    runtime._store_memory(
                        persona, _partial_results, role="system",
                        tags=(
                            (_node_memorize_tags + ["spell"])
                            if _node_memorize_tags else ["conversation", "spell"]
                        ),
                ...
                LOGGER.warning(
                    "[sea][spell] could not store the results received before "
                    "the stop", exc_info=True,
                )
        composed = _compose_stopped_round(
            text, executed,
            stream_cut=bool(text_stream_error) or text_cancelled_midway,
        )
        if composed is not None and _round_memorized:
            # 周の本文はもう記憶にある (周の頭で書いた)。行だけ残す。
            composed["memory_text"] = ""
        if pipeline_streaming_state is not None:
            ...
            return None
        if loop_count == 0 or not isinstance(exc, Exception):
            return None
        final_text = ""
        if composed is not None:
            ...
            # 記憶にまだ無い本文は、呼び出し元の出口の補填が拾えるように
            # 締めの発言として返す。
            final_text = composed["memory_text"]
        return SpellLoopResult(
            segments=segments,
            final_continuation=final_text,
    ...
    def _close_streaming_beat(segment: BeatSegment, memory_text: str) -> None:
        ...
        確定先の部屋は **その下書き行を作った部屋** (= ``segment.building_id``)。
        周の中のスペルで移動していても引き直さない — 引き直すと移動後の部屋に
        存在しない行を更新しに行って空振りし、本文の入らない行が残る
        (2026-06-11 Region RPG 実機。Beat 単位に割ってもこの不変条件は同じ)。

        ``memory_text``: この周の本文の、HTML を含まない姿 (= その周の
        ``assistant_content``)。退避書き込みの回に「建物にはあるが記憶に無い
        本文」として出口へ渡す値で、セグメントの本文 (``<user_only>`` や
        spellResult の HTML 入り) を渡してはいけない —
        docs/issues/spell_html_leak_into_saimemory.md が塞いだ混入が戻る。
        """
        st = pipeline_streaming_state
        if st is None:
            return
        ...
        def _write_beat_body_directly(*, announce: bool) -> bool:
            ...
            ``announce``: 画面へ ``say`` を流すか。既に同じ本文の ``say`` を
            (下書き行の id で) 流した後の退避では False — 二度流すと同じ発言の
            吹き出しが 2 つ並ぶ。

            戻り値は「建物へ書いたか」(本文が空の回は書かない)。
            """
            ...
            if not segment.text.strip():
                ...
                return False
            _direct = _emit_say_and_capture(
                runtime, persona, segment.building_id, segment.text, state,
                pulse_id=state.get("_pulse_id"), metadata=extra,
                event_callback=event_callback,
                occupants_snapshot=segment.occupants,
            )
            if (
        ...
        if not msg_id or st.get("finalized"):
            ...
            # この Beat の本文は直接 1 件として書く (書かないと発言が消える)。
            if segment.text.strip():
                LOGGER.warning(
                    "[sea][pipeline] no live placeholder for beat %d — "
                    "falling back to _emit_say", loop_count,
                )
            _write_beat_body_directly(announce=True)
            return
        seq = int(st.get("sub_seq", 0) or 0) + 1
        st["sub_seq"] = seq
        if event_callback:
            # ストリーミング中に積み上がった生テキストの吹き出しを捨ててから、
            # 整形済み (= <user_only> wrap 済) の 1 件だけを残す。呼び出し元が
            # ループ完了後に 1 回だけ行っていた discard→say を、Beat ごとに行う。
            event_callback({
                "type": "streaming_discard",
                "persona_id": getattr(persona, "persona_id", None),
                "node_id": getattr(node_def, "id", "llm"),
        ...
        if getattr(_sf, "status", None) == "saved":
            st["finalized"] = True
            state["_last_message_id"] = str(msg_id)
            segment.emitted = True
            # この周の本文はスペルの結果で終わる形 (返事が止まった回の後始末が
            # 通告の文面を選ぶ材料)。
            note_saved_utterance(
                getattr(persona, "persona_id", None),
                message_id=str(msg_id), form=segment.form,
            )
            return
        # 確定できなかった回 (対象行なし / 保存失敗)。次の周の頭で下書き行を
        # 引き直すので、この本文を誰かが後で救う機会はもう無い — 建物へ直接
        # 書いて発言を残す。空のまま残る下書き行は孤児 (`_streaming_placeholder`
        # が立ったまま) なので、孤児掃除の網には入る。
        LOGGER.error(
            "[sea][pipeline] beat %d could not be confirmed on its draft row "
            "(msg=%s status=%s) — writing the body straight to the building",
            loop_count, msg_id, getattr(_sf, "status", None),
        )
        if _write_beat_body_directly(announce=False):
            # 本文の行き先は決まったので、この下書き行はもう追いかけない。
            # 印を倒したままだと、直後の Beat 境界で例外が出た回に出口の後始末が
            # 同じ本文でこの行を確定・記憶しに行き、退避で書いた 1 件と二重になる。
            st["finalized"] = True

    def _open_streaming_beat(new_building_id: str) -> None:
        """次の Beat の下書き行を、引き直した部屋に作る。

        ここで作った行が以後の「いま生きている下書き行」— 呼び出し元の最終確定も
        Beat 死亡時の後始末も、共有の dict からこの id と部屋を読む。
        """
        st = pipeline_streaming_state
        if st is None:
            return
        new_msg_id = runtime._emit_speak_start(
            persona, new_building_id, pulse_id=state.get("_pulse_id"),
        )
        st["msg_id"] = new_msg_id
        st["building_id"] = new_building_id
        st["sub_seq"] = 0
        st["finalized"] = False
        st["placeholder_round"] = int(st.get("placeholder_round", 1) or 1) + 1
        st.pop("text_chunks", None)
        st.pop("salvage", None)
        if not new_msg_id:
            LOGGER.error(
                "[sea][pipeline] _emit_speak_start failed for beat %d "
    ...
    try:
        while loop_count < _effective_max_rounds:
            # 周の途中経過を空に戻す (止まった周の本文を組む材料。前の周の
            # 結果を次の周の本文に持ち込まない)。
            _round_progress["valid_spells"] = []
            _round_progress["executed"] = []
            _round_progress["results_memorized"] = False
            _round_memorized = False
            ...
            if assistant_content:
                ...
                _round_memorized = bool(_stored_id)
            ...
            # 実行の途中経過を器に見せる (リストの参照ごと — 以後の append が
            # そのまま見える)。実行の途中で返事が止まったとき、受け取り済みの
            ...
            #
            # この周が終端なら、下書き行はここでは確定させずに残す — 締めの
            # Beat として呼び出し元が全部入りのメタデータで確定する。次の周が
            # 無いので新しい下書き行も作らない (空の行を残さない)。
            ...
            # 記憶へ渡す姿は HTML の無い assistant_content (上でこの周の記憶に
            # 書いたものと同じ本文)。
            _close_streaming_beat(_segment, assistant_content)

            # この周の本文はもうセグメントとして記録し終えた。``text`` を空に
            # するのはここ — 以後 (Beat 境界、ツール一覧の取り直し、知覚 flush、
            # 次の下書き行の発番、そして再呼び出し) のどこで例外が出ても、
            # ループを包む except 節が ``final_continuation = text`` として
            # **同じ本文をもう 1 件** 締めのセグメントに積むことがなくなる
            # (生の /spell 行込みの本文が建物と記憶へ二重に入る形だった)。
            # 空にしておけば、失敗した回は締めのセグメントを積まずに終わる
            # (= partial で停止)。
            text = ""
            ...
            # ---- 次の Beat の記録先の部屋を引き直す ----
            # この周のスペルで移動が起きていたら、次の生成の発言は移動後の部屋に
            # 落ちる (契約 2)。ストリーミング経路では同じ部屋に新しい下書き行を
            # 作り、以後の chunk / sub-speak をそちらへ向ける。作成を再呼び出しの
            # 直前に置くのは、/quick_spell 終端のように再呼び出しが無い周で空の
            # 下書き行を残さないため。
            current_building_id = runtime._effective_building_id(persona, building_id)
            current_occupants = _occupants_snapshot(runtime, current_building_id)
            _open_streaming_beat(current_building_id)
            ...
            # 明示的に空リストで送る。
            #
            # ``text`` は既にこの周のセグメントを積んだ直後で空にしてある
            # (上の `_close_streaming_beat` の直後)。helper / generate() で例外が
            ...
            _retry_stream_error: Optional[Dict[str, Any]] = None
            ...
            try:
                if pipeline_streaming_state is not None:
                    ...
                    # サーバーがこの周のストリームを途中で切った申告。ここで
                    # 消費し、次の周の頭で「スペル行を含む = 次の周が続く」か
                    # 「締めの発言として途切れたまま確定する」かを見分ける
                    # (docs/issues/spell_round_stream_cut_is_not_detected.md)。
                    _retry_stream_error = (
                        llm_client.consume_stream_error()
                        if hasattr(llm_client, "consume_stream_error") else None
                    )
                    if not isinstance(_retry_stream_error, dict):
                        _retry_stream_error = None
                    if _retry_stream_error:
                        LOGGER.warning(
                            "[sea][spell] Round %d: the continuation stream was cut "
                            "by the server (code=%s status=%s)",
                            loop_count, _retry_stream_error.get("code"),
                            _retry_stream_error.get("status", ""),
                        )
                    if _retry_cancelled:
                        LOGGER.info(
                ...
                # 使うモデルが無い・繋げない回は、エラーの札を「モデルを選び
                # 直す」案内にするため、ほかの LLMError の stop_error の道には
                # 乗せず、下の包括 except から返事を止める例外として扱う
                # (それまでの周の発言は `_stop_open_round` が残す)。
                raise
                ...
                # 実行制御の正規イベント。続きの失敗ではなく、返事を止める
                # 例外として下の except が扱う。
                raise
            except Exception as exc:
                # LLMError に包まれずに届いた想定外の例外 (ストリームを読んで
                # いる途中の接続断など)。これも続きの生成の失敗であって、
                # スペル系の内部エラーではない — 包括 except に「スペル内部の
                # 致命エラー」として飲ませると、注記つきで黙って閉じて画面に
                # 何も出ない (スペル無しの同じ切断はエラー札が出るので、同じ
                # 失敗が場面で違う顔になる)。普通の返事の失敗と同じ形
                # (LLM ノードの一番外側の包み方) の LLMError に包んで、
                # 続きの失敗の道に載せる。asyncio.CancelledError は
                # BaseException なのでここには来ない。
                _continuation_error = LLMError(
                    f"LLM node failed: {type(exc).__name__}: {exc}",
                    original_error=exc,
                )
                raise _continuation_error from exc
            ...
            # この retry が作るのは **次の** Beat の本文なので、使用量も次の
            ...
            if retry_usage:
                ...
                pending_llm_usage = {
                    ...
                    "model_display_name": get_model_display_name(retry_usage.model),
                    "input_tokens": retry_usage.input_tokens,
                    "output_tokens": retry_usage.output_tokens,
                    "cached_tokens": retry_usage.cached_tokens,
                    "cache_write_tokens": retry_usage.cache_write_tokens,
            ...
            if isinstance(retry_result, dict):
                ...
                text = ""
            # この本文の途切れ方。次の周の頭で「スペル行を含む = 次の周が
            # 発話を続ける」か「締めの発言として途切れたまま確定する」かが
            # 決まるので、そこまで持ち運ぶ。
            text_stream_error = _retry_stream_error
            text_cancelled_midway = bool(_retry_cancelled)
        ...
        if loop_count == 0:
            ...
            return SpellLoopResult(
                ...
                closing_reasoning_text=pending_reasoning_text,
                closing_reasoning_details=pending_reasoning_details,
                final_stream_error=text_stream_error,
            )
        ...
        if final_continuation:
            segments.append(BeatSegment(
                ...
                reasoning_text=pending_reasoning_text,
                reasoning_details=pending_reasoning_details,
                # サーバーに切られた締めの発言は、途切れたまま確定する。
                form=SAVED_FORM_CUT if text_stream_error else SAVED_FORM_COMPLETE,
            ))
        return SpellLoopResult(
            segments=segments,
            final_continuation=final_continuation,
            ...
            closing_reasoning_text=pending_reasoning_text,
            closing_reasoning_details=pending_reasoning_details,
            # 締めの周の切断 (一文字も来ないうちに切られた回を含む)。
            final_stream_error=text_stream_error,
        )
    except (ExecutionCancelledException, BeatGateClosedError) as exc:
        # Beat 境界の中断 / 関所 fail-closed は「spell 系の内部エラー」ではなく
        # 実行制御の正規イベント。partial 保存へ降格しない
        # (caller = PulseController / run_meta_user が型別に処理する)。
        # まだ確定していない周の本文は残す — ストリーミング経路は器に置いて
        # 投げ、そうでない経路は周を終えていれば書かせてから投げさせる。
        _stopped = _stop_open_round(exc)
...
def lg_llm_node(runtime, node_def: Any, persona: Any, building_id: str, playbook: PlaybookSchema, event_callback: Optional[Callable[[Dict[str, Any]], None]] = None):
    async def node(state: dict):
        ...
        # 「サーバーがこの Beat の最初のストリームを切った」の申告。state には
        # 置かない — Beat をまたいで残留すると、次の Beat の言い切った発言に
        # 偽の印が乗る (2026-09-13 検算)。消費はこの Beat の中で、締めの発言が
        # 途切れたまま確定するときだけ行う (スペルの周を挟む回はループが
        # 引き継いで見分ける。docs/issues/spell_round_stream_cut_is_not_detected.md)。
        _initial_stream_error: Optional[Dict[str, Any]] = None
        ...
        # 確定した回は本文が記憶へも書かれるので、この回の本文に補填の余地は
        # 無い。印を見ずに補填を走らせると、スペルループの途中で死んだ回に
        # **最初の周の生テキスト** (`text`、/spell 行込み) を書いてしまう —
        # その周の発言はループが既に記憶へ書いており、二重になる。
        # もう一つの形は、一言も生まれていない行の取り下げ — この回は記憶を
        # 一切書かないので、前の周が残した「建物にはあるが記憶に無い本文」の
        # 補填だけは通す (`_backfill_memory_on_beat_death` の門)。
        beat_death_settled = False

        def _live_placeholder() -> Tuple[Optional[str], str, bool]:
            """いま生きている下書き行の ``(id, 部屋, 確定済みか)``。

            スペルループは Beat の切れ目ごとに下書き行を確定して次の部屋に新しい
            行を作る (`docs/issues/pulse_beats_merge_into_single_record.md`)。
            そのため閉包の初期値 (`pipeline_msg_id` / `pipeline_eff_bid`) を直に
            読むと、2 周目以降に死んだ回で **もう確定した最初の行** を後始末しに
            行き、進行中の行が孤児になる。共有の器を正とする。
            """
            if _pipeline_spell_state is not None and "msg_id" in _pipeline_spell_state:
                live_id = _pipeline_spell_state.get("msg_id")
                live_finalized = bool(_pipeline_spell_state.get("finalized"))
                if live_id == pipeline_msg_id:
                    # ループを抜けた後は閉包側が同じ行を確定させる。器の印は
                    # ループの中でしか進まないので、両方を見ないと二重に確定する。
                    live_finalized = live_finalized or pipeline_finalized
                return (
                    live_id,
                    _pipeline_spell_state.get("building_id") or pipeline_eff_bid,
                    live_finalized,
                )
            return pipeline_msg_id, pipeline_eff_bid, pipeline_finalized

        def _save_draft_on_beat_death(exc: BaseException) -> None:
            ...
            **発言そのものが消える** (2026-05-19〜08-26 の 3 ヶ月で 32 件。経緯は
            ...
            「確定していない下書き行が残っていたら、その本文で保存する」**を一括で
            保証する。

            ここでやるのは保存だけで、「言い切っていない」印と中断の通告は
            置かない — 返事の実行の一番外側の後始末 (sea/reply_stop_exit.py) が、
            すべての書き込みの後で一回だけ置く (docs/intent/reply_stop_exit.md)。
            この Beat がサブラインの中で死んだ回も同じで、保存したうえで失敗を
            そのまま上へ投げる。
            """
            nonlocal pipeline_sub_seq, pipeline_finalized, beat_death_settled
            live_msg_id, live_bid, already_finalized = _live_placeholder()
            if not live_msg_id or already_finalized:
                return
            pipeline_finalized = True
            beat_death_settled = True
            if _pipeline_spell_state is not None:
                _pipeline_spell_state["finalized"] = True
            try:
                # sub_seq の最新値は、本文ストリームの途中経過と spell loop 側の
                # 器のどちらかで進んでいることがある。古い値で final を打つと
                # voice-tts の連番が既出の番号と衝突する。
                # 2 周目以降の下書き行は連番を 0 から積み直すので、器の値だけが正。
                _spell_round = 1
                if _pipeline_spell_state is not None:
                    _spell_round = int(
                        _pipeline_spell_state.get("placeholder_round", 1) or 1
                    )
                if _spell_round > 1:
                    pipeline_sub_seq = int(
                        _pipeline_spell_state.get("sub_seq", 0) or 0
                    )
                else:
                ...
                # 行に残す本文と、その形。
                # - スペルループが止まった周の本文を器に置いていれば、それが正
                #   (受け取り済みの結果だけを折りたたみに入れ、結果の来ていない
                #   スペルは唱えた行だけを残した形。周の頭で記憶に書いた本文は
                #   重ねない)。
                # - 2 周目以降の下書き行は「その周の retry ストリーム」の行なので、
                #   回収元はスペルループ側の器の chunk (ストリームの途中で死んだ
                #   本文 = 途中で切れた本文)。閉包の text は最初の周の本文で、
                #   それは別の行として既に確定している。
                # - 最初の行: ストリームを受け切った本文 (閉包の text) があれば
                #   スペル行の有無で組み直す (ループに入る前に死んだ回)。無ければ
                #   途中経過の chunk (ストリームの途中で死んだ = 途中で切れた本文)。
                salvage_text = ""
                salvage_form = SAVED_FORM_CUT
                salvage_memory: Optional[str] = None
                _loop_salvage = (
                    _pipeline_spell_state.get("salvage")
                    if _pipeline_spell_state is not None else None
                )
                if (
                    isinstance(_loop_salvage, dict)
                    and _loop_salvage.get("msg_id") == live_msg_id
                ):
                    salvage_text = str(_loop_salvage.get("text") or "")
                    salvage_form = str(_loop_salvage.get("form") or SAVED_FORM_CUT)
                    salvage_memory = str(_loop_salvage.get("memory_text") or "")
                elif _spell_round > 1:
                    _spell_chunks = (
                        _pipeline_spell_state.get("text_chunks")
                        if _pipeline_spell_state is not None else None
                    )
                    if _spell_chunks:
                        salvage_text = "".join(_spell_chunks)
                elif isinstance(text, str) and text.strip():
                    _composed = _compose_stopped_round(
                        text, [], stream_cut=bool(_initial_stream_error),
                    )
                    if _composed is not None:
                        salvage_text = _composed["text"]
                        salvage_form = _composed["form"]
                        salvage_memory = _composed["memory_text"]
                else:
                ...
                # 一言も生まれていない行は、空文字で確定せずに取り下げる。
                # 空文字で確定すると本文の無い記録が建物とペルソナのログに
                # 永続し、下書きの印も倒れて孤児掃除の網から外れる — 締めの
                # Beat の正常復帰側 (H-1) と同じ形なので、判定も取り下げも
                # 同じ関数を通す。断られた回は下の確定へ落ちる。
                if _withdraw_if_nothing_was_said(
                    runtime, persona,
                    building_id=live_bid,
                    msg_id=live_msg_id,
                    partial_text=salvage_text,
                    ...
                    LOGGER.warning(
                        "[sea][pipeline] the dying beat produced nothing at all "
                        "— withdrew the empty draft row (msg=%s building=%s)",
                        live_msg_id, live_bid,
                    )
                    return
                pipeline_sub_seq = _save_cut_utterance(
                    runtime=runtime,
                    persona=persona,
                    state=state,
                    playbook=playbook,
                    ...
                    text=salvage_text,
                    form=salvage_form,
                    memory_text=salvage_memory,
                    node_id=getattr(node_def, "id", "llm"),
                )
            except Exception:
                # ここで新しい例外を立てると、Beat を落とした元の例外が
                # すり替わる。保存の失敗は記録だけして、元の例外を通す。
                LOGGER.exception(
                    "[sea][pipeline] saving the draft row on beat death failed "
        ...
        def _backfill_memory_on_beat_death(exc: BaseException) -> None:
            ...
            (`_save_draft_on_beat_death`) は「確定済み」でスキップする
            ...
            スペルループの退避書き込み (`_close_streaming_beat` の fallback) も
            建物へ本文を入れる口だが、その周の本文はループ自身が記憶へ書いて
            いる。だから発火条件に数えるのは **その記憶の書き込みが失敗した
            回だけ** — ループがその回に限って本文を
            ``BEAT_BODY_UNMEMORIZED_KEY`` へ置く。全部数えると、記憶に入って
            いる周の本文をもう一度書いて二重にする。
            """
            loop_said_text = str(state.get(BEAT_BODY_UNMEMORIZED_KEY) or "")
            if not (pipeline_finalized or beat_said or loop_said_text):
                return
            if beat_death_settled and not loop_said_text:
                # 後始末が確定と記憶をまとめて済ませた回。ここで書くと、本文が
                # 空だった回に限って別の本文 (閉包の text) が紛れ込む。
                # 前の周の未記憶の本文 (`loop_said_text`) が残っている回だけは
                # 通す — 後始末が下書き行を取り下げた回は記憶を一切書かないので、
                # ここで黙ると前の周の本文が建物にだけ残って記憶から消える。
                return
            if state.get("_beat_memorized"):
                return
            memorize_config = getattr(node_def, "memorize", None)
            ...
            # どちらも無い回は、ループの退避書き込みが残した本文。
            if beat_death_settled:
                # 門を通ったのは、後始末が下書き行を取り下げた回だけ。この回の
                # 補填対象は前の周の未記憶の本文であって、閉包の text ではない
                # (取り下げた行には本文が無く、text は別の周のもの)。
                backfill_text = loop_said_text
            elif pipeline_finalized:
                backfill_text = text
            else:
                backfill_text = beat_said_text or loop_said_text
        ...
        try:
            ...
            if effective_tools:
                ...
                if use_tool_streaming:
                    ...
                    # 画面イベントに載せる部屋 (契約 5)。この経路は下書き行を
                    # 使わないので、生成を始めた時点の現在地が吹き出しの居場所。
                    _tool_stream_bid = runtime._effective_building_id(persona, building_id)
                    ...
                    if tool_detection and tool_detection.get("type") in ("tool_call", "both"):
                        ...
                        eff_bid = runtime._effective_building_id(persona, building_id)
                        _tool_said = _emit_say_and_capture(
                            runtime, persona, eff_bid, text, state,
                            pulse_id=pulse_id, metadata=msg_metadata,
                            event_callback=event_callback,
                        )
                        ...
                        if (
                            ...
                            and _tool_said.get("message_id")
                        ):
                            # ストリームの途中で止められた本文。途中で切れた
                            # 本文として記録に書き足す (印と通告は返事の後始末)。
                            state[INTERRUPTED_METADATA_KEY] = True
                            note_saved_utterance(
                                getattr(persona, "persona_id", None),
                                message_id=str(_tool_said["message_id"]),
                                form=SAVED_FORM_CUT,
                                stopped=True,
                            )
                ...
                if _spell_loop_count > 0:
                    ...
                    # ラップ済み) なので、ここには plain な continuation を入れて
                    # <user_only>/spellResult の HTML タグ混入を防ぐ
                    # (docs/issues/spell_html_leak_into_saimemory.md)。
                    ...
                    if _node_speak_flag is False:
                        ...
                        # Beat ごとに 1 件ずつ、その Beat の生成が始まった部屋へ
                        # emit する (2026-09-12。それ以前は merged 全文を 1 件に
                        # まとめて 1 部屋へ出していた)。
                        #
                        # If Phase 1 already emitted text_before early (= TTS warm-
                        # up while spell loop ran), strip the duplicate leading
                        # text_before from the first segment so building history
                        # sees the same record only once. Phase 2-C will remove the
                        ...
                        LOGGER.info(
                            "[sea][spell] Tool-mode: emitted %d beat segment(s) "
                            "(phase1_early=%s)",
                ...
                if use_streaming:
                    ...
                    for stream_attempt in range(max_stream_retries):
                        ...
                        _initial_text, pipeline_sub_seq, _initial_spell_detected, _initial_cancelled = await _consume_pipeline_stream(
                            ...
                            # 音声も画面イベントも、下書き行を作った部屋に揃える
                            # (確定先と食い違わせない)。
                            emit_building_id=pipeline_eff_bid,
                        )
                        text = _initial_text
                        ...
                        if _stream_error and isinstance(_stream_error, dict):
                            LOGGER.warning(
                                "[sea][llm] Stream interrupted by server: code=%s status=%s — "
                                "keeping the partial response",
                                _stream_error.get("code"), _stream_error.get("status", ""),
                            )
                            # この Beat のローカルに引き取る (state に置かない)。
                            # 締めの発言として途切れたまま確定するかは、スペルの
                            # 周を挟むかどうかで決まる — ループが引き継いで見分ける。
                            _initial_stream_error = _stream_error
                    ...
                    # ストリームの途中で止められた回。下書き行を途中までの本文で
                    # 保存する (詳しくは ``_save_cut_utterance`` の docstring)。
                    # 印と通告は返事の一番外側の後始末が置く — スペル無効の
                    # ペルソナはこの後も例外なしで閉じるので、「この後で話が
                    # 止まった」を記録に書き足しておく (保存の関数が行う)。
                    if cancelled_during_stream:
                        # 一語も届かないうちに止められた回は、空文字で確定せず
                        # 行ごと取り下げる (締めの Beat の H-1 / Beat 出口の N-1
                        # と同じ判定を通す)。取り下げられなければ従来の確定へ。
                        if pipeline_msg_id and _withdraw_if_nothing_was_said(
                            runtime, persona,
                            ...
                            LOGGER.warning(
                                "[sea][pipeline] stopped before the first token "
                                "— withdrew the empty draft row (msg=%s building=%s)",
                                pipeline_msg_id, pipeline_eff_bid,
                            )
                        else:
                            pipeline_sub_seq = _save_cut_utterance(
                                runtime=runtime,
                    ...
                    #
                    # 2026-09-12 以降、この dict は「いま生きている下書き行」の
                    # 正本でもある — スペルループは Beat の切れ目ごとに行を確定して
                    # 引き直した部屋に新しい行を作り、msg_id / building_id /
                    # sub_seq / finalized をここで差し替える。
                    ...
                    # 下書き行の後始末は Beat の出口
                    # (``_save_draft_on_beat_death``) が LLM エラー等と
                    # まとめて一括で受け止める。
                    _spell_result_ns = await _run_spell_loop(
                        text=text,
                        spell_enabled=_spell_enabled,
                        llm_client=llm_client,
                        runtime=runtime,
                        ...
                        initial_stream_error=_initial_stream_error,
                    )
                    _continuation_ns = _spell_result_ns.final_continuation
                    _spell_loop_count_ns = _spell_result_ns.loop_count
                    # 締めの発言を運んだストリームがサーバーに切られていたか。
                    # スペルの周を挟んだ回は、ループが周ごとの申告から締めの周の
                    # 分だけを引き継いで返す (途中の周の切断は次の周が発話を
                    # 続けるので載らない)。周が無い回は最初のストリームの申告。
                    _final_stream_error_ns = (
                        _spell_result_ns.final_stream_error
                        if _spell_loop_count_ns > 0 else _initial_stream_error
                    )
                    ...
                    # (もう確定済み) を見に行って、進行中の行が孤児になる。
                    if _pipeline_spell_state is not None:
                        pipeline_msg_id = _pipeline_spell_state.get("msg_id")
                        pipeline_eff_bid = (
                            _pipeline_spell_state.get("building_id") or pipeline_eff_bid
                        )
                        pipeline_sub_seq = int(
                            _pipeline_spell_state.get("sub_seq", pipeline_sub_seq)
                            or 0
                        )
                        pipeline_finalized = bool(
                            _pipeline_spell_state.get("finalized")
                        )
                    ...
                    if _spell_loop_count_ns > 0:
                        ...
                        if _node_speak_flag_ns is False:
                            ...
                            if pipeline_msg_id and not pipeline_finalized:
                                ...
                                _sf_result = _finalize_speak_with_signal(
                                    runtime, persona,
                                    pipeline_eff_bid,
                                    pipeline_msg_id, _continuation_ns,
                                    pulse_id=state.get("_pulse_id"),
                                    extra_metadata=None,
                            ...
                            # 中間 Beat はループの中で「その Beat の下書き行」に
                            # 確定済み。ここで扱うのは締めの Beat だけ — 締めの
                            # 本文は最後のセグメント (= まだ確定していないもの)。
                            # /quick_spell 終端ではそれが最後の周の本文で、
                            # 通常はスペルを含まない最終応答。
                            _final_seg_ns = (
                                _spell_result_ns.segments[-1]
                                if (
                                    _spell_result_ns.segments
                                    and not _spell_result_ns.segments[-1].emitted
                                )
                                else None
                            )
                            _final_text_ns = (
                                _final_seg_ns.text if _final_seg_ns is not None else ""
                            )
                            _final_bid_ns = (
                                _final_seg_ns.building_id
                                if _final_seg_ns is not None
                                else pipeline_eff_bid
                            )
                            ...
                            if not pipeline_msg_id:
                                # 下書き行をそもそも作れなかった回の救済。この回の
                                # ループはストリーミングを使っていないので中間 Beat も
                                # 未記録のまま — まだ建物へ書いていないセグメントを
                                # 全部、それぞれの部屋へ 1 件ずつ出す。
                                _pending_segs_ns = [
                                    seg for seg in _spell_result_ns.segments
                                    if not seg.emitted
                                ]
                                if _pending_segs_ns:
                                    LOGGER.warning(
                                        "[sea][pipeline] no placeholder msg_id — "
                                        "falling back to _emit_say for %d beat(s)",
                                        len(_pending_segs_ns),
                                    )
                                    _fb_said_ns = _emit_beat_segments(
                                        runtime, persona, state, _pending_segs_ns,
                                        pulse_id=pulse_id,
                                        event_callback=event_callback,
                                    ...
                                    if _final_stream_error_ns and _final_has_body_ns:
                                        ...
                                        # 保存の記録の形 (書き込みの共通の出口が
                                        # 行ごとに書き足す) で確かめる。
                                        _rec_ns = last_saved_utterance(
                                            getattr(persona, "persona_id", None),
                                        )
                                        _closing_row_saved_ns = bool(
                                            _fb_said_ns and _rec_ns is not None
                                            and _rec_ns.form == SAVED_FORM_CUT
                                            and _rec_ns.message_id
                                            == str(state.get("_last_message_id") or "")
                                        )
                                        _note_stream_cut(
                                            persona, _final_stream_error_ns,
                                            message_id=(
                                                state.get("_last_message_id")
                                                if _closing_row_saved_ns else None
                                            ),
                                            reply_last_message_id=state.get(
                                                "_last_message_id"
                                            ),
                                            event_callback=event_callback,
                                        )
                            elif not _final_has_body_ns and not pipeline_finalized:
                                # 締めの Beat が本文を一つも生まなかった回 —
                                # 次の周のために先に開けた下書き行が残っている
                                # のに、再呼び出しが例外で落ちたか空応答だった。
                                # ここで空文字のまま確定すると、本文の無い記録が
                                # 建物とペルソナのログに永続し、下書きの印も倒れて
                                # 孤児掃除の網から外れる。画面に流れかけた chunk の
                                # 吹き出しも宙吊りのまま残る
                                # (docs/issues/pulse_beats_merge_into_single_record.md
                                #  の H-1)。
                                _partial_ns = ""
                                ...
                                if _spell_chunks_ns:
                                    _partial_ns = "".join(_spell_chunks_ns)
                                if _withdraw_if_nothing_was_said(
                                    runtime, persona,
                                    building_id=pipeline_eff_bid,
                                    msg_id=pipeline_msg_id,
                                    partial_text=_partial_ns,
                                    # 連番はスペル側の器から引き継いだ値 (2 周目
                                    # 以降は 0 から積み直している)。
                                    sub_seq=pipeline_sub_seq,
                                    pulse_id=pulse_id,
                                    node_id=getattr(node_def, "id", "llm"),
                                    ...
                                    # 一文字も流れていない回 — 行ごと取り下げた。
                                    # 音声のストリームは開いていない (開くのは
                                    # 最初の sub-speak で、それも飛んでいない)
                                    # ので、閉じ損ねは起きない。
                                    LOGGER.warning(
                                        "[sea][pipeline] the closing beat produced "
                                        "nothing at all — withdrew the empty "
                                        "draft row (msg=%s building=%s)",
                                        pipeline_msg_id, pipeline_eff_bid,
                                    )
                                    pipeline_finalized = True
                                    if _pipeline_spell_state is not None:
                                        _pipeline_spell_state["finalized"] = True
                                elif _partial_ns.strip() or pipeline_sub_seq > 0:
                                    # 途中まで流れた本文がある回 (続きの生成が
                                    # 途中で内部エラーに落ち、ループが注記つきで
                                    # 閉じた回)。その部分文を途中で切れた本文
                                    # として保存し、「この後で話が止まった」を
                                    # 記録に書き足す — この返事は例外なしで
                                    # 閉じるので、印と通告は返事の後始末が
                                    # その書き足しを見て置く (2026-08-25 裁定の型)。
                                    # 確定先はこの行を作った部屋、連番は
                                    # スペル側の器の値 (2 周目以降は 0 から
                                    # 積み直しているので閉包の値では衝突する)。
                                    # 音声のストリームもこの確定が閉じる。
                                    LOGGER.warning(
                                        "[sea][pipeline] the closing beat produced "
                                        "no body — saving the draft row with the "
                                        "%d chars that did stream (msg=%s)",
                                        len(_partial_ns), pipeline_msg_id,
                                    )
                                    pipeline_sub_seq = _save_cut_utterance(
                                        runtime=runtime,
                                        persona=persona,
                                    ...
                                    pipeline_finalized = True
                                # どちらでもない回 = 一文字も流れていないのに
                                # 取り下げを断られた回 (誰かが既に読んでいた等)。
                                # 下の確定へ落ちる — 空の記録は残るが、確定しない
                                # 方 (= 永久に残る孤児の下書き行) よりは掃除の
                                # 当てがある。
                            elif event_callback and _final_has_body_ns:
                            ...
                            if pipeline_msg_id and not pipeline_finalized:
                                # 締めの本文が無い回 (retry が失敗して partial で
                                # 戻った等) でも確定は打つ — 打たないと本文の
                                # 入らない下書き行がそのまま残り、voice-tts の
                                # ストリームも閉じない。
                                pipeline_sub_seq += 1
                                _sf_result = _finalize_speak_with_signal(
                                    runtime, persona, _final_bid_ns, pipeline_msg_id,
                                    _final_text_ns,
                                    pulse_id=pulse_id,
                                ...
                                if _closing_saved_ns:
                                    pipeline_finalized = True
                                    state["_last_message_id"] = pipeline_msg_id
                                    # 締めの本文の形 (/quick_spell 終端の周の本文
                                    # など) を記録に書き足す。
                                    if (
                                        _final_seg_ns is not None
                                        and _final_seg_ns.form != SAVED_FORM_COMPLETE
                                    ):
                                        note_saved_utterance(
                                            getattr(persona, "persona_id", None),
                                            message_id=str(pipeline_msg_id),
                                            form=_final_seg_ns.form,
                                        )
                                LOGGER.info(
                                ...
                                if _final_stream_error_ns and _final_has_body_ns:
                                    # 締めの発言がサーバーに切られていた回
                                    # (docs/issues/spell_round_stream_cut_is_not_detected.md)。
                                    # 途切れたまま確定したので、記録に「途中で
                                    # 切れた本文 + この後で話が止まった」を書き
                                    # 足す。印と通告と画面への知らせは、返事の
                                    # 後始末がこの書き足しを見て一回だけ出す。
                                    _note_stream_cut(
                                        persona, _final_stream_error_ns,
                                        message_id=(
                                            str(pipeline_msg_id)
                                            if _closing_saved_ns else None
                                        ),
                                        reply_last_message_id=state.get(
                                            "_last_message_id"
                                        ),
                            ...
                            if _final_stream_error_ns and not _final_has_body_ns:
                                # 締めの周のストリームが、一文字も来ないうちに
                                # サーバーに切られた回。話は直前の発言 (スペルの
                                # 結果で終わる周の本文) の後で止まった — そこに
                                # 「この後で話が止まった」を付ける。
                                _note_stream_cut(
                                    persona, _final_stream_error_ns,
                                    message_id=None,
                            ...
                            if _final_stream_error_ns and _final_has_body_ns:
                                # 後段の memorize が記憶へ書く締めの発言にも、
                                # 途中で切れた印を付ける。
                                state[INTERRUPTED_METADATA_KEY] = True

                        # state["last"] が後段の memorize ノードで SAIMemory に
                        # 保存される。 spell が走った回は最終発言部分だけ
                        # (= continuation) を入れて、 SAIMemory には 「最終発言
                        # のみのレコード」 が単独で残るようにする
                        # (周ごとの発言はループが既に記録済み)。
                        text = _continuation_ns

                        # 続きの生成が失敗した / 返事を止める例外で止まった回。
                        # 下書き行を作れなかった回 (_pipeline_spell_state が None)
                        # はループがストリーミングを使わずに走るので、止まった
                        # 理由は例外ではなく戻り値に載って返る。それまでの周の
                        # 発言は上で建物へ書き終えたので、ここで理由を投げる —
                        # 投げないと取り消し・関所の閉鎖・使うモデルが無い回・
                        # 続きの失敗が握り潰され、返事が正常に終わったことになる
                        # (SpellLoopResult.stop_error の docstring。ツールモード・
                        # 全文一括・作業セッションの口と同じ)。
                        if _spell_result_ns.stop_error is not None:
                            raise _spell_result_ns.stop_error
                    else:
                        ...
                        # ここでやるのは、切れた本文を保存して「途中で切れた本文 +
                        # この後で話が止まった」を記録に書き足すことだけで、Beat は
                        # そのまま閉じる。「言い切っていない」印・中断の通告・画面への
                        # 知らせ (案内の材料つき) は、返事の後始末がこの書き足しを
                        # 見て一回だけ出す (docs/intent/reply_stop_exit.md)。
                        _stream_err = _final_stream_error_ns
                        _stream_cut_body = bool(_stream_err) and bool(text.strip())
                        if _stream_cut_body:
                            # 後段の memorize が記憶へ書く本文に、途中で切れた印を
                            # 付ける (記憶の側は本文そのものの事実)。
                            state[INTERRUPTED_METADATA_KEY] = True
                        ...
                        if _speak_base_metadata and isinstance(_speak_base_metadata, dict):
                            completion_event["metadata"] = _speak_base_metadata
                        # 停止された回は、上流で既に途中までの本文を保存している
                        # (保存の関数が確定済みの印を立てる)。完了の合図は重ねない。
                        ...
                        # 本文が**この場で**建物の記録へ載った行の id。切断の
                        # 書き足しはこの行にだけ付ける — 確定に失敗した回に付けると、
                        # 見えない行に印と通告が付く。
                        _partial_saved_id: Optional[str] = None
                        if pipeline_finalized:
                            # 停止の保存で既に確定させた回。ここで書き足すと同じ
                            # 本文が二度 Building に入る。下の救済経路は「下書き行を
                            # 作れなかった」ときのためのもので、「確定済み」はそれに
                            # 当たらない。
                            LOGGER.info(
                                "[sea][pipeline] already saved by the stop handling "
                            ...
                            if getattr(_sf_result, "status", None) == "saved":
                                pipeline_finalized = True
                                state["_last_message_id"] = pipeline_msg_id
                                _partial_saved_id = str(pipeline_msg_id)
                            LOGGER.info(
                        ...
                        if _stream_cut_body:
                            LOGGER.warning(
                                ...
                                "(code=%s); keeping the partial utterance and closing "
                                "the beat",
                                _stream_err.get("code"),
                            )
                            # この書き足しが無いと、会話の末尾が本人の途中発言の
                            # ままになり、続きの生成でプロンプト末尾がモデル発話に
                            # なる — プリフィルを受け付けない Gemini 3.x はそこで
                            # 拒否する (docs/issues/
                            #  server_cut_stream_writes_no_interruption_notice.md)。
                            # 通告は返事の後始末が、載った行の部屋へ置く。
                            _note_stream_cut(
                                persona, _stream_err,
                                message_id=_partial_saved_id,
                                reply_last_message_id=state.get("_last_message_id"),
                                event_callback=event_callback,
                            )
                    ...
                    # 続きの失敗 (LLMError) だけでなく、返事を止める例外
                    # (取り消し・関所の閉鎖) も入る (SpellLoopResult.stop_error)。
                    _spell_stop_error_sync: Optional[BaseException] = None
                    ...
                    if speak_flag is True:
                        ...
                        if _spell_loop_count_sync > 0:
                            # Beat ごとに 1 件ずつ、その Beat の生成が始まった
                            # 部屋へ (2026-09-12。それ以前は merged 全文を 1 件に
                            # まとめて 1 部屋へ出していた)。早期 emit 済みの
                            # 先頭部分は最初の Beat から切り落とす。
                            _sync_say_extra: Dict[str, Any] = {}
                            if reasoning_text:
                                _sync_say_extra["reasoning"] = reasoning_text
                            if _at_speak:
                                _sync_say_extra["activity_trace"] = list(_at_speak)
                            _sync_said = _emit_beat_segments(
                    ...
                    # 周ごとの本文 (<user_only> + spellResult HTML 含む) を
