...
**Local preferences**: If `Codex.local.md` exists in the repository root, read it for additional context (names, personal preferences, etc.).
...
**IMPORTANT for Codex**: After writing or modifying Python code, always run `ruff check` on the changed files before considering the task complete. This catches undefined variables (like `LOGGER` instead of `logging`), unused imports, and other common errors that would cause runtime failures.
