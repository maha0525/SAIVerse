        ...
        <ModalOverlay onClose={onClose} className={styles.overlay}>
            <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
                <div className={styles.header}>
                    <h2 data-i18n="components.MemoryModal.text001" className={styles.title}>{personaName || personaId}{uiText("components.MemoryModal.text001")}</h2>
                ...
                <div className={styles.tabs}>
                    <button data-i18n="components.MemoryModal.text002"
                        className={`${styles.tab} ${activeTab === 'browser' ? styles.activeTab : ''}`}
                        onClick={() => setActiveTab('browser')}
                    >
                        <MessageSquare size={16} style={{ display: 'inline', marginRight: 8, verticalAlign: 'text-bottom' }} />{uiText("components.MemoryModal.text002")}</button>
                    <button data-i18n="components.MemoryModal.text003"
                        className={`${styles.tab} ${activeTab === 'core_memory' ? styles.activeTab : ''}`}
                        onClick={() => setActiveTab('core_memory')}
                    >
                        <Anchor size={16} style={{ display: 'inline', marginRight: 8, verticalAlign: 'text-bottom' }} />{uiText("components.MemoryModal.text003")}</button>
                    {/* 手帳 (autonomous_behavior_v3.md §13)。ペルソナが自分で書く
                        アクティビティ・メモと、引き受けている約束 (タスク帳) の
                        読み口。v0.3 は読むだけ。 */}
                    <button data-i18n="components.MemoryModal.text004"
                        className={`${styles.tab} ${activeTab === 'pocketbook' ? styles.activeTab : ''}`}
                        onClick={() => setActiveTab('pocketbook')}
                    >
                        <Notebook size={16} style={{ display: 'inline', marginRight: 8, verticalAlign: 'text-bottom' }} />{uiText("components.MemoryModal.text004")}</button>
                    <button
                    ...
                    >
                        ...
                        {uiText("components.MemoryModal.label001")}</button>
                    <button
                    ...
                    >
                        ...
                        {uiText("components.MemoryModal.label002")}</button>
                    {/* 経験の台帳 (experience_ledger.md §3)。コア記憶タブ拡張でなく
                        新タブ: コア記憶=常駐・編集可の実データ / 台帳=参照専用の
                        動的合成ビュー、という性質差を画面でも分ける。 */}
                    {!isHidden('experience') && (
                        <button data-i18n="components.MemoryModal.text005"
                            className={`${styles.tab} ${activeTab === 'experience' ? styles.activeTab : ''}`}
                            onClick={() => setActiveTab('experience')}
                        >
                            <Footprints size={16} style={{ display: 'inline', marginRight: 8, verticalAlign: 'text-bottom' }} />{uiText("components.MemoryModal.text005")}</button>
                    )}
                    {!isHidden('pulse_timeline') && (
                        <button data-i18n="components.MemoryModal.text006"
                            className={`${styles.tab} ${activeTab === 'pulse_timeline' ? styles.activeTab : ''}`}
                            onClick={() => setActiveTab('pulse_timeline')}
                        >
                            <Activity size={16} style={{ display: 'inline', marginRight: 8, verticalAlign: 'text-bottom' }} />{uiText("components.MemoryModal.text006")}</button>
                    )}
                    <button data-i18n="components.MemoryModal.text007"
                        className={`${styles.tab} ${activeTab === 'import' ? styles.activeTab : ''}`}
                        onClick={() => setActiveTab('import')}
                    >
                        <Download size={16} style={{ display: 'inline', marginRight: 8, verticalAlign: 'text-bottom' }} />{uiText("components.MemoryModal.text007")}</button>
                    <button data-i18n="components.MemoryModal.text008"
                        className={`${styles.tab} ${activeTab === 'debug' ? styles.activeTab : ''}`}
                        onClick={() => setActiveTab('debug')}
                    >
                        <Bug size={16} style={{ display: 'inline', marginRight: 8, verticalAlign: 'text-bottom' }} />{uiText("components.MemoryModal.text008")}</button>
                </div>
